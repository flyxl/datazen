...
| 5 | Workflows 后端 (WorkflowRegistry + WorkflowExecutor) | ✅ 已完成 | ✅ 9 pass | — | — |
| 5 | Workflows 前端 (WorkflowPanel + Chat集成) | ✅ 已完成 | ✅ TS 编译通过 | 🔲 | — |
| 5 | Workflows MCP 集成 (list_workflows + run_workflow) | ✅ 已完成 | ✅ 编译通过 | 🔲 | — |
...
- **完成时间**: 2026-08-01
- **变更**:
  - `src-tauri/src/mcp/workflows.rs` — 完整 Workflows 系统实现
    - YAML 格式的 Workflow 定义（query + ai 步骤类型）
    - WorkflowRegistry — 从文件系统加载/保存/删除/列表
    - WorkflowExecutor — 顺序执行步骤（SQL 查询 + AI 推理）
    - WorkflowContext — 变量替换（{{var}} + {{steps.id.result}} + 内置变量）
    - 路径遍历防护（ID 校验）
    - 必填变量验证 + 默认值填充
    - 查询结果行数限制（1000）
    - 9 个单元测试
  - `src-tauri/src/commands/ai.rs` — 5 个 IPC 命令（list/execute/save/delete/reload）
  - `src-tauri/src/commands/mod.rs` — AppState 添加 workflow_registry
  - `src-tauri/src/lib.rs` — 初始化 WorkflowRegistry + 注册命令
  - 依赖: serde_yaml, regex

### Workflows MCP 集成
- **完成时间**: 2026-08-01
- **变更**:
  - `src-tauri/src/mcp/server.rs` — 新增 `list_workflows` + `run_workflow` tools + `datazen://workflows` resource

### Workflows 前端
- **完成时间**: 2026-08-01
- **变更**:
  - `src/types/index.ts` — Workflow 相关类型定义
  - `src/commands/ai.ts` — Workflow IPC 封装
  - `src/stores/aiStore.ts` — Workflow 状态管理
  - `src/components/ai/WorkflowPanel.tsx` — Workflow 选择、变量输入、执行、结果展示
  - `src/components/ai/AiChatPanel.tsx` — Chat/Workflows 标签页切换
  - `src/locales/zh-CN.ts` + `en.ts` — Workflows 翻译键

...
This document covers the four optional engines called out in the P2 roadmap: **MongoDB**, **ClickHouse**, **DuckDB**, and **SQL Server**. Git-based drivers (Kiwi, OLAP, Superset) are documented in [`plugin-development.md`](plugin-development.md).
...
See [plugin-development.md](plugin-development.md) — path drivers use the same `DatabaseDriver` trait and `.drivers-dev.json` symlink workflow as git plugins, but the source stays in `packages/drivers/<id>/`.

## Related docs

- [AGENTS.md](../AGENTS.md) — driver selection, codegen, capabilities
- [plugin-development.md](plugin-development.md) — git plugins and trait reference

...
所有子窗口通过 `src/lib/windowManager.ts` 的 `openWindow()` 函数创建：
- Tauri 环境下调用 Rust 命令 `create_sub_window`（确保 macOS `acceptFirstMouse`）
- 浏览器开发环境下 `window.open()` 降级
- Settings 窗口使用 singleton 模式（`settings-singleton` label）

### 17.2 ErrorBoundary

全局 `ErrorBoundary` 组件（`src/components/ErrorBoundary.tsx`）包裹整个应用：
- 捕获未处理的 React 渲染错误

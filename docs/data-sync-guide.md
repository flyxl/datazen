...
> 架构说明见 [data-sync.md](./architecture/backend/data-sync.md)；产品定义见 [data-synchronization-prd.zh-CN.md](./data-synchronization-prd.zh-CN.md)。
...
1. **同方言族**：V1 仅 MySQL（含 MariaDB）↔ MySQL、PostgreSQL ↔ PostgreSQL。异构对（如 PG→MySQL）请用 **[Data Transfer](./data-transfer-guide.md)**。
...
结构不一致时：先用 **[Schema Diff](./schema-diff-guide.md)** 对齐 DDL，或改用 Transfer。
...
| 产品 | 做什么 | 何时用 |
|------|--------|--------|
| **[Schema Diff](./schema-diff-guide.md)** | 结构对比 + 受控 DDL 部署 | 列/索引/PK 不一致，要先改结构 |
| **Data Sync（本文）** | 结构一致 + 相同 PK → 行 Diff → 审查 → DML | 两库表结构已对齐，补行级差异 |
| **[Data Transfer](./data-transfer-guide.md)** | 异构 / 结构不同 / 无 PK / 可建表 → 单向搬运 | 迁库、异构导入、空库灌数 |
...
- E2E 覆盖：[docs/e2e-coverage.md](./e2e-coverage.md)
- Transfer：[docs/data-transfer-guide.md](./data-transfer-guide.md)
- Schema Diff：[docs/schema-diff-guide.md](./schema-diff-guide.md)

...
5. **Plan** 步：先点击 **生成部署脚本**，再核对 SQL 与风险标记（`additive` / `destructive` / `rewrite`）
...
注意：Compare 步的差异列表与 Plan 步不是同一个状态对象。进入 Plan 步时，计划尚未生成，页面会先提示「生成部署脚本」；只有点击该按钮后，才会重新读取两端结构并生成 DDL。
...
| 现象 | 可能原因 |
|------|----------|
| 计划尚未生成 | 还未点击「生成部署脚本」，不代表没有差异 |
| 计划为空且无警告 | 按当前计划选项，确实没有可执行语句 |
| 计划为空但有警告 | 部分差异可能因破坏性操作或跨方言不支持而被跳过 |
| 对比显示 ADD，但实际生成计划仍为空 | 先确认已经点击「生成部署脚本」且期间数据库结构未变化；同一快照下仍复现且无警告时，应视为异常并提交复现信息 |

...
const NON_TABLE_CREATE_OBJECTS = new Set([
  'AS',
  'SELECT',
  'WITH',
  'VIEW',
...
export function buildOracleSyntaxDiagnostics(
  source: string,
...
function columnDefinitionRange(tokens: readonly OracleSyntaxToken[]): TokenRange | null {
  ...
  if (tokens[definitionStart]?.kind === 'word' && tokens[definitionStart].value === 'COLUMN')
    definitionStart += 1;
...
function tokenRangeInsideParentheses(
  tokens: readonly OracleSyntaxToken[],
  openIndex: number,
): TokenRange {
...
function splitColumnDefinitions(
  tokens: readonly OracleSyntaxToken[],
  range: TokenRange,
): OracleSyntaxToken[][] {
...
function defaultAfterNotNull(definition: readonly OracleSyntaxToken[]): OracleSyntaxToken | null {
  const notNullIndex = definition.findIndex(
    (token, index) =>
      token.kind === 'word' &&
      token.value === 'NOT' &&
      definition[index + 1]?.kind === 'word' &&
      definition[index + 1].value === 'NULL',
  );
  if (notNullIndex === -1) return null;
  return (
    definition
      .slice(notNullIndex + 2)
      .find((token) => token.kind === 'word' && token.value === 'DEFAULT') ?? null
  );
}

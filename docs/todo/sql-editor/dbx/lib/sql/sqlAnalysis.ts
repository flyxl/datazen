...
const POSTGRES_FOLDED_IDENTIFIER_TYPES = new Set([
  'postgres',
  'redshift',
  'gaussdb',
  'highgo',
 ...
 */
export function resolveMetadataColumnName(
  databaseType: string,
  sourceName: string,
  sourceNameQuoted: boolean | undefined,
  ...
  if (sourceNameQuoted === undefined) {
    ...
    if (
      exact ||
      POSTGRES_FOLDED_IDENTIFIER_TYPES.has(databaseType) ||
      ORACLE_FOLDED_IDENTIFIER_TYPES.has(databaseType)
    )
      return exact;
    const caseOnlyMatches = metadataColumns.filter(
      (column) => column.toLowerCase() === sourceName.toLowerCase(),
    );
  ...
  for (const column of analysis.columns) {
    ...
    for (const tableColumn of tableSource.columns) {
      expanded.push({
        ...
        expression: column.sourceQualifier
          ? `${column.sourceQualifier}.${tableColumn.name}`
          : tableColumn.name,
      });
    }
  }
...
function resolveProjectionColumnToSource(
  ...
  for (const tableSource of tableSources) {
    ...
    if (canonicalName)
      matches.push({ sourceKey: tableSource.source.key, sourceColumn: canonicalName });
  }
...
export function analyzeEditableQueryEditability(sql: string): QueryEditability {
  ...
  const selectBodyWithoutDistinct = distinct
    ? rawSelectBody.replace(/^DISTINCT\b/i, '').trimStart()
    : rawSelectBody;
  ...
  if (distinct && /^ON\b/i.test(selectBodyWithoutDistinct))
  ...
  if (
    forIndex >= 0 &&
    !/^FOR\s+(?:UPDATE|SHARE|NO\s+KEY\s+UPDATE|KEY\s+SHARE)\b/i.test(normalized.slice(forIndex))
  )
    return { editable: false, reason: 'complex-source' };
  const fromEnd = firstTopLevelKeywordIndex(
    normalized,
    ['WHERE', 'ORDER', 'LIMIT', 'OFFSET', 'FETCH', 'FOR'],
    fromIndex + 4,
  );
  const fromBody = normalized
    .slice(fromIndex + 4, fromEnd < 0 ? normalized.length : fromEnd)
    .trim();
...
export function analyzeSelectStructureForDisplay(sql: string): EditableQueryInfo | null {
  ...
  const fromBody = normalized
    .slice(fromIndex + 'FROM'.length, fromEnd < 0 ? normalized.length : fromEnd)
    .trim();
  ...
  const hasWindowClause =
    findTopLevelKeyword(normalized, 'OVER', 'SELECT'.length) >= 0 ||
    findTopLevelKeyword(normalized, 'WINDOW', fromIndex + 'FROM'.length) >= 0;
...
function parseGroupByColumns(
  sql: string,
  groupIndex: number,
  sources: EditableQuerySource[],
): EditableQueryColumn[] | undefined {
  ...
  const bodyEnd = firstTopLevelKeywordIndex(
    sql,
    ['HAVING', 'ORDER', 'LIMIT', 'OFFSET', 'FETCH', 'FOR', 'WINDOW'],
    bodyStart,
  );
...
function parseStarSelectColumn(
  col: string,
  sources?: EditableQuerySource[],
): EditableQueryColumn | null {
  if (col === '*') {
  ...
  const starMatch = col.match(
    /^((?:[\p{ID_Start}_][\p{ID_Continue}$]*|"[^"]+"|`[^`]+`|\[[^\]]+\]))\s*\.\s*\*$/u,
  );
...
function parseComputedSelectColumn(col: string): EditableQueryColumn | null {
  ...
  const alias =
    parseExpressionAlias(col) ??
    (looksLikeComputedExpression(expression) ? { expression, resultName: expression } : null);
...
function parseExpressionAlias(col: string): { expression: string; resultName: string } | null {
  ...
  if (index !== undefined) {
    ...
    if (
      alias &&
      alias.end === aliasText.length &&
      (alias.quoted || !isReservedImplicitAlias(alias.value)) &&
      looksLikeComputedExpression(expression)
    ) {
      return { expression, resultName: alias.value };
    }
  }
...
function looksLikeComputedExpression(expression: string): boolean {
  ...
  return (
    /^[()+\-*/%=<>!|&^:?]/u.test(rest) ||
    /^(?:IS|IN|LIKE|ILIKE|BETWEEN|COLLATE|AT|DIV|MOD|REGEXP|RLIKE)\b/iu.test(rest) ||
    /^(?:CASE|NOT)\b/iu.test(trimmed)
  );
}
...
function parseTableSourceAt(
  text: string,
  start: number,
  index: number,
): { source: EditableQuerySource; end: number } | null {
...
function isExternalFromSource(body: string): boolean {
  const trimmed = body.trim();
  return (
    /^'(?:''|[^'])*'(?:\s+(?:AS\s+)?[A-Za-z_][\w$]*)?$/i.test(trimmed) ||
...
function tableSourceTerminatorAt(text: string, pos: number): boolean {
  ...
  return [
    ...
    'OUTER',
    'NATURAL',
  ].some((keyword) => startsWithKeyword(text, pos, keyword));
}
...
function sourceKeyForQualifier(
  sources: EditableQuerySource[] | undefined,
  qualifier: string,
): string | undefined {
  ...
  const matches = sources.filter(
    (source) =>
      (source.alias ?? source.tableName).toLowerCase() === normalizedQualifier ||
      source.tableName.toLowerCase() === normalizedQualifier,
  );
...
function matchColumnsForResult(
  analysis: EditableQueryInfo,
  resultColumns: string[],
): EditableQueryColumn[] | undefined {
  ...
  for (const resultColumn of resultColumns) {
    let matchIndex = analysis.columns.findIndex(
      (column, index) => index >= searchFrom && column.resultName === resultColumn,
    );
    if (matchIndex < 0) {
      const normalized = resultColumn.toLowerCase();
      const caseOnlyMatches = analysis.columns.flatMap((column, index) =>
        index >= searchFrom && column.resultName.toLowerCase() === normalized ? [index] : [],
      );
...
export function allEditableColumnsWriteable(
  analysis: EditableQueryInfo,
  resultColumns: string[],
  sourceKey?: string,
): boolean {
  ...
  return (
    !!matchedColumns &&
    matchedColumns.every(
      (source) => !sourceKey || !source.sourceName || source.sourceKey === sourceKey,
    )
  );
}

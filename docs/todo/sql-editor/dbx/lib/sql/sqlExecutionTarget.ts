...
export function isSqlExecutionSnapshot(
  value: SqlExecutionOverride | undefined,
): value is SqlExecutionSnapshot {
  return (
    typeof value === 'object' &&
    value !== null &&
    typeof value.fullSql === 'string' &&
    typeof value.selectedSql === 'string' &&
    typeof value.cursorPos === 'number' &&
    typeof value.selectionFrom === 'number' &&
    typeof value.selectionTo === 'number'
  );
}

export function executionCandidateForMode(
  candidates: SqlExecutionCandidate[],
  mode: ExecuteMode,
  options: { executeAllOnBlankLine?: boolean } = {},
): SqlExecutionCandidate | null {

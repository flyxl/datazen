...
export type QueryContextObjectAction =
  | 'view-data'
...
export type QueryContextObjectRoute =
  ...
  | {
      event: 'openObjectSource';
      payload: [target: SqlObjectNavigationTarget, initialEditing: boolean];
    }
...
export function queryCursorTableCandidate(
...
export function queryTableCandidateAtSqlPosition(
  input: QueryTableCandidateAtPositionInput,
): QueryCursorTableCandidate | null {
  ...
  const parts = extractQualifiedIdentifierPartsAt(input.sql, input.position).map((part) =>
    dialect.normalizeIdentifier(part.value, part.quoted),
  );
  ...
  if (parts.length >= 3) {
    ...
    if (
      input.databaseType &&
      !isSchemaAware(input.databaseType) &&
      !isSingleDatabase(input.databaseType)
    ) {
      database = parts[0];
      schema = undefined;
    } else {
...
export function queryTableNavigationTargetAtSqlPosition(
  input: QueryTableCandidateAtPositionInput,
...
export function resolveQueryContextCandidateDatabase(
  candidate: QueryCursorTableCandidate,
...
export function resolveQueryContextObjectTarget(
  candidate: QueryCursorTableCandidate,
  tables: readonly SqlCompletionTable[],
): SqlObjectNavigationTarget {
  const nameMatches = tables.filter((table) => sameIdentifier(table.name, candidate.tableName));
  const match = candidate.schema
    ? (nameMatches.find((table) => sameIdentifier(table.schema, candidate.schema)) ??
      nameMatches.find((table) => !table.schema))
    : nameMatches[0];
  return sqlObjectNavigationTarget({
...
export function queryContextObjectActions(
  type?: SqlObjectNavigationType,
): QueryContextObjectAction[] {
...
export function queryContextObjectRoute(
  action: QueryContextObjectAction,
  target: SqlObjectNavigationTarget,
...
export function queryContextTargetFromCandidate(
  tab: QueryTab | undefined | null,
  candidate?: QueryCursorTableCandidate | null,
): ActiveTabSidebarTarget | null {
  if (!tab || tab.mode !== 'query' || !tab.connectionId || !tab.database) return null;
  return {
    type: 'query-context',
    connectionId: tab.connectionId,
...
export function findLoadedTableTargetForCandidate(

 ...
 *   the SQL formatter.
 */
export type StructuredFormatResult =
  | { kind: 'sql' }
  | { kind: 'json' | 'xml'; formatted: string }
  | { kind: 'unsupported'; detectedType: AutoDetectedStructuredType };
 ...
 */
export function detectAndFormatStructured(
  text: string,
  options: { indentSize: number; useTabs?: boolean },
): StructuredFormatResult {
  ...
  if (looksLikeXml(text)) {
    try {
      return {
        kind: 'xml',
        formatted: formatXmlSource(text, options.useTabs ? '\t' : ' '.repeat(options.indentSize)),
      };
    } catch {

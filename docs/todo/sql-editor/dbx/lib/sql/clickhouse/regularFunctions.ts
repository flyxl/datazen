...
const CLICKHOUSE_REGULAR_NAMES_BY_CATEGORY: Partial<Record<ClickHouseFunctionCategory, string>> = {
  ...
  nullable:
    'assumeNotNull coalesce firstNonDefault ifNull isNotNull isNull isNullable isZeroOrNull nullIf',
  random:
  ...
  window:
    'cume_dist dense_rank first_value lag lagInFrame last_value lead leadInFrame nth_value ntile percent_rank rank row_number',
  other:
...
const SIGNATURE_OVERRIDES: Record<string, ClickHouseFunctionSignature[]> = {
  ...
  toStartOfInterval: [
    ...
    sig(['value', 'INTERVAL x unit', 'time_zone']),
    sig(['value', 'INTERVAL x unit', 'origin', 'time_zone?']),
  ],
...
export const CLICKHOUSE_WINDOW_FUNCTION_NAMES = new Set([
  'cume_dist',
  'dense_rank',
  'denseRank',
  'first_value',
...
function definition(
  ...
  const signatures = Object.prototype.hasOwnProperty.call(SIGNATURE_OVERRIDES, name)
    ...
    : [sig(ZERO_ARGUMENT_FUNCTIONS.has(name) ? [] : ['argument', '...arguments?'])];
  const aliases = Object.prototype.hasOwnProperty.call(CLICKHOUSE_ALIASES_BY_CANONICAL, name)
    ? CLICKHOUSE_ALIASES_BY_CANONICAL[name]
    : undefined;
  return {
    name,
    kind,
...
export const CLICKHOUSE_REGULAR_FUNCTIONS: ClickHouseFunctionDefinition[] = Object.entries(
  CLICKHOUSE_REGULAR_NAMES_BY_CATEGORY,

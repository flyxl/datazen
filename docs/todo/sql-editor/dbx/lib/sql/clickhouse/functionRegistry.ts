...
function validateDefinition(definition: ClickHouseFunctionDefinition): void {
  ...
  if (definition.signatures.length === 0)
    throw new Error(`ClickHouse function ${definition.name} must define a signature`);
...
export function createClickHouseFunctionRegistry(
  definitions: readonly ClickHouseFunctionDefinition[],
): ClickHouseFunctionRegistry {
      ...
      return ordered
        .filter(
          (definition) =>
            ...
            definitionKeys(definition).some((key) => key.startsWith(normalized)),
        )
        .slice(0, limit);
    },
    all: () => ordered,

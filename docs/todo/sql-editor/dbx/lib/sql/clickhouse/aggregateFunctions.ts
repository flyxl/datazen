...
const aggregate = (name: string): ClickHouseFunctionDefinition => {
  const signatures = Object.prototype.hasOwnProperty.call(SIGNATURE_OVERRIDES, name)
    ? SIGNATURE_OVERRIDES[name]
    : [sig(['expression', '...expressions?'])];
  const aliases = Object.prototype.hasOwnProperty.call(CLICKHOUSE_AGGREGATE_ALIASES, name)
    ? CLICKHOUSE_AGGREGATE_ALIASES[name]
    : undefined;
  return {
    name,
    kind: 'aggregate',
...
export const CLICKHOUSE_AGGREGATE_FUNCTIONS: ClickHouseFunctionDefinition[] =
  CLICKHOUSE_AGGREGATE_NAMES.split(' ')
    .filter(Boolean)

...
function transformLastGroup(
  signature: ClickHouseFunctionSignature,
  transform: (group: string[]) => string[],
): ClickHouseFunctionSignature {
...
function applyCombinator(
  ...
  switch (combinator) {
    ...
    case 'Resample': {
      ...
      transformed.parameterGroups.splice(Math.max(0, transformed.parameterGroups.length - 1), 0, [
        'start',
        'end',
        'step',
      ]);
...
function generateDefinition(
  base: ClickHouseFunctionDefinition,
  sequence: readonly CombinatorName[],
): ClickHouseFunctionDefinition {
  const signatures = base.signatures.map((signature) =>
    sequence.reduce(applyCombinator, signature),
  );
  return {
    ...base,
...
export function generateAggregateCombinatorCandidates(
  prefix: string,
  limit: number,
): ClickHouseFunctionDefinition[] {
  if (limit <= 0) return [];
  ...
  for (const base of CLICKHOUSE_AGGREGATE_FUNCTIONS) {
    ...
    if (normalized && !baseName.startsWith(normalized) && !normalized.startsWith(baseName))
      continue;

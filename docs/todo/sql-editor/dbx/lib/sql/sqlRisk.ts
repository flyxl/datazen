import {
  classifyElasticsearchRequestRisk,
  classifyElasticsearchSourceRisk,
  type ElasticsearchRequestRisk,
} from '@/lib/elasticsearch/elasticsearchRequestRisk';
import {
...
const EXPLAIN_OPTION_KEYWORDS = new Set([
  'explain',
  'analyze',
  'analyse',
  'verbose',
...
const PRIMARY_STATEMENT_KEYWORDS = new Set([
  ...READ_KEYWORDS,
  ...WRITE_KEYWORDS,
  ...DDL_KEYWORDS,
...
const SAFE_READ_PRAGMA_NAMES = new Set([
  'table_info',
  'table_xinfo',
  'index_list',
  'index_info',
...
export function classifySqlRisk(sql: string, options: SqlRiskOptions = {}): SqlRiskAssessment {
  ...
  const searchEngineRisk = searchEngineAssessment(
    sql,
    options.dialect,
    classifyElasticsearchSourceRisk,
  );
  ...
  const statements = splitSqlStatementsForSafety(sql).map((statement) =>
    classifySqlStatementRisk(statement, options),
  );
  if (!statements.length) return { risk: 'unknown', statements: [] };
  const highest = statements.reduce<SqlRiskStatementAssessment>(
    (current, statement) =>
      RISK_ORDER[statement.risk] > RISK_ORDER[current.risk] ? statement : current,
    { risk: 'read' },
  );
  return { ...highest, statements };
}

export function classifySqlStatementRisk(
  sql: string,
  options: SqlRiskOptions = {},
): SqlRiskStatementAssessment {
  const mongoStatements = mongoShellStatements(sql, options.dialect);
...
function searchEngineAssessment(
  ...
  classify: (value: string) => ElasticsearchRequestRisk | null,
  ...
  if (!risk) return null;
  return {
    risk: risk === 'read' ? 'read' : risk === 'write' ? 'write' : 'ddl',
    firstKeyword: 'rest',
  };
}

const MONGO_READ_KINDS = new Set<MongoCommand['kind']>([
  'find',
  'findOne',
  'countDocuments',
  'distinct',
...
const MONGO_DDL_KINDS = new Set<MongoCommand['kind']>([
  'createIndex',
  'dropIndex',
  'dropIndexes',
  'dropCollection',
...
function highestRiskStatement(
  statements: SqlRiskStatementAssessment[],
): SqlRiskStatementAssessment {
  return statements.reduce<SqlRiskStatementAssessment>(
    (current, statement) =>
      RISK_ORDER[statement.risk] > RISK_ORDER[current.risk] ? statement : current,
    { risk: 'read' },
  );
...
function mongoCommandRisk(command: MongoCommand): SqlRiskLevel {
  ...
  if (command.kind === 'aggregate')
    return mongoAggregateWriteStage(command.pipeline) ? 'write' : 'read';
  if (command.kind === 'runCommand') return 'unknown';
  if (MONGO_DDL_KINDS.has(command.kind)) return 'ddl';
  return 'write';
}

function classifyDynamoDbStatementRisk(
  sql: string,
  dialect?: DatabaseType | string,
): SqlRiskStatementAssessment | null {
  ...
  if (
    header === 'DBX DYNAMODB INSERT ITEM' ||
    header === 'DBX DYNAMODB PUT ITEM' ||
    header === 'DBX DYNAMODB DELETE ITEM'
  ) {
...
function classifyTokens(tokens: SqlRiskToken[]): SqlRiskStatementAssessment {
  ...
  if (READ_KEYWORDS.has(firstKeyword)) {
    return {
      risk: firstKeyword === 'select' && hasTopLevelSelectInto(useful) ? 'write' : 'read',
      firstKeyword,
    };
  }
...
function classifyExplainTokens(tokens: SqlRiskToken[]): SqlRiskStatementAssessment {
  const analyze = tokens.some(
    (token) => token.normalized === 'analyze' || token.normalized === 'analyse',
  );
  const innerIndex = tokens.findIndex(
    (token, index) =>
      index > 0 &&
      PRIMARY_STATEMENT_KEYWORDS.has(token.normalized) &&
      !EXPLAIN_OPTION_KEYWORDS.has(token.normalized),
  );
  if (innerIndex < 0) return { risk: 'read', firstKeyword: 'explain' };
  const inner = classifyTokens(tokens.slice(innerIndex));
  if (!analyze)
    return { risk: inner.risk === 'unknown' ? 'unknown' : 'read', firstKeyword: 'explain' };
...
function classifyPragmaTokens(tokens: SqlRiskToken[]): SqlRiskLevel {
  ...
  if (name && SAFE_READ_PRAGMA_NAMES.has(name) && !tokens.some((token) => token.text === '='))
    return 'read';
  return 'write';
}
...
function highestRiskInTokens(tokens: SqlRiskToken[]): SqlRiskLevel | undefined {
  ...
  for (const token of tokens) {
    const risk = WRITE_KEYWORDS.has(token.normalized)
      ? 'write'
      : DDL_KEYWORDS.has(token.normalized)
        ? 'ddl'
        : TRANSACTION_KEYWORDS.has(token.normalized)
          ? 'transaction'
          : undefined;

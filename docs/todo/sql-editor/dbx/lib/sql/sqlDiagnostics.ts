...
function oracleInvalidIdentifierRange(
  sql: string,
  message: string,
  position: number,
): SqlErrorRange | null {
  const identifierMatch = /\bORA-00904:\s*"((?:""|[^"])*)"\s*:\s*invalid identifier\b/i.exec(
    message,
  );
  ...
  for (const match of sql.matchAll(identifierPattern)) {
    ...
    if (
      quoted
        ...
        : identifier.toUpperCase() !== reportedIdentifier.toUpperCase()
    )
      continue;
...
export function sqlErrorDecorationRange(sql: string, message: string): SqlErrorRange | null {
  ...
  return (
    oracleInvalidIdentifierRange(sql, message, position) ?? { from: position, to: position + 1 }
  );
}

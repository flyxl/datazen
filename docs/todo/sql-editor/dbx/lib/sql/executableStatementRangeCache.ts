...
export type ExecutableStatementRangeParser = (
  sql: string,
  databaseType?: DatabaseType,
  parameterOptions?: SqlParameterOptions,
) => SqlTextRange[];
...
export function executableStatementRangeCacheForDoc(
  ...
  const parameterOptions =
    typeof parameterOptionsOrParse === 'function' ? undefined : parameterOptionsOrParse;
  const parse =
    typeof parameterOptionsOrParse === 'function' ? parameterOptionsOrParse : customParse;
  const parameterSyntaxKey = parameterOptions?.enabledSyntaxes
    ? parameterOptions.enabledSyntaxes.join(',')
    : '*';
  if (
    cache?.doc === doc &&
    cache.databaseType === databaseType &&
    cache.parameterSyntaxKey === parameterSyntaxKey
  )
    return cache;
  ...
  for (const range of ranges) {
    ...
    const executableStart = executableStartAfterLeadingDirective(
      sql,
      range,
      databaseType,
      parameterOptions,
  ...
  return {
    ...
    parameterSyntaxKey,
    byStart,
    byExecutableLineStart,
    ranges,
  };
}

function executableStartAfterLeadingDirective(
  sql: string,
  range: SqlTextRange,
  databaseType?: DatabaseType,
  parameterOptions?: SqlParameterOptions,
  ...
  const hasExecutableDirective =
    text.startsWith('/*+') ||
    text.startsWith('/*@') ||
    text.startsWith('/*&') ||
    (databaseType === 'mysql' && text.startsWith('/*proxy*/'));
  ...
  while (offset < text.length) {
    ...
    if (
      ...
      text[offset] === '#' &&
      readSqlBracedParameterAt(sql, range.from + offset, parameterOptions)?.syntax !== 'mybatis'
    ) {
      const newline = text.indexOf('\n', offset + 1);
      if (newline < 0) return null;
      offset = newline + 1;
...
export function executableStatementRangeStartingAt(
  cache: ExecutableStatementRangeCache,
  lineFrom: number,
): SqlTextRange | null {
  return cache.byStart.get(lineFrom) ?? cache.byExecutableLineStart.get(lineFrom) ?? null;
}

export function executableStatementRangeAtCursor(
  cache: ExecutableStatementRangeCache,
  cursorPos: number,
): SqlTextRange | null {
  ...
  const startsHashComment =
    lineText.startsWith('#') &&
    readSqlBracedParameterAt(cache.doc.toString(), lineContentStart, cache.parameterOptions)
      ?.syntax !== 'mybatis';
  if (
    !lineText ||
    lineText.startsWith('--') ||
    startsHashComment ||
    isCursorOnLeadingBlockComment(line.text, pos - line.from)
  )
    return null;
  ...
  for (let index = 0; index < cache.ranges.length; index += 1) {
    ...
    if (
      pos < range.from &&
      range.from <= line.to &&
      cache.doc.sliceString(pos, range.from).trim() === ''
    ) {
      return range;
    }

...
type InsertValueHintColumn = Pick<SqlServerColumnMetadata, 'name'> &
  Partial<
    Pick<
      SqlServerColumnMetadata,
      'is_identity' | 'is_computed' | 'is_hidden' | 'generated_always_type'
    >
  >;

export function insertValueHintColumnNames(
  databaseType: DatabaseType | undefined,
  columns: readonly InsertValueHintColumn[],
): string[] {
  return columns
    .filter((column) => {
      ...
      return (
        ...
        !column.is_computed &&
        !column.is_hidden &&
        (column.generated_always_type ?? 0) === 0
      );
    })

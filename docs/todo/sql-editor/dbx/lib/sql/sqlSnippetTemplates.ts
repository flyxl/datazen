...
const SELECT_SNIPPET_LIMIT_STYLE_BY_DATABASE: Partial<
  Record<DatabaseType, SelectSnippetLimitStyle>
> = {
  oracle: 'rownum',
...
const PARENTHESIZED_ADD_COLUMN_DATABASES = new Set<DatabaseType>([
  'oracle',
  'oceanbase-oracle',
  'yashandb',
  'xugu',
...
const ADD_COLUMN_WITHOUT_COLUMN_KEYWORD_DATABASES = new Set<DatabaseType>([
  'sqlserver',
  'kingbase',
  'cassandra',
  'teradata',
...
function buildSelectSnippetBody(databaseType?: DatabaseType): string {
  const style = databaseType
    ? (SELECT_SNIPPET_LIMIT_STYLE_BY_DATABASE[databaseType] ?? 'limit')
    : 'limit';
  switch (style) {
    case 'top':
...
export function resolveSqlSnippetBodyForDatabase(
  snippet: SqlSnippet,

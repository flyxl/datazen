...
import {
  isElasticsearchCompatibleDatabaseType,
  isMeilisearchDatabaseType,
  type DatabaseType,
} from '@/types/database';
...
function isHttpJsonRestDatabaseType(databaseType?: DatabaseType): boolean {
  return (
    isElasticsearchCompatibleDatabaseType(databaseType) || isMeilisearchDatabaseType(databaseType)
  );
}

export function elasticsearchRestRequestRanges(
  sql: string,
  ...
  return requests.length > 0 &&
    requests.every((request) => ELASTICSEARCH_REST_REQUEST.test(request.sql))
    ? requests
    : [];
}

const NON_SQL_EXECUTION_TARGET_TYPES: ReadonlySet<DatabaseType> = new Set([
  'mongodb',
  'elasticsearch',
  'easysearch',
  'meilisearch',
...
export function supportsExecutionTargetPicker(databaseType?: DatabaseType): boolean {
  return (
    !!databaseType &&
    (databaseType === 'redis' ||
      isHttpJsonRestDatabaseType(databaseType) ||
      !NON_SQL_EXECUTION_TARGET_TYPES.has(databaseType))
  );
}

export function hasMultipleExecutionTargets(
  sql: string,
  databaseType?: DatabaseType,
...
function elasticsearchRequestLineCandidates(
  lines: ElasticsearchRequestLine[],
): ElasticsearchRequestLineCandidate[] {
...
export function parseElasticsearchRestRequestTarget(
  value: string,
): ElasticsearchRestRequestTarget | null {
...
function splitElasticsearchRestRequestRanges(sql: string): RawStatement[] | undefined {
  ...
  const firstRequestIndex = candidates.findIndex((candidate) =>
    isElasticsearchRequestPreamble(sql.slice(0, candidate.from)),
  );
  ...
  const hitFroms = requests.map((request, requestIndex) => {
    ...
    for (
      let preambleLine = previousRequestLine + 1;
      preambleLine < request.lineIndex;
      preambleLine += 1
    ) {
...
const EXPLAIN_STATEMENT_KEYWORDS = new Set([
  'SELECT',
  'WITH',
  'INSERT',
  'UPDATE',
...
const STARROCKS_CREATE_MATERIALIZED_VIEW_REFRESH_MODIFIERS = new Set([
  'ASYNC',
  'MANUAL',
  'SCHEDULE',
  'DEFERRED',
...
const CLICKHOUSE_ALTER_TABLE_HEADER =
  /^ALTER\s+TABLE\s+(?:(?:[A-Za-z_][\w$]*|`(?:``|[^`])+`|"(?:""|[^"])+")\s*\.\s*)?(?:[A-Za-z_][\w$]*|`(?:``|[^`])+`|"(?:""|[^"])+")(?:\s+ON\s+CLUSTER\s+(?:[A-Za-z_][\w$]*|`(?:``|[^`])+`|"(?:""|[^"])+"|'(?:''|[^'])+'))?\s*$/i;
...
const ORACLE_LIKE_PL_SQL_DATABASES: ReadonlySet<DatabaseType> = new Set([
  'oracle',
  'dameng',
  'gaussdb',
  'yashandb',
...
const MYSQL_ROUTINE_BLOCK_DATABASES: ReadonlySet<DatabaseType> = new Set([
  'mysql',
  'doris',
  'starrocks',
  'manticoresearch',
...
const MYSQL_CREATE_TABLE_OPTION_DATABASES: ReadonlySet<DatabaseType> = new Set([
  'mysql',
  'doris',
  'starrocks',
  'manticoresearch',
...
const MYSQL_NON_ROUTINE_CREATE_TYPES = new Set([
  'DATABASE',
  'INDEX',
  'LOGFILE',
  'ROLE',
...
export function splitSqlStatementRanges(
  ...
  const mysqlRoutineTokensUpTo = (to: number): MysqlRoutineToken[] => {
    if (!mysqlRoutineScan || mysqlRoutineScan.from !== statementStart) {
      mysqlRoutineScan = {
        from: statementStart,
        scannedTo: statementStart,
        lexState: 'none',
        tokens: [],
        completeness: newMysqlRoutineBlockCompleteness(),
      };
    }
    if (to > mysqlRoutineScan.scannedTo) {
      const { tokens: newTokens, endState } = mysqlRoutineTokens(
        sql,
        parameterOptions,
        mysqlRoutineScan.scannedTo,
        to,
        mysqlRoutineScan.lexState,
      );
  ...
  const markContent = (pos: number) => {
    if (statementStart === -1) {
      ...
      const directiveStart =
        ...
        (sql.startsWith('/*proxy*/', pendingMysqlDirectiveStart) ||
          pos < pendingMysqlDirectiveLineEnd)
          ? pendingMysqlDirectiveStart
          : -1;
      statementStart =
        directiveStart !== -1 ? directiveStart : pendingHintStart === -1 ? pos : pendingHintStart;
      pendingHintStart = -1;
      pendingMysqlDirectiveStart = -1;
  ...
  const flush = (to = statementEnd) => {
    ...
    if (trimmedTo > statementStart) {
      statements.push({
        hitFrom: statementHitStart,
        from: statementStart,
        to: trimmedTo,
        sql: sql.slice(statementStart, trimmedTo),
      });
    }
    statementStart = -1;
    statementEnd = -1;
  ...
  while (i < len) {
    ...
    if (ch === '/' && next === '*') {
      ...
      if (
        statementStart === -1 &&
        pendingHintStart === -1 &&
        (hintMarker === '+' || hintMarker === '@' || hintMarker === '&')
      )
        pendingHintStart = i;
    ...
    if (
      supportsSqlServerGoCommands(databaseType) &&
      isAtLineStart(sql, i) &&
      isSqlServerGoLine(sql, i)
    ) {
      const lineEnd = findLineEnd(sql, i);
      flush(i);
      i = nextLineStart(sql, lineEnd);
    ...
    if (!customDelimiter && ch === '$') {
      ...
      if (tagMatch) {
        ...
        if (
          ...
          statementStart !== -1 &&
          startsWithPostgresDollarQuotedRoutinePrefix(sql.slice(statementStart, i))
        ) {
    ...
    if (customDelimiter) {
      ...
      const routineTokensBeforeSemicolon =
        isMysqlRoutineBlockDatabase(databaseType) && statementStart !== -1
          ? mysqlRoutineTokensUpTo(i)
          : null;
      const isMysqlRoutineBlock =
        routineTokensBeforeSemicolon !== null &&
        isMysqlRoutineDdlStartFromWords(mysqlRoutineDdlStartWords(routineTokensBeforeSemicolon)) &&
        mysqlRoutineTokensContainBegin(routineTokensBeforeSemicolon);
      if (isMysqlRoutineBlock) {
        ...
        if (
          oraclePlSqlStatementEnd === undefined &&
          isOracleLikeDatabase(databaseType) &&
          !postgresDollarQuotedRoutine &&
          statementStart !== -1
        ) {
          const statementSoFar = sql.slice(statementStart, i);
          oraclePlSqlStatementEnd = startsWithOraclePlSqlBlock(statementSoFar)
            ? statementStart +
              (oraclePlSqlBlockEnd(sql.slice(statementStart)) ?? sql.length - statementStart)
            : null;
        }
        ...
        const isSapHanaScriptBlock =
          isSapHanaScriptBlockDatabase(databaseType) &&
          statementStart !== -1 &&
          startsWithSapHanaScriptBlock(sql.slice(statementStart, i));
        if (isOraclePlSql || isSapHanaScriptBlock) {
          ...
          if (
            isSapHanaScriptBlock &&
            !sapHanaScriptBlockIsComplete(sql.slice(statementStart, i + 1))
          ) {
            i += 1;
            continue;
          }
...
export function statementRangeAtCursor(
  ...
  for (let index = 0; index < statements.length; index += 1) {
    ...
    const softRanges = splitStatementRangeAtSoftStarts(
      sql,
      statement,
      databaseType,
      parameterOptions,
    ...
    if (
      pos > statement.to &&
      (!next || pos < next.from) &&
      isCursorInTrailingDelimiterGap(sql, statement.to, pos)
    ) {
      return (
        rangeForCursorInSoftRanges(sql, softRanges, pos) ??
        rangeFor(softRanges[softRanges.length - 1] ?? statement, sql)
      );
    }
    ...
    if (
      pos >= statement.hitFrom &&
      pos < statement.from &&
      (sql.slice(pos, statement.from).trim() === '' ||
        (isHttpJsonRestDatabaseType(databaseType) &&
          isElasticsearchRequestPreamble(sql.slice(statement.hitFrom, statement.from))))
    ) {
      const previous = statements[index - 1];
      if (previous && isCursorInTrailingDelimiterGap(sql, previous.to, pos)) {
        const previousSoftRanges = splitStatementRangeAtSoftStarts(
          sql,
          previous,
          databaseType,
          parameterOptions,
        ...
        return (
          rangeForCursorInSoftRanges(sql, previousSoftRanges, pos) ??
          rangeFor(previousSoftRanges[previousSoftRanges.length - 1] ?? previous, sql)
        );
      }
      return rangeForCursorInSoftRanges(sql, softRanges, pos) ?? rangeFor(statement, sql);
    }
...
export function mongoCommandRangeAtCursor(sql: string, cursorPos: number): SqlTextRange | null {
  ...
  for (let index = 0; index < commands.length; index += 1) {
    ...
    if (
      pos > command.to &&
      (!next || pos < next.from) &&
      isCursorInTrailingDelimiterGap(sql, command.to, pos)
    )
      return range;

    if (
      pos < command.from &&
      sql.slice(pos, command.from).trim() === '' &&
      isCursorOnStatementLine(sql, pos, command)
    )
      return range;
  }
...
function isCursorInTrailingDelimiterGap(
  sql: string,
  previousStatementEnd: number,
  cursorPos: number,
): boolean {
  return cursorBelongsToTrailingStatementDelimiter(sql, previousStatementEnd, cursorPos);
}

function rangeForCursorInSoftRanges(
  sql: string,
  ranges: RawStatement[],
  pos: number,
): SqlTextRange | null {
  for (let index = 0; index < ranges.length; index += 1) {
    ...
    if (
      pos > range.to &&
      (!next || pos < next.hitFrom) &&
      isCursorOnRangeEndLine(sql, pos, range)
    ) {
      return rangeFor(range, sql);
    }
  }
...
function splitStatementRangeAtSoftStarts(
  sql: string,
  ...
  if (
    isMysqlRoutineBlockDatabase(databaseType) &&
    startsWithMysqlRoutineBlock(statement.sql, parameterOptions)
  )
    return [statement];

  const lineStarts = topLevelSoftStatementLineStarts(
    sql,
    statement,
    databaseType,
    parameterOptions,
  ...
  let currentExplainTargetKeyword = explainLikeTargetKeywordAt(
    sql,
    statement.from,
    databaseType,
    parameterOptions,
  ...
  for (const lineStart of lineStarts) {
    ...
    if (
      currentBodyKeyword === 'WITH' &&
      !consumedWithMainStatement &&
      WITH_MAIN_STATEMENT_KEYWORDS.has(lineStart.keyword)
    ) {
    ...
    if (
      isSetOperationQueryContinuation(
        sql,
        statement.from,
        lineStart.from,
        lineStart.keyword,
    ...
    if (
      !consumedExplainStatement &&
      EXPLAIN_STATEMENT_KEYWORDS.has(lineStart.keyword) &&
      (currentKeyword === 'EXPLAIN' || currentExplainTargetKeyword !== null)
    ) {
      consumedExplainStatement = true;
      currentBodyKeyword = lineStart.keyword;
      continue;
    ...
    if (
      currentBodyKeyword === 'CREATE' &&
      isStarRocksCreateMaterializedViewRefreshContinuation(
        sql,
        statement.from,
        lineStart.from,
        lineStart.keyword,
    ...
    if (
      currentBodyKeyword === 'CREATE' &&
      isMysqlCreateTableOptionContinuation(
        sql,
        statement.from,
        lineStart.from,
        lineStart.keyword,
    ...
    if (
      currentBodyKeyword === 'ALTER' &&
      isClickHouseAlterTableUpdateContinuation(
        sql,
        boundaries[boundaries.length - 1].from,
        lineStart.from,
        lineStart.keyword,
        databaseType,
      )
    ...
    if (
      currentBodyKeyword === 'ALTER' &&
      isMysqlAlterTableTruncatePartitionContinuation(
        sql,
        boundaries[boundaries.length - 1].from,
        lineStart.from,
        lineStart.keyword,
    ...
    currentExplainTargetKeyword = explainLikeTargetKeywordAt(
      sql,
      lineStart.from,
      databaseType,
      parameterOptions,
  ...
  for (let index = 0; index < boundaries.length; index += 1) {
    ...
    const to = next
      ? trimRangeEndBeforeNextBoundary(
          sql,
          boundary.from,
          next.from,
          databaseType,
      ...
      : trimRangeEnd(sql, boundary.from, statement.to);
    if (to > boundary.from) {
      ranges.push({
...
function topLevelSoftStatementLineStarts(
  sql: string,
  statement: RawStatement,
  databaseType?: DatabaseType,
  parameterOptions?: SqlParameterOptions,
): Array<{ hitFrom: number; from: number; keyword: string }> {
  ...
  const unclosedExplainOptionsStart =
    explainOptionsStart !== null &&
    skipBalancedParens(sql, explainOptionsStart, databaseType, parameterOptions) === null
      ? explainOptionsStart
      : null;
  ...
  while (i < len) {
    ...
    if (
      ...
      ch !== '\r' &&
      !isSqlWhitespace(ch) &&
      !startsLineComment(sql, i, databaseType, parameterOptions) &&
      !startsBlockComment(sql, i)
    ) {
...
function softStatementKeywordAt(
  sql: string,
  pos: number,
  databaseType?: DatabaseType,
  parameterOptions?: SqlParameterOptions,
  ...
  if (
    SOFT_STATEMENT_FUNCTION_KEYWORDS.has(keyword) &&
    nextNonWhitespaceChar(sql, pos + match[0].length) === '('
  )
    return null;
  ...
  if (
    keyword === 'COMMENT' &&
    nextSqlWord(sql, pos + match[0].length, databaseType, parameterOptions) !== 'ON'
  )
    return null;
...
function softStatementStartKeywords(databaseType?: DatabaseType): Set<string> {
  return new Set([
    ...COMMON_SOFT_STATEMENT_START_KEYWORDS,
    ...(databaseType ? (DATABASE_SOFT_STATEMENT_KEYWORDS[databaseType] ?? []) : []),
  ]);
}

function isSetOperationQueryContinuation(
  sql: string,
  from: number,
  to: number,
  keyword: string,
...
function isMysqlCreateTableOptionContinuation(
  sql: string,
...
function isStarRocksCreateMaterializedViewRefreshContinuation(
  sql: string,
  statementFrom: number,
  lineStartFrom: number,
  keyword: string,
...
function isClickHouseAlterTableUpdateContinuation(
  sql: string,
  statementFrom: number,
  lineStartFrom: number,
  keyword: string,
...
function isMysqlAlterTableTruncatePartitionContinuation(
  sql: string,
  statementFrom: number,
  lineStartFrom: number,
  keyword: string,
...
function explainLikeTargetKeywordAt(
  sql: string,
  pos: number,
  databaseType?: DatabaseType,
  parameterOptions?: SqlParameterOptions,
...
function startsLineComment(
  ...
  return (
    (sql[pos] === '-' && sql[pos + 1] === '-') ||
    startsHashLineComment(sql, pos, databaseType, parameterOptions)
  );
}

function startsHashLineComment(
  sql: string,
  pos: number,
  databaseType?: DatabaseType,
  parameterOptions?: SqlParameterOptions,
...
function trimRangeEndBeforeNextBoundary(
  sql: string,
  from: number,
  nextBoundaryFrom: number,
  databaseType?: DatabaseType,
  parameterOptions?: SqlParameterOptions,
): number {
  let state: QuoteState | 'lineComment' | 'blockComment' = 'none';
...
function startsWithMysqlRoutineBlock(sql: string, parameterOptions?: SqlParameterOptions): boolean {
  return (
    isMysqlRoutineDdlStart(sql, parameterOptions) &&
    mysqlRoutineTokensContainBegin(mysqlRoutineTokens(sql, parameterOptions).tokens)
  );
}
...
function newMysqlRoutineBlockCompleteness(): MysqlRoutineBlockCompleteness {
  return {
    ...
    sawBegin: false,
    lastWordToken: null,
    pendingEndAwaitingSuffix: false,
    lastTokenKind: null,
  };
}

/** Folds any `tokens` not yet seen into `state` and returns whether the routine body is complete so far. */
function advanceMysqlRoutineBlockCompleteness(
  state: MysqlRoutineBlockCompleteness,
  tokens: readonly MysqlRoutineToken[],
): boolean {
 ...
 */
function mysqlRoutineTokens(
  sql: string,
  parameterOptions?: SqlParameterOptions,
  fromIndex = 0,
  toIndex = sql.length,
  initialState: MysqlRoutineLexState = 'none',
): { tokens: MysqlRoutineToken[]; endState: MysqlRoutineLexState } {
...
function startsWithOraclePlSqlBlockWords(words: readonly string[]): boolean {
  ...
  if (ORACLE_PL_SQL_BLOCK_STARTERS.has(first))
    return first !== 'BEGIN' || words[1] !== 'TRANSACTION';
...
function startsWithPostgresDollarQuotedRoutinePrefix(sql: string): boolean {
  ...
  return (
    (words[objectIndex] === 'FUNCTION' || words[objectIndex] === 'PROCEDURE') &&
    words[words.length - 1] === 'AS'
  );
}
...
function sapHanaScriptBlockIsComplete(sql: string): boolean {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (token.value === 'END') {
      ...
      const target = SAP_HANA_SCRIPT_BLOCK_TERMINATORS.has(next ?? '')
        ? next
        : top === 'CASE'
          ? 'CASE'
          : 'BLOCK';
...
function oraclePlSqlBlockEnd(sql: string): number | null {
  ...
  const stack: string[] =
    objectKind === 'body' ? ['OBJECT_BODY'] : objectKind === 'spec' ? ['OBJECT_SPEC'] : [];
  let sawBegin = false;
  for (let index = 0; index < tokens.length; index += 1) {
    const token = tokens[index];
    if (token.kind === 'semicolon') {
    ...
    if (token.value === 'BEGIN') {
      if (tokens[index - 1]?.kind === 'word' && tokens[index - 1]?.value === 'TRANSACTION')
        continue;
    ...
    if (token.value === 'END') {
      ...
      const next = nextWordToken(tokens, index);
      const target = ORACLE_PL_SQL_TERMINATORS.has(next ?? '')
        ? next
        : top === 'OBJECT_BODY' || top === 'OBJECT_SPEC'
          ? top
          : 'BLOCK';
...
function previousWordToken(
  tokens: Array<{ kind: 'word' | 'semicolon'; value: string }>,
  index: number,
): string | null {
...
function isCursorOnStatementLine(
  sql: string,
  pos: number,
  statement: Pick<RawStatement, 'from'>,
): boolean {
  const lineStart = sql.lastIndexOf('\n', pos - 1) + 1;
  let lineEnd = sql.indexOf('\n', pos);
  if (lineEnd === -1) lineEnd = sql.length;
...
function isCursorOnRangeEndLine(
  sql: string,
  pos: number,
  range: Pick<RawStatement, 'to'>,
): boolean {
  const lineStart = sql.lastIndexOf('\n', pos - 1) + 1;
  let lineEnd = sql.indexOf('\n', pos);
  if (lineEnd === -1) lineEnd = sql.length;
 ...
 */
export function executableStatementRanges(
  sql: string,
  databaseType?: DatabaseType,
  parameterOptions?: SqlParameterOptions,
): SqlTextRange[] {
  ...
  if (databaseType === 'mongodb')
    return splitMongoCommandRanges(sql).map(({ from, to, text }) => ({ from, to, sql: text }));
  return splitSqlStatementRanges(sql, databaseType, parameterOptions).flatMap((statement) =>
    splitStatementRangeAtSoftStarts(sql, statement, databaseType, parameterOptions).map((range) =>
      rangeFor(range, sql),
    ),
  );
}

export function currentExecutableStatementRange(
  sql: string,
  cursorPos: number,
  databaseType?: DatabaseType,
  parameterOptions?: SqlParameterOptions,
...
export function buildExecutionCandidates(
  ...
  databaseType?: DatabaseType,
  parameterOptions?: SqlParameterOptions,
): SqlExecutionCandidate[] {
  const full = fullSqlRange(sql);
  const cursorStatement = currentExecutableStatementRange(
    sql,
    cursorPos,
    databaseType,
    parameterOptions,
  ...
  return [
    candidateFromRange(cursorStatement, 'cursor', databaseType),
    candidateFromRange(full, 'all', databaseType),
  ];
}

function candidateFromRange(
  range: SqlTextRange,
  kind: SqlExecutionCandidate['kind'],
  databaseType?: DatabaseType,
  supportedKinds: SqlExecutionCandidate['supportedKinds'] = [kind],
): SqlExecutionCandidate {

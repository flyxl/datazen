...
export function buildQueryWithHiddenPrimaryKeys(options: {
  sql: string;
  databaseType: DatabaseType;
  primaryKeys: string[];
  existingResultNames: string[];
  sourceExpressions?: Record<string, string>;
}): HiddenPrimaryKeyQuery | undefined {
  ...
  const expressions = projections.map(
    ({ sourceName, alias }) =>
      `${options.sourceExpressions?.[sourceName] ?? dialect.quoteIdentifier(sourceName)} AS ${dialect.quoteIdentifier(alias)}`,
  );
  const sql = appendSelectProjections(options.sql, expressions, options.databaseType);
  return sql ? { sql, projections } : undefined;
}

export function hiddenResultColumnIndexes(
  columns: string[],
  projections: HiddenPrimaryKeyProjection[],
): number[] {
  return projections.flatMap((projection) => {
    const index = columns.findIndex(
      (column) => column.toLowerCase() === projection.alias.toLowerCase(),
    );
    return index < 0 ? [] : [index];
  });
}

function appendSelectProjections(
  sql: string,
  expressions: string[],
  databaseType: DatabaseType,
): string | undefined {
  ...
  const selectIndex = tokens.findIndex(
    (token) => token.kind === 'word' && token.depth === 0 && token.normalized === 'select',
  );
  ...
  const projectionTokens = tokens
    .slice(selectIndex + 1, fromIndex)
    .filter((token) => token.kind !== 'comment');
  const bareWildcard =
    projectionTokens.length === 1 && projectionTokens[0]?.text === '*'
      ? projectionTokens[0]
      : undefined;
  const sourceReference =
    bareWildcard && databaseType === 'oracle'
      ? singleOracleSourceReference(sql, bareWildcard.span.start)
      : undefined;
  ...
  const qualifiedProjectionEnd =
    projectionEnd + qualifiedWildcard.length - bareWildcard.text.length;

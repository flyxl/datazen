...
export function resolveSqlStatementWindow(
  sql: string,
  cursor: number,
...
export function resolveStatementWindowFromSyntaxTree(
  state: EditorState,
  cursor: number,
): TextRange | null {
  ...
  if (
    cursor < from ||
    (cursor > statementEnd && state.doc.sliceString(statementEnd, cursor).trim().length > 0)
  )
    return null;
...
export function resolveLexicalLeafFromSyntaxTree(
  state: EditorState,
  cursor: number,
): { inLineComment: boolean; inBlockComment: boolean; inStringLiteral: boolean } | null {
  if (!syntaxTreeAvailable(state, cursor)) return null;

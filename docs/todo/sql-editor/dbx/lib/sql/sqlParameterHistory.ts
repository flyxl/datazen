...
function readHistory(): StoredSqlParameterHistory {
  ...
  try {
    ...
    if (parsed.version !== 1 || !parsed.parameters || typeof parsed.parameters !== 'object')
      return emptyHistory();
...
export function loadSqlParameterHistory(name: string): SqlParameterInput[] {
  return (readHistory().parameters[normalizeParameterName(name)] ?? []).slice(
    0,
    MAX_SQL_PARAMETER_HISTORY,
  );
}

export function rememberSqlParameterValue(
  name: string,
  ...
  history.parameters[key] = [
    normalized,
    ...previous.filter(
      (entry) => entry.value !== normalized.value || entry.kind !== normalized.kind,
    ),
  ].slice(0, MAX_SQL_PARAMETER_HISTORY);
  writeHistory(history);
  return history.parameters[key];
}

export function rememberSqlParameterValues(
  values: Record<string, SqlParameterInput>,
): Record<string, SqlParameterInput[]> {

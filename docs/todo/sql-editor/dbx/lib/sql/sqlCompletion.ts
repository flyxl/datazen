...
import {
  isEditorStatePlausibleFor,
  resolveLexicalLeafFromSyntaxTree,
  resolveSqlStatementWindow,
} from '@/lib/sql/sqlSyntaxTreeWindow';
import {
  DEFAULT_SQL_SNIPPETS,
  MANTICORESEARCH_SQL_SNIPPETS,
  resolveSqlSnippetBodyForDatabase,
} from '@/lib/sql/sqlSnippetTemplates';
import {
  requiresMysqlIdentifierQuote,
  requiresPostgresIdentifierQuote,
} from '@/lib/sql/sqlIdentifier';
...
import {
  driverProfileCompletionObjects,
  driverProfileCompletionTableMetadata,
  driverProfileCompletionTables,
  driverProfileRoutineSignatures,
} from '@/lib/database/driverProfileExtensions';
...
const MANTICORESEARCH_SQL_KEYWORDS = [
  'FACET',
  'MATCH',
  'SHOW',
  'SHOW META',
...
const ORACLE_SYSTEM_VALUE_NAMES = [
  'SYSDATE',
  'SYSTIMESTAMP',
  'CURRENT_DATE',
  'CURRENT_TIMESTAMP',
...
const NON_ORACLE_COMPLETION_WORDS = new Set([
  'BIGSERIAL',
  'BOOLEAN',
  'ELSEIF',
  'LIMIT',
...
const ORACLE_SQL_KEYWORDS = Array.from(
  new Set([
    ...sqlDialectCompletionWords(PLSQL.spec.keywords).filter(
      (keyword) => !NON_ORACLE_COMPLETION_WORDS.has(keyword),
    ),
    ...ORACLE_SQL_TYPES,
    'BULK COLLECT',
    'CONNECT BY',
...
const TABLE_TRIGGER_KEYWORDS = new Set([
  'from',
  'join',
  'update',
  'into',
...
const JOIN_MODIFIER_KEYWORD_PHRASES = [
  'LEFT JOIN',
  'RIGHT JOIN',
  'INNER JOIN',
  'FULL JOIN',
...
function isTableTriggerKeyword(keyword: string, options: SqlSemanticBuildOptions): boolean {
  return (
    TABLE_TRIGGER_KEYWORDS.has(keyword) ||
    (keyword === 'desc' && resolveSqlDialectId(options) === 'mysql')
  );
}
...
const DUAL_ROLE_SQL_KEYWORDS = new Set([
  'LEFT',
  'RIGHT',
  'IF',
  'TRUNCATE',
...
const CLOUDFLARE_D1_FUNCTION_SIGNATURES = new Map(
  Array.from(SQLITE_FUNCTION_SIGNATURES.entries()).filter(([name]) => name !== 'NOW'),
);
...
const SQLSERVER_DATEPART_VALUES = [
  'year',
  'quarter',
  'month',
  'dayofyear',
...
const SQL_ALIAS_KEYWORD_WORDS = new Set(
  sqlAliasKeywordWords(
    SQL_KEYWORDS.join(' '),
    StandardSQL.spec.keywords,
    MySQL.spec.keywords,
    MariaSQL.spec.keywords,
    PostgreSQL.spec.keywords,
    MSSQL.spec.keywords,
    SQLite.spec.keywords,
    PLSQL.spec.keywords,
    Cassandra.spec.keywords,
  ),
);
...
export function shouldChainSqlCompletionAfterAccept(item: {
  type?: string;
  apply?: string;
}): boolean {
...
const MYSQL_LIKE_IDENTIFIER_DATABASES = new Set<DatabaseType>([
  'mysql',
  'clickhouse',
  'hive',
  'argo',
...
const POSTGRES_LIKE_IDENTIFIER_DATABASES = new Set<DatabaseType>([
  'postgres',
  'redshift',
  'gaussdb',
  'kingbase',
...
const ORACLE_COMPAT_IDENTIFIER_DATABASES = new Set<DatabaseType>([
  'oracle',
  'oceanbase-oracle',
  'yashandb',
  'oscar',
...
function sqlCompletionApplyDialect(
  databaseType: DatabaseType | undefined,
  fallback: 'mysql' | 'postgres' | 'sqlserver' | undefined,
): SqlCompletionApplyDialect | undefined {
...
export function prepareSqlCompletionReplacement(
  sql: string,
  cursor: number,
  context: Pick<SqlCompletionContext, 'prefix' | 'qualifier' | 'replacementRange'>,
  items: SqlCompletionItem[],
): { from: number; items: SqlCompletionItem[] } {
  const range = context.replacementRange;
  const from =
    range && range.start >= 0 && range.start <= cursor && range.end === cursor
      ? range.start
      : cursor - context.prefix.length;
  ...
  return {
    ...
    items: items.map((item) => {
      ...
      if (
        item.type === 'column' &&
        !(apply.startsWith(sql[from] ?? '') && apply.endsWith(closingQuote)) &&
        (context.qualifier || !apply.includes('.'))
      ) {
      ...
      return replaceClosingQuote && !prepared.replaceClosingQuote
        ? { ...prepared, replaceClosingQuote }
        : prepared;
    }),
  };
}
...
export function buildSqlCompletionItemsFromContext(
  ...
  input: SqlCompletionProviderInput,
): SqlCompletionItem[] {
  ...
  build(): SqlCompletionItem[] {
    ...
    if (this.databaseType === 'mongodb') {
      return dedupeAndSort(
        buildMongoCompletionItemsFromContext({ mode: 'root', prefix: context.prefix, from: 0 }).map(
          mongoCompletionItemToSqlCompletionItem,
        ),
      );
    }
    ...
    if (context.preferredValueKeywords?.length) {
      return dedupeAndSort(
        buildPreferredKeywordItems(
          context.prefix,
          context.preferredValueKeywords,
          this.input.keywordCase,
        ),
      );
    }

    const preferReferencedColumns = hasMatchingReferencedColumnPrefix(
      context,
      this.input.columnsByTable,
    );
    if (
      !pendingJoinKeyword &&
      ...
      const snippets =
        this.databaseType === 'manticoresearch'
          ? [...(this.input.snippets ?? DEFAULT_SQL_SNIPPETS), ...MANTICORESEARCH_SQL_SNIPPETS]
          : (this.input.snippets ?? DEFAULT_SQL_SNIPPETS);
      if (!preferReferencedColumns) {
        this.items.push(
          ...buildSnippetItems(context.prefix, snippets, this.input.keywordCase, this.databaseType),
        );
      }
      if (!preferReferencedColumns || context.suggestRoutines) {
        const functionItems = context.dataTypeContext
          ? []
          : buildFunctionSnippetItems(
              context.prefix,
              getFunctionDescriptions(this.t),
              this.databaseType,
              context.openingParenAfterCursor,
              this.input.keywordCase,
              this.input.functionCase,
            );
        this.items.push(
          ...(preferReferencedColumns
            ? functionItems.filter((item) =>
    ...
    if (this.databaseType === 'manticoresearch' && context.exclusiveRoutineSuggestions) {
      this.items.push(
        ...buildSnippetItems(
          context.prefix,
          MANTICORESEARCH_SQL_SNIPPETS.filter(
            (snippet) => snippet.id === 'builtin-manticore-call-pq',
          ),
          this.input.keywordCase,
          this.databaseType,
        ),
    ...
    if (
      ...
      if (context.isEmptyGroupBy && !context.prefix) {
        const allSelectAliasesItem = buildGroupByAllSelectAliasItem(
          context,
          selectAliasItems,
          this.input.columnsByTable,
          this.dialect,
    ...
    if (
      ...
      this.items.push(
        ...buildNonAggregatedColumnItems(context, this.input.columnsByTable, this.dialect),
      );
    }
    ...
    if (context.suggestKeywords && !context.exclusiveRoutineSuggestions && !pendingJoinKeyword) {
      ...
      this.items.push(
        ...buildKeywordItems(context.prefix, context, this.databaseType, this.input.keywordCase),
      );
    } else if (shouldOfferKeywordPrefixContinuations(context, pendingJoinKeyword)) {
      this.items.push(
        ...buildKeywordPrefixContinuationItems(
          context.prefix,
          context,
          this.databaseType,
          this.input.keywordCase,
    ...
    const emptyTableNameCompletion =
      !context.prefix && (context.suggestTables || context.exclusiveTableSuggestions);
    if (
      !pendingJoinKeyword &&
      !emptyTableNameCompletion &&
      !context.tableAliasAfterCursor &&
      context.referencedTables.length > 0 &&
      !context.suggestColumns &&
      !context.insertTable
    ) {
    ...
    if (!context.exclusiveColumnSuggestions && context.suggestTables) {
      this.items.push(
        ...buildForeignKeyRelatedTableItems(
          context,
          completionTables,
          this.input.foreignKeysByTable,
          this.dialect,
          !!this.input.autoAliasTables && context.autoAliasTableCompletions,
          this.databaseType,
          this.input.keywordCase,
          this.input.currentSchema,
        ),
      );
      this.items.push(
        ...buildTableItems(
      ...
      if (this.databaseType === 'clickhouse') {
        this.items.push(
          ...buildClickHouseFunctionItems(context.prefix, context.openingParenAfterCursor, 'table'),
        );
      }
      if (isOracleLikeDatabase(this.databaseType)) {
        this.items.push(
          ...buildOracleTableFunctionItems(
            context.prefix,
            this.input.keywordCase,
            this.input.functionCase,
          ),
    ...
    if (
      context.suggestRoutines ||
      context.exclusiveRoutineSuggestions ||
      context.oracleTableFunctionContext
    ) {
      const profileObjects = driverProfileCompletionObjects(this.input.driverProfile, context);
      this.items.push(
        ...buildObjectItems(
          context,
          [...(this.input.objects ?? []), ...profileObjects],
          this.dialect,
          this.databaseType,
          this.input.currentSchema,
        ),
    ...
    if (context.onStar) {
      const starItem = buildStarExpansionItem(
        context,
        this.input.columnsByTable,
        this.t,
        this.dialect,
    ...
    if (context.prefix) {
      for (const item of this.items) {
        ...
        const isAliasSnippet =
          item.type === 'snippet' &&
          item.apply ===
            formatAliasCompletionApply(item.label, this.databaseType, this.input.keywordCase);
        const isExactLabelMatch =
          !isAliasSnippet && item.label.toLowerCase() === context.prefix.toLowerCase();
        const isExactFilterTextMatch =
          item.filterText?.toLowerCase() === context.prefix.toLowerCase();
...
export function shouldAutoOpenSqlCompletion(
  sql: string,
  cursor: number,
  options: SqlSemanticBuildOptions = {},
): boolean {
  ...
  if (previousChar === '(' && (context.insertTable || context.preferredValueKeywords?.length))
    return true;
  ...
  if (
    context.exclusiveColumnSuggestions ||
    shouldAutoOpenColumnCompletion(context, sql, cursor, options.databaseType)
  )
    return true;
  return /[A-Za-z_$@.]/.test(previousChar);
}

function shouldAutoOpenColumnCompletion(
  context: SqlCompletionContext,
  sql: string,
  cursor: number,
  databaseType: DatabaseType | undefined,
...
function isColumnCompletionExpressionStart(
  beforeCursor: string,
  databaseType: DatabaseType | undefined,
): boolean {
...
export function isSqlCompletionSuppressedContext(
  sql: string,
  cursor: number,
  options: SqlSemanticBuildOptions = {},
): boolean {
 ...
 */
export function getPostgresSequenceLiteralCompletionContext(
  sql: string,
  cursor: number,
  databaseType?: DatabaseType,
): PostgresSequenceLiteralCompletionContext | null {
...
export function buildPostgresSequenceLiteralCompletionItems(
  context: PostgresSequenceLiteralCompletionContext,
  objects: SqlCompletionObject[],
): SqlCompletionItem[] {
  return objects
    ...
    .filter((object) => {
      if (context.schema) {
        ...
        const schemaMatches = context.schemaQuoted
          ? object.schema === context.schema
          : object.schema.toLowerCase() === context.schema.toLowerCase();
        if (!schemaMatches) return false;
      }
      return context.nameQuoted
        ? object.name.startsWith(context.prefix)
        : object.name.toLowerCase().startsWith(context.prefix.toLowerCase());
    })
    .map((object) => {
      const identifier = context.nameQuoted
        ? `${escapePostgresSequenceLiteralQuotedIdentifier(object.name)}"`
        : escapePostgresSequenceLiteralIdentifier(quoteSqlIdentifier(object.name, 'postgres'));
      return {
        label: object.name,
        filterText: context.nameQuoted
          ? identifier.slice(0, -1)
          : escapePostgresSequenceLiteralIdentifier(object.name),
        type: 'variable' as const,
        detail: object.schema ? `sequence in ${object.schema}` : 'sequence',
        info: object.comment?.trim() || undefined,
        apply: identifier,
        replaceClosingQuote:
          context.nameQuoted && !context.nameQuoteClosed ? ('"' as const) : undefined,
...
function isPostgresSequenceFunctionCall(beforeLiteral: string): boolean {
  ...
  if (
    quotedQualifier != null
      ? qualifierName !== 'pg_catalog'
      : qualifierName.toLowerCase() !== 'pg_catalog'
  )
    return false;
...
function parsePostgresRegclassPrefix(raw: string): {
  nameStart: number;
  name: string;
  schema?: string;
  schemaQuoted: boolean;
  ...
  while (index <= raw.length && parts.length < 2) {
    ...
    index++;
    if (index >= raw.length)
      parts.push({ start: index, value: '', quoted: false, quoteClosed: false });
  }
...
export function isSqlStringLiteralContext(
  sql: string,
  cursor: number,
  options: SqlSemanticBuildOptions = {},
): boolean {
...
export function isSqlCommentContext(
  sql: string,
  cursor: number,
  options: SqlSemanticBuildOptions = {},
): boolean {
...
function getSqlLexicalContext(
  sql: string,
  cursor: number,
  options: SqlSemanticBuildOptions,
): { inLineComment: boolean; inBlockComment: boolean; inStringLiteral: boolean } {
...
export function isSqlLikeCompletionStatement(
  sql: string,
  cursor: number,
  options: SqlSemanticBuildOptions = {},
): boolean {
...
function activeSqlCompletionStatementSpan(
  sql: string,
  cursor: number,
  options: SqlSemanticBuildOptions,
): SqlSemanticSpan {
  ...
  const firstStatementToken = tokens.find(
    (token) =>
      token.kind !== 'comment' &&
      token.span.end > statementSpan.start &&
      token.span.start < statementSpan.end,
  );
  const result = firstStatementToken
    ? { start: firstStatementToken.span.start, end: statementSpan.end }
    : statementSpan;
  return { start: result.start + window.from, end: result.end + window.from };
}

function currentSqlLikeLineBlockSpan(
  sql: string,
  cursor: number,
  activeStatementSpan: SqlSemanticSpan,
): SqlSemanticSpan | null {
  const safeCursor = Math.max(0, Math.min(cursor, sql.length));
  ...
  return {
    start,
    end: blockEnd == null ? activeStatementSpan.end : Math.min(activeStatementSpan.end, blockEnd),
  };
}
...
function currentLineBlockEnd(sql: string, cursor: number, start: number): number | null {
  ...
  while (lineStart < sql.length) {
    ...
    if (
      lineStart > start &&
      (!trimmed || /^(get|post|put|delete|patch|head)\s+\//i.test(trimmed))
    ) {
      return lineStart;
    }
    if (lineEnd < 0) break;
...
export function getSqlFunctionSignatureHelp(
  ...
  if (databaseType !== 'clickhouse') {
    ...
    const parameters =
      activeFunctionSignatures(databaseType).get(lookupName) ??
      driverProfileRoutineSignatures(driverProfile).get(lookupName);
  ...
  const parameterGroups = searchClickHouseFunctions(call.name, 50)
    .find((definition) =>
      [definition.name, ...(definition.aliases ?? [])].some(
        (name) => name.toLowerCase() === call.name.toLowerCase(),
      ),
    )
  ...
  const overloads = parameterGroups
    ...
    .sort((left, right) => {
      const leftAccepts = functionParameterGroupAccepts(
        left.groups[call.activeGroup],
        observedParameter,
      );
      const rightAccepts = functionParameterGroupAccepts(
        right.groups[call.activeGroup],
        observedParameter,
      );
...
function sqlCompletionStatementSpan(
  sql: string,
  cursor: number,
  options: SqlSemanticBuildOptions,
): SqlSemanticSpan {
...
export function getSqlCompletionContext(
  ...
  const cursorInStatement =
    cursor - statementSpan.start - (rawStatement.length - rawStatement.trimStart().length);
  ...
  let referencedTables = extractReferencedTables(
    maskResolvedCteBodies(fullStatement, cursorInStatement, cteDefs),
  ...
  for (const cte of cteDefs) {
    if (!referencedTables.some((rt) => rt.name.toLowerCase() === cte.name.toLowerCase())) {
      ...
      const existing = referencedTables.find(
        (rt) => rt.name.toLowerCase() === cte.name.toLowerCase(),
      );
  ...
  for (const sq of subqueryRefs) {
    if (
      !referencedTables.some(
        (rt) => rt.name.toLowerCase() === sq.name.toLowerCase() && rt.alias === sq.alias,
      )
    ) {
  ...
  const oracleTableFunctionContext = detectOracleTableFunctionContext(
    beforeCursor,
    options.databaseType,
  );
  ...
  const afterTableTrigger =
    isTableTriggerKeyword(lastWord, options) ||
  ...
  const autoAliasTableCompletions =
    (lastWord === 'from' ||
      lastWord === 'join' ||
      (JOIN_MODIFIERS.has(lastWord) && isFollowedByJoin(beforeToken)) ||
      inTableListContext) &&
  ...
  const activePrefixIsCte = cteDefs.some(
    (cte) => normalizeIdentifierPart(cte.name) === normalizeIdentifierPart(prefix),
  );
  ...
  const inColumnContext =
    selectListColumnContext || isInColumnContext(beforeCursor) || !!insertInfo;
  ...
  const inPotentialPackageMemberContext =
    !!qualifier &&
    !exclusiveTableSuggestions &&
    !insertInfo &&
    !updateInfo?.inSetClause &&
    !oracleTableFunctionContext;
  const suggestColumns =
    !!qualifier ||
    !!updateInfo?.inSetClause ||
    !!insertInfo ||
    (inColumnContext && referencedTables.length > 0);
  const suggestRoutines =
    inCallRoutineContext ||
    oracleTableFunctionContext ||
    inPotentialPackageMemberContext ||
    (!exclusiveTableSuggestions &&
  ...
  const preferredValueKeywords = sqlServerDatepartCompletionValues(
    beforeCursor,
    options.databaseType,
  );
  const preferredKeywords = qualifier
    ? []
    : preferredKeywordsForCompletion(
        beforeCursor,
        beforeToken,
  ...
  return {
    ...
    suggestKeywords:
      ...
      !inCallRoutineContext,
    suggestRoutines,
    suggestJoinConditions: insertInfo
      ? false
      : inJoinConditionContext && referencedTables.length >= 2,
    exclusiveTableSuggestions: insertInfo ? false : exclusiveTableSuggestions,
    exclusiveColumnSuggestions:
      exclusiveColumnSuggestions || !!insertInfo || !!updateInfo?.inSetClause,
...
function sqlServerDatepartCompletionValues(
  beforeCursor: string,
  databaseType?: DatabaseType,
): string[] | undefined {
  if (databaseType !== 'sqlserver') return undefined;
  const call = findActiveFunctionCall(beforeCursor);
  if (!call || call.activeGroup !== 0 || !SQLSERVER_DATEPART_FUNCTIONS.has(call.name.toUpperCase()))
    return undefined;
  return countTopLevelCommas(call.groupText) === 0 ? SQLSERVER_DATEPART_VALUES : undefined;
}

function isCreateTableColumnTypeContext(
  beforeToken: string,
  databaseType: DatabaseType | undefined,
): boolean {
...
function removeActiveTableCompletionReference(
  referencedTables: SqlCompletionReferencedTable[],
  prefix: string,
  qualifier?: string,
  ...
  return referencedTables.filter((table) => {
    ...
    if (
      activeQualifier &&
      table.schema &&
      normalizeIdentifierPart(table.schema) !== activeQualifier
    )
      return true;
    return false;
  });
...
function parseTrailingIdentifierContext(
  input: string,
  databaseType?: DatabaseType,
): { start: number; prefix: string; qualifier?: string; qualifierParts?: string[] } | null {
  ...
  while (index > 0) {
    ...
    if (!parsed) {
      const omittedSqlServerSchema =
        databaseType === 'sqlserver' &&
        tail[index - 1] === '.' &&
        parseTrailingIdentifierPart(tail, index - 1);
...
function parseTrailingIdentifierPart(
  input: string,
  endExclusive: number,
  ...
  if (
    ...
    !SQL_IDENTIFIER_START_CHAR.test(String.fromCodePoint(firstCodePoint))
  )
    return null;
...
function detectComparisonLeftColumn(beforeCursor: string): string | undefined {
  ...
  const match = /\b([A-Za-z_][\w$]*(?:\.[A-Za-z_][\w$]*)?)\s*(?:=|!=|<>|>=|<=|>|<)\s*$/i.exec(
    beforeCursor,
  );
  return match?.[1];
}

function detectInsertColumnListContext(
  beforeCursor: string,
): { table: string; database?: string; schema?: string } | null {
  ...
  const match = new RegExp(`\\binsert\\s+into\\s+(${qualifiedIdentifier})\\s*\\([^)]*$`, 'i').exec(
    cleaned,
  );
  if (!match) return null;
...
function detectUpdateCompletionContext(beforeCursor: string): {
  target: { table: string; schema?: string };
  afterTarget: boolean;
  inSetClause: boolean;
  afterSetAssignments: boolean;
} | null {
  const cleaned = beforeCursor.replace(/'[^']*'/g, "''").replace(/"[^"]*"/g, '""');
  const match =
    /^\s*update\s+((?:"[^"]+"|`[^`]+`|[A-Za-z_][\w$]*)(?:\.(?:"[^"]+"|`[^`]+`|[A-Za-z_][\w$]*))?)(?:\s+(?:as\s+)?((?!set\b|where\b)[A-Za-z_][\w$]*))?/i.exec(
      cleaned,
    );
  if (!match) return null;
  ...
  const afterSetAssignments =
    inSetClause && /(?:=|,)\s*(?:''|""|[A-Za-z0-9_.$]+)?\s+[A-Za-z_][\w$]*$/i.test(setSegment);
  return { target, afterTarget: false, inSetClause, afterSetAssignments };
}

function detectDeleteCompletionContext(
  beforeCursor: string,
): { target?: { table: string; schema?: string }; afterTarget: boolean } | null {
  const cleaned = beforeCursor.replace(/'[^']*'/g, "''").replace(/"[^"]*"/g, '""');
  const match =
    /^\s*delete(?:\s+[A-Za-z_][\w$]*)?\s+from\s+((?:"[^"]+"|`[^`]+`|[A-Za-z_][\w$]*)(?:\.(?:"[^"]+"|`[^`]+`|[A-Za-z_][\w$]*))?)(?:\s+(?:as\s+)?([A-Za-z_][\w$]*))?/i.exec(
      cleaned,
    );
  if (!match)
    return /^\s*delete\s+(?:from\s+)?[A-Za-z_][\w$]*$/i.test(cleaned)
      ? { afterTarget: false }
      : null;
  const [first, second] = splitQualifiedName(match[1] ?? '');
...
function detectOracleTableFunctionContext(
  beforeCursor: string,
  databaseType: DatabaseType | undefined,
): boolean {
  const cleaned = maskSqlLiteralsAndComments(beforeCursor, databaseType);
  return /\b(?:from|join)\s+table\s*\(\s*(?:(?:"[^"]+"|`[^`]+`|[A-Za-z_][\w$]*)\.){0,2}[A-Za-z_][\w$]*$/i.test(
    cleaned,
  );
}
...
function preferredKeywordsForCompletion(
  ...
  if (selectListColumnContext && hasSelectListExpression(beforeCursor, databaseType))
    keywords.push('FROM');
  if (isAfterJoinModifierContext(beforeCursor, databaseType)) keywords.push('JOIN');
  if (!exclusiveTableSuggestions && isAfterSelectBodyExpression(beforeToken, databaseType))
    keywords.push('LIMIT');
...
function hasSelectListExpression(
  beforeCursor: string,
  databaseType: DatabaseType | undefined,
): boolean {
  const cleaned = maskSqlLiteralsAndComments(beforeCursor, databaseType).trimEnd();
...
function isAfterSelectBodyExpression(
  beforeToken: string,
  databaseType: DatabaseType | undefined,
): boolean {
...
const SELECT_BODY_INCOMPLETE_TAIL_KEYWORDS = new Set([
  'where',
  'and',
  'or',
  'not',
...
const CONDITION_INCOMPLETE_TAIL_KEYWORDS = new Set([
  'where',
  'and',
  'or',
  'not',
...
function isAfterConditionExpression(
  beforeToken: string,
  databaseType: DatabaseType | undefined,
): boolean {
  const cleaned = maskSqlLiteralsAndComments(beforeToken, databaseType).trimEnd();
...
function isAfterJoinModifierContext(
  beforeCursor: string,
  databaseType: DatabaseType | undefined,
): boolean {
  const cleaned = maskSqlLiteralsAndComments(beforeCursor, databaseType).trimEnd();
  ...
  const lastTableIntro = Math.max(
    lastTopLevelKeywordIndex(beforeModifier, 'from'),
    lastTopLevelKeywordIndex(beforeModifier, 'join'),
  );
...
function isExpressionTailComplete(sql: string): boolean {
  ...
  return /(?:\)|\]|\b(?:true|false|null)\b|`[^`]+`|"[^"]*"|''|\b\d+(?:\.\d+)?\b|\b[A-Za-z_][\w$]*\b)$/i.test(
    trimmed,
  );
}
...
function lastTopLevelKeywordIndex(sql: string, keyword: string): number {
  ...
  for (let index = 0; index < lower.length; index++) {
    ...
    if (
      depth === 0 &&
      lower.startsWith(target, index) &&
      !isIdentifierPart(lower[index - 1]) &&
      !isIdentifierPart(lower[index + target.length])
    ) {
      lastIndex = index;
...
const ASCII_ONLY_UNQUOTED_IDENTIFIER_DATABASES = new Set<DatabaseType>([
  'clickhouse',
  'snowflake',
  'bigquery',
  'hive',
...
const BACKSLASH_ESCAPE_STRING_DIALECTS = new Set<DatabaseType>([
  'mysql',
  'doris',
  'starrocks',
  'hive',
...
function buildTableRefPattern(
  unquotedIdentifier: string,
  aliasIdentifier: string,
  allowSqlServerDoubleDot: boolean,
): RegExp {
  const identifier = `(?:${TABLE_REF_QUOTED_IDENTIFIER}|${unquotedIdentifier})`;
  const qualifiedSeparator = allowSqlServerDoubleDot
    ? `\\.(?:${identifier}|\\.${identifier})`
    : `\\.${identifier}`;
  return new RegExp(
    `\\b(?:${TABLE_REF_INTRODUCER_KEYWORDS.join('|')})\\s+(${identifier}(?:${qualifiedSeparator}){0,3})(?:\\s+(?:as\\s+)?(${aliasIdentifier}))?`,
    'giu',
  );
}
...
const TABLE_REF_PATTERN_ASCII = buildTableRefPattern(
  TABLE_REF_UNQUOTED_IDENTIFIER_ASCII,
  TABLE_REF_ALIAS_ASCII,
  false,
);
const TABLE_REF_PATTERN_UNICODE_DEFAULT = buildTableRefPattern(
  TABLE_REF_UNQUOTED_IDENTIFIER_UNICODE,
  TABLE_REF_ALIAS_UNICODE,
  false,
);
const TABLE_REF_PATTERN_UNICODE_SQLSERVER = buildTableRefPattern(
  TABLE_REF_UNQUOTED_IDENTIFIER_UNICODE,
  TABLE_REF_ALIAS_UNICODE,
  true,
);
...
function maskSqlLiteralsAndComments(sql: string, databaseType?: DatabaseType): string {
  ...
  for (const t of tokens) {
    ...
    const isTableRefSegment =
      tableRefState === 'expectSegment' && (t.kind === 'quoted_identifier' || t.kind === 'word');
    const maskAsValue =
      t.kind === 'string' ||
      (isDoubleQuotedIdentifier && (afterValueOperator || (mysqlFamily && !isTableRefSegment)));
    if (t.kind === 'comment') {
      out += ' ';
    } else if (maskAsValue) {
...
function extractReferencedTables(
  ...
  const pattern =
    !databaseType || ASCII_ONLY_UNQUOTED_IDENTIFIER_DATABASES.has(databaseType)
      ? TABLE_REF_PATTERN_ASCII
      : databaseType === 'sqlserver'
        ? TABLE_REF_PATTERN_UNICODE_SQLSERVER
        : TABLE_REF_PATTERN_UNICODE_DEFAULT;
  ...
  while ((match = pattern.exec(scanSql)) !== null) {
    ...
    const quotedName =
      !!rawName && (rawName.startsWith('"') || rawName.startsWith('`') || rawName.startsWith('['));
    ...
    const omittedSqlServerSchema =
      databaseType === 'sqlserver' && rawParts.length >= 3 && rawParts[rawParts.length - 2] === '';
    ...
    const table: SqlCompletionReferencedTable = {
      ...
      database: omittedSqlServerSchema
        ? unquotedRawParts[unquotedRawParts.length - 3] || undefined
        : parts.length >= 3
          ? parts[parts.length - 3]
          : undefined,
      schema: omittedSqlServerSchema
        ? SQLSERVER_DEFAULT_SCHEMA
        : parts.length >= 2
          ? parts[parts.length - 2]
          : undefined,
      schemaQuoted: omittedSqlServerSchema
        ? undefined
        : parts.length >= 2
          ? isQuotedIdentifier(rawParts[rawParts.length - 2])
          : undefined,
      alias: cleanAlias,
    };
...
function extractSelectAlias(expression: string): string | null {
  ...
  if (!expressionWithoutAlias || /^[A-Za-z_][\w$]*(?:\.[A-Za-z_][\w$]*)?$/.test(trimmed))
    return null;
 ...
 */
function maskResolvedCteBodies(
  statement: string,
  cursorOffset: number,
  ctes: readonly ScannedCteDefinition[],
): string {
...
export function quoteSqlIdentifier(
  ...
  if (dialect === 'oracle') {
    if (
      ...
      !POSTGRES_IDENTIFIER_KEYWORDS.has(identifier.toLowerCase())
    )
      return identifier;
  ...
  if (
    dialect !== 'postgres' ||
    !requiresPostgresIdentifierQuote(identifier, POSTGRES_IDENTIFIER_KEYWORDS)
  )
    return identifier;
  return `"${identifier.replaceAll('"', '""')}"`;
}
...
function quoteCompletionApplyIdentifier(
  identifier: string,
  dialect?: SqlCompletionApplyDialect,
): string {
  if (dialect === 'mysql') {
...
function quoteCompletionApplyName(applyName: string, dialect?: SqlCompletionApplyDialect): string {
  ...
  return parts
    .map((part) =>
      isQuotedIdentifier(part) ? part : quoteCompletionApplyIdentifier(part, dialect),
    )
    .join('.');
}

function quoteCompletionRoutineIdentifier(
  identifier: string,
  dialect?: SqlCompletionApplyDialect,
): string {
  if (
...
function quoteCompletionRoutineName(
  applyName: string,
  dialect?: SqlCompletionApplyDialect,
): string {
  ...
  return parts
    .map((part) =>
      isQuotedIdentifier(part) ? part : quoteCompletionRoutineIdentifier(part, dialect),
    )
    .join('.');
}

function quoteSelectStarColumnIdentifier(
  identifier: string,
  dialect?: SqlCompletionApplyDialect,
  databaseType?: DatabaseType,
): string {
...
function resolveTableSchemaQualification(
  ...
  const oracleSchemaQualification =
    databaseType === 'oracle' &&
    table.schema &&
    table.schema.toUpperCase() !== 'PUBLIC' &&
    (!currentSchema ||
      normalizeIdentifierPart(table.schema) !== normalizeIdentifierPart(currentSchema));
  ...
  const ambiguousTableName =
    databaseType !== 'oracle' &&
    (schemasByTableName.get(normalizeIdentifierPart(table.name))?.size ?? 0) > 1;
  const schemaQualification = !!table.schema && (oracleSchemaQualification || ambiguousTableName);
  const defaultApplyName = schemaQualification
    ? `${quoteCompletionApplyIdentifier(table.schema!, dialect)}.${quoteCompletionApplyIdentifier(table.name, dialect)}`
    : quoteCompletionApplyIdentifier(table.name, dialect);
  ...
  return matchingTables
    .map((table) => {
      const qualifiedByContext =
        !!qualifierSchema &&
        !!table.schema &&
        normalizeIdentifierPart(qualifierSchema) === normalizeIdentifierPart(table.schema);
      const { ambiguousTableName, defaultApplyName } = resolveTableSchemaQualification(
        table,
        dialect,
        databaseType,
        currentSchema,
      ...
      const applyName = qualifiedByContext
        ? quoteCompletionApplyIdentifier(table.name, dialect)
        : ambiguousTableName &&
            !!table.schema &&
            (!suppliedApplyName || !suppliedApplyNameIsQualified)
          ? defaultApplyName
          : (suppliedApplyName ?? defaultApplyName);
      const alias = autoAliasTables
        ? generateTableCompletionAlias(table.name, existingAliases)
        : '';
      const schemaDetail =
        ambiguousTableName && table.schema ? `${table.schema}.${table.name}` : undefined;
      const detail =
        table.detail && schemaDetail
          ? `${schemaDetail}  ${table.detail}`
          : (table.detail ?? schemaDetail ?? (table.type === 'table' ? undefined : table.type));
      return {
        label: table.name,
        type: 'table' as const,
        detail,
        ...
        dedupeKey:
          table.applyName || ambiguousTableName || (databaseType === 'oracle' && table.schema)
            ? applyName
            : undefined,
      };
    })
...
function buildForeignKeyRelatedTableItems(
      ...
      for (const foreignKey of foreignKeys) {
        if (referencedTableMatchesName(ref, owner.name, owner.schema)) {
          ...
          if (target && matchesPrefix(target.name, context.prefix)) {
            candidates.set(`${target.schema ?? ''}.${target.name}`.toLowerCase(), {
              table: target,
              detail: `related by ${foreignKey.column} → ${qualifiedCompletionName(foreignKey.ref_table, foreignKey.ref_schema)}.${foreignKey.ref_column}`,
            });
          }
          ...
          if (target && matchesPrefix(target.name, context.prefix)) {
            candidates.set(`${target.schema ?? ''}.${target.name}`.toLowerCase(), {
              table: target,
              detail: `related by ${qualifiedCompletionName(owner.name, owner.schema)}.${foreignKey.column} → ${foreignKey.ref_column}`,
            });
          }
        }
      }
  ...
  const schemasByTableName = collectSchemasByTableName(
    tables.filter((table) => matchesPrefix(table.name, context.prefix)),
  );
  ...
  return [...candidates.values()]
    .map(({ table, detail }) => {
      ...
      return {
        ...
        dedupeKey:
          ambiguousTableName || (databaseType === 'oracle' && table.schema) ? applyName : undefined,
      };
    })
    .sort(compareCompletionItems);
}
...
function findCompletionTable(
  ...
  name: string,
  schema?: string | null,
): SqlCompletionTable | undefined {
  const normalizedName = normalizeIdentifierPart(name);
  const normalizedSchema = schema ? normalizeIdentifierPart(schema) : undefined;
  return tables.find(
    (table) =>
      normalizeIdentifierPart(table.name) === normalizedName &&
      (!normalizedSchema ||
        !table.schema ||
        normalizeIdentifierPart(table.schema) === normalizedSchema),
  );
}
...
function buildObjectItems(
  ...
  const onlyFunctions =
    context.suggestColumns && context.referencedTables.length > 0 && !context.qualifier;
  const prioritizeOracleFunctions = databaseType === 'oracle' && context.statementKind === 'select';
  return objects
    .filter(
      (object) =>
        object.type !== 'sequence' &&
        (!onlyProcedures || object.type === 'procedure') &&
        (!onlyFunctions ||
          (object.type === 'function' &&
            object.name.toLowerCase().startsWith(context.prefix.toLowerCase()))) &&
        objectMatchesCompletionContext(object, context),
    )
    .map((object) => {
      const qualifiedByContext = objectIsQualifiedByContext(object, context);
      const objectInCurrentSchema =
        !!currentSchema &&
        !!object.schema &&
        normalizeIdentifierPart(object.schema) === normalizeIdentifierPart(currentSchema);
      const suppliedApplyName = object.applyName
        ? quoteCompletionRoutineName(object.applyName, dialect)
        : undefined;
      const applyName =
        qualifiedByContext ||
        (context.qualifier && object.schema?.toLowerCase() === context.qualifier.toLowerCase())
          ...
          : (suppliedApplyName ??
            (object.schema && !objectInCurrentSchema
              ? `${quoteCompletionRoutineIdentifier(object.schema, dialect)}.${quoteCompletionRoutineIdentifier(object.name, dialect)}`
              : quoteCompletionRoutineIdentifier(object.name, dialect)));
      const locationDetail =
        object.type === 'trigger' && object.parentName
          ? `trigger on ${object.parentName}`
          : object.parentName
            ? `${object.type} in ${object.parentName}`
            : object.schema
      ...
      const detail = [
        ...
        object.dataType ? `[${object.dataType}]` : undefined,
      ]
        .filter(Boolean)
        .join('  ');
      ...
      const baseDedupeKey =
        object.applyName || (databaseType === 'oracle' && object.schema) ? applyName : undefined;
      return {
        label: object.name,
        ...
        apply:
          object.type === 'trigger' || object.type === 'package'
            ? applyName
            : buildRoutineApply(applyName, object.signature),
...
function buildRoutineInfo(object: SqlCompletionObject): string | undefined {
  const qualifiedName = object.parentName
    ? [object.parentSchema ?? object.schema, object.parentName, object.name]
        .filter(Boolean)
        .join('.')
    : [object.schema, object.name].filter(Boolean).join('.');
  const parts = [
...
function completionQualifierIsReferencedTable(context: SqlCompletionContext): boolean {
  ...
  return context.referencedTables.some((table) =>
    referencedTableMatchesColumnQualifier(table, qualifier, qualifierLower, qualifiedTarget),
  );
}

function objectIsQualifiedByContext(
  ...
  const qualifierSchema =
    qualifierParts.length > 1 ? qualifierParts[qualifierParts.length - 2] : undefined;
  const qualifierPackage = qualifierParts[qualifierParts.length - 1];
  return (
    object.parentName.toLowerCase() === qualifier ||
    (!!qualifierPackage &&
      object.parentName.toLowerCase() === qualifierPackage &&
      (!qualifierSchema ||
        !object.parentSchema ||
        object.parentSchema.toLowerCase() === qualifierSchema))
  );
}

function objectMatchesCompletionContext(
  object: SqlCompletionObject,
  context: SqlCompletionContext,
): boolean {
  ...
  if (context.qualifier) {
    ...
    const qualifierSchema =
      qualifierParts.length > 1 ? qualifierParts[qualifierParts.length - 2] : undefined;
    const qualifierPackage = qualifierParts[qualifierParts.length - 1];
    if (object.parentName && object.parentName.toLowerCase() === qualifier)
    ...
    if (
      ...
      return matchesPrefix(object.name, context.prefix);
    if (object.schema && object.schema.toLowerCase() === qualifier)
      return matchesPrefix(object.name, context.prefix);
    if (
      object.parentSchema &&
      `${object.parentSchema}.${object.parentName ?? ''}`.toLowerCase() === qualifier
    )
      return matchesPrefix(object.name, context.prefix);
  }
  return matchesPrefix(object.name, context.prefix);
...
function buildOracleTableFunctionItems(
  prefix: string,
  keywordCase?: SqlKeywordCase,
  functionCase?: SqlKeywordCase,
): SqlCompletionItem[] {
  const items = [
    {
      label: applySqlKeywordCase('TABLE', keywordCase),
      detail: 'Oracle table function',
      apply: `${applySqlKeywordCase('TABLE', keywordCase)}(\${function_call})`,
    },
    {
      label: applySqlKeywordCase('THE', keywordCase),
      detail: 'Oracle nested-table expression',
      apply: `${applySqlKeywordCase('THE', keywordCase)}(\${subquery})`,
    },
    {
      label: applySqlFunctionCase('XMLTABLE', functionCase),
      detail: 'XML to relational rows',
      apply: `${applySqlFunctionCase('XMLTABLE', functionCase)}(\${xpath})`,
    },
    {
      label: applySqlFunctionCase('JSON_TABLE', functionCase),
      detail: 'JSON to relational rows',
      apply: `${applySqlFunctionCase('JSON_TABLE', functionCase)}(\${expr}, \${path})`,
    },
  ];
  return items
...
function applyGeneratedSqlTemplateKeywordCase(value: string, keywordCase?: SqlKeywordCase): string {
  return value
    ...
    .map((part) =>
      part.startsWith('${')
        ? part
        : part.replace(GENERATED_SQL_TEMPLATE_KEYWORDS_RE, (keyword) =>
            applySqlKeywordCase(keyword, keywordCase),
          ),
    )
    .join('');
}
...
function applyBuiltinSnippetKeywordCase(
  snippet: SqlSnippet,
  text: string,
  keywordCase?: SqlKeywordCase,
): string {
...
const BUILTIN_SNIPPET_PLACEHOLDER_RE =
  /\b(idx_name|left_column|right_column|columns|values|condition|column|default|value|name|type|table)\b/g;
...
function buildPreferredKeywordItems(
  prefix: string,
  keywords: string[],
  keywordCase?: SqlKeywordCase,
): SqlCompletionItem[] {
...
function selectStarExpansionColumns(
  context: SqlCompletionContext,
  columnsByTable: Map<string, SqlCompletionColumn[]>,
): SqlCompletionColumn[] {
  ...
  if (context.qualifier || references.length > 1) {
    return references.flatMap((reference) =>
      uniqueColumnsByName(columnsForSelectAllReferencedTable(reference, columnsByTable)),
    );
  }
  return uniqueColumnsByName([...columnsByTable.values()].flat());
}

export function selectStarResultColumnsMatch(options: {
  currentSql: string;
  targetFrom: number;
  targetTo: number;
  statementSql: string;
  ...
  return (
    options.targetFrom >= options.sourceFrom! &&
    options.targetTo <= options.sourceTo! &&
    sourceToAtBoundary &&
    options.currentSql.slice(options.sourceFrom, options.sourceTo) === options.sourceStatement
  );
}

export function buildSelectStarExpansion(
  context: SqlCompletionContext,
  columnsByTable: Map<string, SqlCompletionColumn[]>,
  dialect?: SqlCompletionApplyDialect,
  qualifierSql = context.qualifier,
  databaseType?: DatabaseType,
): string | null {
  ...
  if (qualifierSql)
    return buildSelectAllColumnExpansion(columns, qualifierSql, true, dialect, databaseType);

  const references = referencedTablesForSelectAllColumns(context);
  if (references.length <= 1)
    return columns
      .map((column) => quoteSelectStarColumnIdentifier(column.name, dialect, databaseType))
      .join(', ');
  ...
  return references
    .flatMap((reference) => {
      const qualifier =
        reference.aliasSql ??
        (reference.alias
          ? quoteCompletionApplyIdentifier(reference.alias, dialect)
          : quoteCompletionApplyIdentifier(reference.name, dialect));
      return uniqueColumnsByName(columnsForSelectAllReferencedTable(reference, columnsByTable)).map(
        (column) =>
          `${qualifier}.${quoteSelectStarColumnIdentifier(column.name, dialect, databaseType)}`,
      );
    })
    .join(', ');
}
...
function buildStarExpansionItem(
  ...
  const expansion = buildSelectStarExpansion(
    context,
    columnsByTable,
    dialect,
    context.qualifier,
...
function buildSelectAllColumnItems(
  context: SqlCompletionContext,
  columnsByTable: Map<string, SqlCompletionColumn[]>,
  t?: SqlCompletionTranslations,
  dialect?: SqlCompletionApplyDialect,
  ...
  for (const ref of targetRefs) {
    ...
    const qualifier =
      context.qualifier ||
      ref.alias ||
      (shouldQualify ? quoteCompletionApplyIdentifier(ref.name, dialect) : undefined);
    const expansion = buildSelectAllColumnExpansion(
      columns,
      qualifier,
      !!context.qualifier,
      dialect,
    ...
    const countText = (t?.starExpansionColumns ?? '{count} columns').replace(
      '{count}',
    ...
    items.push({
      ...
      apply: expansion,
      boost:
        2400 + selectAllColumnItemPrefixBoost(label, ref, columns, context.prefix) - items.length,
    });
  }
...
function buildInsertAllColumnItems(
  context: SqlCompletionContext,
  columnsByTable: Map<string, SqlCompletionColumn[]>,
  t?: SqlCompletionTranslations,
  dialect?: SqlCompletionApplyDialect,
  ...
  if (
    !selectAllColumnItemMatchesPrefix(
      label,
      { name: context.insertTable, schema: context.insertSchema },
      columns,
      context.prefix,
    )
  )
  ...
  const columnList = columns
    .map((column) => quoteCompletionApplyIdentifier(column.name, dialect))
    .join(', ');
...
function referencedTablesForSelectAllColumns(
  context: SqlCompletionContext,
): SqlCompletionReferencedTable[] {
  ...
  const qualifierLower = qualifier.toLowerCase();
  const qualifiedTarget = qualifiedTableTargetFromContext(context);
  return context.referencedTables.filter((table) =>
    referencedTableMatchesColumnQualifier(table, qualifier, qualifierLower, qualifiedTarget),
  );
}

function buildSelectAllColumnExpansion(
  columns: SqlCompletionColumn[],
  qualifier: string | undefined,
  qualifierAlreadyTyped: boolean,
  dialect?: SqlCompletionApplyDialect,
  databaseType?: DatabaseType,
): string {
  return columns
...
function columnsForSelectAllReferencedTable(
...
function selectAllColumnItemMatchesPrefix(
  label: string,
  ref: SqlCompletionReferencedTable,
  columns: SqlCompletionColumn[],
  prefix: string,
): boolean {
  ...
  if (
    ...
    matchesPrefix(ref.name, prefix) ||
    (!!ref.alias && matchesPrefix(ref.alias, prefix))
  )
    return true;
  return columns.some((column) => matchesPrefix(column.name, prefix));
}

function selectAllColumnItemPrefixBoost(
  label: string,
  ref: SqlCompletionReferencedTable,
  columns: SqlCompletionColumn[],
  prefix: string,
  ...
  if (!prefix) return 0;
  const scores = [
    computeBoost(label, prefix),
    computeBoost(ref.name, prefix),
    ref.alias ? computeBoost(ref.alias, prefix) : -1,
    ...columns.map((column) => computeBoost(column.name, prefix)),
  ];
...
function buildComparisonValueItems(
  ...
  if (qualifier) {
    const ref = context.referencedTables.find(
  ...
  if (dt === 'bit' || dt === 'boolean' || dt === 'bool') {
    items.push(
      {
        label: applySqlKeywordCase('TRUE', keywordCase),
        type: 'keyword' as const,
        detail: t?.booleanValue ?? 'Boolean value',
        boost: 1700,
      },
      {
        label: applySqlKeywordCase('FALSE', keywordCase),
        type: 'keyword' as const,
        detail: t?.booleanValue ?? 'Boolean value',
        boost: 1650,
      },
...
function buildReferencedAliasItems(
  context: SqlCompletionContext,
  t?: SqlCompletionTranslations,
): SqlCompletionItem[] {
  const seen = new Set<string>();
...
function formatTableAliasApply(
  tableName: string,
  alias: string,
  databaseType?: DatabaseType,
  keywordCase?: SqlKeywordCase,
  ...
  return isOracleLikeDatabase(databaseType)
    ? `${tableName} ${alias}`
    : `${tableName} ${applySqlKeywordCase('AS', keywordCase)} ${alias}`;
}

function formatAliasCompletionApply(
  alias: string,
  databaseType?: DatabaseType,
  keywordCase?: SqlKeywordCase,
): string {
  return isOracleLikeDatabase(databaseType)
    ? `${alias} `
    : `${applySqlKeywordCase('AS', keywordCase)} ${alias} `;
}
...
function buildAliasCandidates(tableName: string): string[] {
  ...
  if (parts.length > 1) {
    ...
    if (chars.length >= 3 && consonants.length >= 2)
      candidates.push(`${chars[0]}${consonants[0]}${consonants[1]}`);
...
function isInTableListContext(
  beforeToken: string,
  databaseType: DatabaseType | undefined,
): boolean {
  if (isInOrderOrGroupByContext(beforeToken)) return false;
  const cleaned = activeQueryBlockSql(
    maskSqlLiteralsAndComments(beforeToken, databaseType).trimEnd(),
  );
  if (!/,\s*$/.test(cleaned)) return false;

  // Only commas in the active top-level table segment should continue table completion.
  const lastTableIntro = Math.max(
    lastTopLevelKeywordIndex(cleaned, 'from'),
    lastTopLevelKeywordIndex(cleaned, 'join'),
    lastTopLevelKeywordIndex(cleaned, 'update'),
    lastTopLevelKeywordIndex(cleaned, 'into'),
  );
  if (lastTableIntro < 0) return false;
...
function activeQueryBlockSql(sql: string): string {
  ...
  for (let index = 0; index < lower.length; index += 1) {
    ...
    if (
      lower.startsWith('select', index) &&
      !isIdentifierPart(lower[index - 1]) &&
      !isIdentifierPart(lower[index + 6])
    ) {
  ...
  const activeDepth = Math.max(
    ...[...selectIndexes.keys()].filter((selectDepth) => selectDepth <= depth),
    -1,
  );
...
const completionColumnSearchIndexes = new WeakMap<
  readonly SqlCompletionColumn[],
  CompletionColumnSearchIndex
>();

function completionColumnSearchIndex(
  ...
  const index: CompletionColumnSearchIndex = {
    entries: columns
      ...
      .sort(
        (left, right) =>
          left.normalizedName.localeCompare(right.normalizedName) || left.index - right.index,
      ),
  };
...
function completionColumnPrefixCandidates(
  columns: readonly SqlCompletionColumn[],
  prefix: string,
  limit = 256,
): SqlCompletionColumn[] {
  ...
  if (candidates.length < limit) {
    for (
      let indexPosition = 0;
      indexPosition < columns.length && candidates.length < limit;
      indexPosition += 1
    ) {
...
function collectCompletionColumns(
  columnsByTable: Map<string, SqlCompletionColumn[]>,
  prefix = '',
): Array<SqlCompletionColumn & { key: string }> {
...
function columnsForInsertTarget(
  context: SqlCompletionContext,
  columnsByTable: Map<string, SqlCompletionColumn[]>,
): Array<SqlCompletionColumn & { key: string }> {
  if (!context.insertTable) return [];
  const tableKey = normalizeIdentifierPart(context.insertTable);
  const schemaKey = context.insertSchema
    ? normalizeIdentifierPart(context.insertSchema)
    : undefined;
  const databaseKey = context.insertDatabase
    ? normalizeIdentifierPart(context.insertDatabase)
    : undefined;
  const qualifiedKey = schemaKey
    ? normalizeCompletionKey(
        `${context.insertDatabase ? `${context.insertDatabase}.` : ''}${context.insertSchema}.${context.insertTable}`,
      )
    : undefined;
  return collectCompletionColumns(columnsByTable).filter((column) => {
    ...
    if (!databaseKey && column.schema && normalizeIdentifierPart(column.schema) === schemaKey)
      return true;
...
function buildColumnItems(
  ...
  if (context.insertTable) {
    ...
    const qualifiedTarget = qualifiedTableTargetFromContext(context);
    const relatedTables = context.referencedTables.filter((table) =>
      referencedTableMatchesColumnQualifier(table, q, qLower, qualifiedTarget),
    );
    relevantCols = relatedTables.flatMap((table) =>
      completionColumnsForReferencedTable(table, allColumns),
    );
    if (relatedTables.length === 0 && qualifiedTarget)
      relevantCols = allColumns.filter((column) =>
        columnMatchesQualifiedTable(column, qualifiedTarget),
      );
  } else if (context.referencedTables.length > 0) {
    relevantCols = context.referencedTables.flatMap((table) =>
      completionColumnsForReferencedTable(table, allColumns),
    );
  }
  ...
  const qualifyAllColumns =
    !context.qualifier && !context.insertTable && context.referencedTables.length > 1;
  const seen = new Set<string>();
  ...
  const rankedColumns = uniqueColumns
    ...
    .filter(
      ({ column }) =>
        matchesIdentifierSearch(column.name, context.prefix) ||
        matchesIdentifierSearch(column.displayLabel, context.prefix),
    )
    .map(({ column, index }) => {
      const keyBoost = isKeyColumn(column.name) ? 500 : 0;
      const matchScore = Math.max(
        identifierMatchScore(column.name, context.prefix),
        identifierMatchScore(column.displayLabel, context.prefix),
      );
  ...
  const batchSelectionMode = context.insertTable
    ? 'insert'
    : context.statementKind === 'select' && context.selectListColumnContext
      ? 'select'
      : undefined;
  ...
  return rankedColumns.map(({ column, boost }) => {
    return {
      ...
      batchSelectionQualifier:
        batchSelectionMode === 'select' && context.qualifier
          ? (context.qualifierParts ?? context.qualifier.split('.').filter(Boolean))
              .map((part) => quoteCompletionApplyIdentifier(part, dialect))
              .join('.')
          : undefined,
    };
  });
...
function completionColumnsForReferencedTable<T extends SqlCompletionColumn & { key: string }>(
  table: SqlCompletionReferencedTable,
  columns: readonly T[],
): T[] {
  ...
  return aliasedColumns.map((column) => ({
    ...column,
    sourceAlias: table.alias,
    sourceQualifierSql: table.aliasSql,
  }));
}

function applyReferencedColumnAliases<T extends SqlCompletionColumn>(
  table: SqlCompletionReferencedTable,
  columns: readonly T[],
): T[] {
...
function hasMatchingReferencedColumnPrefix(
  context: SqlCompletionContext,
  columnsByTable: Map<string, SqlCompletionColumn[]>,
): boolean {
  if (!context.suggestColumns || !context.prefix || context.referencedTables.length === 0)
    return false;
  return context.referencedTables.some(
    (table) =>
      completionColumnPrefixCandidates(
        columnsForReferencedTable(table, columnsByTable),
        context.prefix,
        1,
      ).length > 0,
  );
...
function qualifiedTableTargetFromContext(
  context: SqlCompletionContext,
...
function referencedTableMatchesColumnQualifier(
  table: SqlCompletionReferencedTable,
  qualifier: string,
  qualifierLower: string,
  qualifiedTarget: { database?: string; schema: string; table: string } | null,
): boolean {
  ...
  if (normalizeIdentifierPart(table.name) !== normalizeIdentifierPart(qualifiedTarget.table))
    return false;
  if (
    table.database &&
    qualifiedTarget.database &&
    normalizeIdentifierPart(table.database) !== normalizeIdentifierPart(qualifiedTarget.database)
  )
    return false;
  return (
    !table.schema ||
    normalizeIdentifierPart(table.schema) === normalizeIdentifierPart(qualifiedTarget.schema)
  );
}

function columnMatchesReferencedTable(
  column: SqlCompletionColumn & { key: string },
  table: SqlCompletionReferencedTable,
): boolean {
  if (normalizeIdentifierPart(column.table) !== normalizeIdentifierPart(table.name)) return false;
  if (!table.schema) return true;
  return columnMatchesQualifiedTable(column, {
...
function columnMatchesQualifiedTable(
  column: SqlCompletionColumn & { key: string },
  ...
  if (
    !target.database &&
    column.schema &&
    normalizeIdentifierPart(column.schema) === normalizeIdentifierPart(target.schema)
  )
    return true;
...
function buildColumnApply(
  ...
  if (
    ...
    !column.displayLabel.includes('.')
  ) {
    return quoteCompletionApplyIdentifier(column.name, dialect);
  }
  const qualifier =
    column.sourceQualifierSql ??
    quoteCompletionApplyIdentifier(column.sourceAlias ?? column.table, dialect);
...
function buildColumnInfo(column: SqlCompletionColumn): string | undefined {
  const hasDetails =
    !!column.dataType || column.isNullable !== undefined || !!column.comment?.trim();
  if (!hasDetails) return undefined;
  const title = column.schema
...
function buildJoinConditionItems(
  ...
  for (const previous of previousRefs) {
    ...
    items.push(
      ...buildForeignKeyJoinConditionItemsForPair(
        previous,
        latest,
        foreignKeysByTable,
        context.prefix,
      ...
      ...buildJoinConditionItemsForPair(
        previous,
        previousColumns,
        latest,
        latestColumns,
...
function columnsForReferencedTable(
  ...
  const keys = table.schema
    ? [
        table.database ? `${table.database}.${table.schema}.${table.name}` : undefined,
...
function buildDirectionalForeignKeyJoinConditionItems(
  ...
  for (const group of groups) {
    ...
    items.push({
      ...
      detail:
        group.length > 1
          ? 'JOIN condition from composite foreign key'
          : 'JOIN condition from foreign key',
      apply,
...
function buildJoinConditionPart(
  ...
  const ownerApplyRef = owner.alias
    ? owner.alias
    : quoteCompletionApplyIdentifier(owner.name, dialect);
  const referencedApplyRef = referenced.alias
    ? referenced.alias
    : quoteCompletionApplyIdentifier(referenced.name, dialect);
  return {
...
function groupForeignKeysByConstraint(
  foreignKeys: SqlCompletionForeignKey[],
): SqlCompletionForeignKey[][] {
...
function buildJoinConditionItemsForPair(
  ...
  const rightApplyRef = right.alias
    ? right.alias
    : quoteCompletionApplyIdentifier(right.name, dialect);
  ...
  const addPair = (
    ...
    if (!leftColumn || !rightColumn || !areJoinColumnTypesCompatible(leftColumn, rightColumn))
      return;
  ...
  items.push(
    ...buildCompositeHeuristicJoinConditionItems(
      left,
      leftColumns,
      right,
      leftByName,
...
function indexColumnsByLowerName(
  columns: SqlCompletionColumn[],
): Map<string, SqlCompletionColumn[]> {
...
function buildCompositeHeuristicJoinConditionItems(
  ...
  const candidates: Array<{
    parent: 'left' | 'right';
    parentId: SqlCompletionColumn;
    childFk: SqlCompletionColumn;
  }> = [];
  ...
  const sharedScopeColumns = leftColumns
    .map((leftColumn) => {
      ...
      if (
        !rightColumn ||
        !isLikelyScopeColumnName(name) ||
        !areJoinColumnTypesCompatible(leftColumn, rightColumn)
      ) {
        return null;
      }
    ...
    .filter(
      (value): value is { leftColumn: SqlCompletionColumn; rightColumn: SqlCompletionColumn } =>
        !!value,
    )
    .slice(0, 2);
...
function areJoinColumnTypesCompatible(
  left: SqlCompletionColumn,
  right: SqlCompletionColumn,
): boolean {
...
function normalizeJoinType(dataType?: string): string | null {
  ...
  if (
    /\b(bigint|int8|integer|int|int4|smallint|int2|tinyint|serial|bigserial|number|numeric|decimal)\b/.test(
      type,
    )
  ) {
    return 'number';
...
export function buildSnippetItemsForTest(
  prefix: string,
  snippets: SqlSnippet[],
  keywordCase?: SqlKeywordCase,
  databaseType?: DatabaseType,
...
function buildSnippetItems(
  ...
  return snippets
    .filter((snippet) => {
      ...
      const matchesSnippetLabel =
        prefix.length > snippet.prefix.length && matchesPrefix(snippet.label, prefix);
    ...
    .map((snippet) => {
      ...
      const apply = applyBuiltinSnippetKeywordCase(
        snippet,
        applyBuiltinSnippetPlaceholders(snippet, resolvedBody),
        keywordCase,
      );
      return {
        label: snippet.label,
...
function activeFunctionSignatures(databaseType?: DatabaseType): Map<string, string[]> {
  const commonFunctionNames =
    databaseType === 'cloudflare-d1'
      ? CLOUDFLARE_D1_COMMON_FUNCTION_NAMES
      : COMMON_SQL_FUNCTION_NAMES;
  const signatures = databaseType
    ? new Map(
        Array.from(SQL_FUNCTION_SIGNATURES.entries()).filter(([name]) =>
          commonFunctionNames.has(name),
        ),
      )
...
function formatFunctionSignatureApply(
  definition: ClickHouseFunctionDefinition,
  omitOpeningParen: boolean,
): string {
...
function clickHouseFunctionDetail(definition: ClickHouseFunctionDefinition): string {
  const status =
    definition.status && definition.status !== 'stable' ? ` · ${definition.status}` : '';
  const overloads =
    definition.signatures.length > 1 ? ` · ${definition.signatures.length} overloads` : '';
  return `ClickHouse · ${definition.category}${overloads}${status}`;
}

function buildClickHouseFunctionItems(
  prefix: string,
  omitOpeningParen: boolean,
  ...
  return searchClickHouseFunctions(prefix, 200, kind).map((definition) => {
    const statusPenalty =
      definition.status === 'deprecated' ? -600 : definition.status === 'experimental' ? -300 : 0;
...
function buildFunctionSnippetItems(
  ...
  for (const [name, parameters] of activeFunctionSignatures(databaseType).entries()) {
    ...
    const paramStr =
      parameters.length > 0
        ? parameters
            .map((p) => `\${${applyGeneratedSqlTemplateKeywordCase(p, keywordCase)}}`)
            .join(', ')
        : '';
    const mysqlApply =
      databaseType === 'mysql' ? MYSQL_FUNCTION_APPLY_TEMPLATES.get(name) : undefined;
    items.push({
      label: functionName,
      type: 'function' as const,
      detail: functionDescriptions.get(name) ?? 'function',
      apply: mysqlApply
        ? `${functionName}${applyGeneratedSqlTemplateKeywordCase(mysqlApply.slice(name.length), keywordCase)}`
  ...
  for (const name of WINDOW_FUNCTIONS) {
    ...
    items.push({
      ...
      apply: applyGeneratedSqlTemplateKeywordCase(
        `${functionName}() OVER (PARTITION BY \${col} ORDER BY \${col})`,
        keywordCase,
      ),
...
function buildOracleSystemValueItems(
  prefix: string,
  keywordCase?: SqlKeywordCase,
): SqlCompletionItem[] {
...
function buildGroupByAllSelectAliasItem(
  context: SqlCompletionContext,
  selectAliasItems: SqlCompletionItem[],
  columnsByTable: Map<string, SqlCompletionColumn[]>,
  dialect?: SqlCompletionApplyDialect,
): SqlCompletionItem | null {
  const nonAggregatedAliases = new Set(
    context.nonAggregatedSelectColumns.map((column) => column.toLowerCase()),
  );
  const columnApplyByName = new Map(
    buildNonAggregatedColumnItems(context, columnsByTable, dialect).map((item) => [
  ...
  return {
    ...
    apply: safeAliasItems
      .map((item) => columnApplyByName.get(item.label.toLowerCase()) ?? item.apply ?? item.label)
      .join(', '),
    boost: 3650,
...
function activeSqlKeywords(databaseType?: DatabaseType): string[] {
  ...
  const keywords = databaseType
    ? Array.from(new Set([...COMMON_SQL_KEYWORDS, ...(databaseKeywords ?? [])]))
    : Array.from(new Set(SQL_KEYWORDS));
  return isOracleLikeDatabase(databaseType)
    ? keywords.filter((keyword) => !NON_ORACLE_COMPLETION_WORDS.has(keyword))
    : keywords;
}
...
function buildJoinModifierKeywordItems(
  prefix: string,
  keywordCase?: SqlKeywordCase,
): SqlCompletionItem[] {
  if (!prefix) return [];
  return JOIN_MODIFIER_KEYWORD_PHRASES.filter((keyword) => matchesPrefix(keyword, prefix)).map(
    (keyword) => {
...
function isPendingJoinKeywordContext(context: SqlCompletionContext): boolean {
  return (
    !context.prefix &&
    context.preferredKeywords.includes('JOIN') &&
    !!context.tableTriggerWord &&
    JOIN_MODIFIERS.has(context.tableTriggerWord)
  );
}
...
function buildKeywordItems(
  ...
  return activeSqlKeywords(databaseType)
    .filter((keyword) => {
      if (
        functionSignatures.has(keyword) &&
        !DATA_TYPE_KEYWORDS.has(keyword) &&
        !DUAL_ROLE_SQL_KEYWORDS.has(keyword)
      )
        return false;
      if (WINDOW_FUNCTIONS.has(keyword)) return false;
      if (!matchesPrefix(keyword, prefix)) return false;
      if (!showDdl && isDml && (DDL_ONLY_KEYWORDS.has(keyword) || DATA_TYPE_KEYWORDS.has(keyword)))
        return false;
      return true;
    })
    .map((keyword) => {
...
function shouldOfferKeywordPrefixContinuations(
  context: SqlCompletionContext,
  pendingJoinKeyword: boolean,
): boolean {
  return (
    !!context.prefix &&
    !pendingJoinKeyword &&
...
function buildKeywordPrefixContinuationItems(
  ...
  return buildKeywordItems(prefix, context, databaseType, keywordCase).filter((item) => {
    ...
    return (
      normalizedLabel.length > normalizedPrefix.length &&
...
function computeMatchScore(candidate: string, prefix: string): number {
  ...
  if (substringIndex >= 0) {
    const boundaryBonus = isIdentifierBoundary(candidate, substringIndex)
      ? 400
      : Math.max(0, 180 - substringIndex * 12);
...
function compareCompletionItems(left: SqlCompletionItem, right: SqlCompletionItem): number {
  ...
  return (
    right.boost +
    rightBonus +
    getTypePriorityBoost(right.type) -
    (left.boost + leftBonus + getTypePriorityBoost(left.type))
  );
}
...
function findActiveFunctionCall(sqlBeforeCursor: string): ActiveFunctionCall | null {
  ...
  const firstGroupOpenParen = findMatchingOpenParen(
    beforeActiveGroup,
    beforeActiveGroup.length - 1,
  );
  if (firstGroupOpenParen == null) return null;
  const parametricName = /([A-Za-z_][\w$]*)$/.exec(
    beforeActiveGroup.slice(0, firstGroupOpenParen).trimEnd(),
  )?.[1];

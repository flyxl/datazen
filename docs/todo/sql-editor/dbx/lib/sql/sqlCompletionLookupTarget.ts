...
export function sqlServerUseDatabaseFromStatement(statement: string): string | undefined {
  const match =
    /^USE\s+(?:\[((?:[^\]]|\]\])*)\]|"((?:[^"]|"")*)"|([\p{L}_@#][\p{L}\p{N}_@$#]*))\s*;?\s*$/iu.exec(
      sqlStatementWithoutLeadingComments(statement),
    );
...
export function replaceSqlServerLeadingUseQuery(
  sql: string,
  script: SqlServerLeadingUseScript,
  querySql: string,
): string {
...
function unclosedQuotedIdentifierPrefix(
  value: string,
  quoteStyle: 'bracket' | 'double',
): string | undefined {
...
export function resolveSqlServerUseDatabaseCompletion(options: {
  sql: string;
  cursor: number;
  databaseType?: DatabaseType;
}): SqlServerUseDatabaseCompletion | undefined {
  ...
  if (!statement || (statement.to > position && options.sql.slice(position, statement.to).trim()))
    return undefined;
  ...
  while (targetOffset < beforeCursor.length && /\s/u.test(beforeCursor[targetOffset]!))
    targetOffset += 1;
...
export function buildSqlServerUseDatabaseCompletionItems(
  databaseNames: readonly string[],
  completion: SqlServerUseDatabaseCompletion,
): SqlCompletionItem[] {
  return databaseNames.map((database) => {
    const escapedDatabase =
      completion.quoteStyle === 'double'
        ? database.replaceAll('"', '""')
        : database.replaceAll(']', ']]');
    const apply =
      completion.quoteStyle === 'bracket'
        ? `${escapedDatabase}]`
        : completion.quoteStyle === 'double'
          ? `${escapedDatabase}"`
          : `[${escapedDatabase}]`;
...
export function sqlServerUseCompletionDatabaseNames(options: {
  databaseNames: readonly string[];
  ...
  return currentDatabase
    ? [findExactName(options.databaseNames, currentDatabase) ?? currentDatabase]
    : [];
}
...
export function resolveSqlCompletionScope(options: {
  ...
  const database = parsedDatabase
    ? findExactName(options.knownDatabases, parsedDatabase)
    : undefined;
  ...
  if (
    !database ||
    !schema ||
    (!targetsCurrentDatabase && options.supportsSessionDatabaseSwitch !== true)
  ) {
    return {
      database: options.currentDatabase,
      schema: options.currentSchema,
  ...
  return {
    ...
    completionContext: {
      ...
      insertDatabase:
        options.completionContext.insertTable && !options.completionContext.insertDatabase
          ? database
          : options.completionContext.insertDatabase,
      insertSchema:
        options.completionContext.insertTable && !options.completionContext.insertSchema
          ? schema
          : options.completionContext.insertSchema,
...
function findCaseSensitiveName(
...
export function mergeSqlCompletionQualifierNames(
...
export function resolveSqlCompletionSchemaLookupDatabase(options: {
  ...
  completionContext: Pick<
    SqlCompletionContext,
    'qualifier' | 'qualifierParts' | 'suggestTables' | 'insertTable'
  >;
  knownDatabases?: readonly string[];
  ...
  if (
    !options.supportsDatabaseSchemaQualifier ||
  ...
  const qualifierParts =
    completionContext.qualifierParts?.filter(Boolean) ??
    qualifier?.split('.').filter(Boolean) ??
    [];
...
export function resolveSqlCompletionTableLookupTarget(options: {
  ...
  completionContext: Pick<
    SqlCompletionContext,
    'qualifier' | 'qualifierParts' | 'prefix' | 'suggestTables' | 'insertTable'
  >;
  knownDatabases?: readonly string[];
  ...
  const qualifierIsDatabase =
    options.supportsDatabaseQualifier &&
    !!qualifier &&
    completionContext.suggestTables &&
    !completionContext.insertTable;
  ...
  return {
    ...
    filter:
      qualifier && completionContext.suggestTables
        ? completionContext.prefix
        : qualifier || completionContext.prefix,
  };
}

export function resolveSqlCompletionRoutineLookupTarget(options: {
  currentDatabase: string;
  currentSchema?: string;
  supportsDatabaseSchemaQualifier?: boolean;
  completionContext: Pick<SqlCompletionContext, 'qualifier' | 'qualifierParts' | 'prefix'>;
}): SqlCompletionRoutineLookupTarget {
  const qualifier = options.completionContext.qualifier?.trim();
  const qualifierParts =
    options.completionContext.qualifierParts?.filter(Boolean) ??
    qualifier?.split('.').filter(Boolean) ??
    [];
  const hasDatabaseQualifier =
    options.supportsDatabaseSchemaQualifier && qualifierParts.length >= 2;
  const database = hasDatabaseQualifier
    ? qualifierParts[qualifierParts.length - 2]!

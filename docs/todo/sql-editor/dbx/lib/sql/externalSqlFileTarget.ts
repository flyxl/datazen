  ...
  try {
    ...
    return parsed
      ...
      .map((item) => ({
        ...
        schema: item.schema,
      })) as StoredExternalSqlFileTarget[];
  } catch {
    return [];
  }
}
...
export function rememberExternalSqlFileTarget(path: string, target: ExternalSqlFileTarget) {
  ...
  saveExternalSqlFileTargets(
    [{ path: normalizedPath, ...target, updatedAt: Date.now() }, ...remaining].slice(
      0,
...
export function moveExternalSqlFileTarget(previousPath: string, nextPath: string) {
  ...
  saveExternalSqlFileTargets(
    [{ ...target, path: next, updatedAt: Date.now() }, ...remaining].slice(
      0,
      MAX_EXTERNAL_SQL_FILE_TARGETS,
    ),
  );
...
export function forgetExternalSqlFileTarget(path: string) {
  ...
  if (!normalizedPath) return;
  saveExternalSqlFileTargets(
    loadExternalSqlFileTargets().filter((item) => item.path !== normalizedPath),
  );
}

export function resolveExternalSqlFileTarget(
  path: string,
  connectionExists: (connectionId: string) => boolean,
  fallback: ExternalSqlFileTarget,
): ExternalSqlFileTarget {
  const normalizedPath = normalizeExternalSqlPath(path);

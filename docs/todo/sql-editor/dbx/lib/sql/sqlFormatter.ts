...
const CLICKHOUSE_IDENTIFIER_LIKE_DATE_PARTS = new Set([
  'D',
  'DD',
  'H',
  'HH',
...
function resolveClickHouseIdentifierSafeDialect(
  sqlFormatter: SqlFormatterModule,
): SqlFormatterModule['clickhouse'] {
  ...
  clickHouseIdentifierSafeDialect = {
    ...
    tokenizerOptions: {
      ...
      reservedKeywords: sqlFormatter.clickhouse.tokenizerOptions.reservedKeywords.filter(
        (keyword) => !CLICKHOUSE_IDENTIFIER_LIKE_DATE_PARTS.has(keyword),
      ),
    },
  };
...
function protectDuckDbPrefixAliasSeparators(sql: string): { sql: string; marker: string | null } {
  ...
  while (index < sql.length) {
    ...
    const prefixAlias =
      ...
      next !== ':' &&
      next !== '=' &&
      (quotedIdentifierEnd === index || hasDuckDbBarePrefixAliasBefore(sql, index));
    if (prefixAlias) {
...
export async function formatSqlText(
  ...
  settings: Partial<SqlFormatterSettings> = DEFAULT_SQL_FORMATTER_SETTINGS,
): Promise<string> {
  ...
  const protectedInput =
    dialect === 'duckdb'
      ? protectDuckDbPrefixAliasSeparators(sqlWithProtectedEmptyLines)
      : { sql: sqlWithProtectedEmptyLines, marker: null };
    ...
    try {
      if (dialect === 'clickhouse') {
        return formatDialect(input, {
          ...formatterOptions,
          dialect: resolveClickHouseIdentifierSafeDialect(sqlFormatter),
        });
      }
  ...
  const finalizeFormattedSql = (formatted: string): string => {
    const restoredDuckDbAliases = restoreDuckDbPrefixAliasSeparators(
      formatted,
      protectedInput.marker,
    );
    const laidOut = applySqlFormatterLayout(restoredDuckDbAliases, normalizedSettings, dialect);
    return emptyLineProtection
      ? restoreProtectedEmptyLines(laidOut, emptyLineProtection.markers)
      : laidOut;
  };
  ...
  try {
    ...
    const trailingDot =
      dialect === 'dameng' ? splitTrailingStandaloneDot(protectedInput.sql) : null;
 ...
 */
function maskStringAndCommentSpans(
  sql: string,
  dialect: SqlFormatDialect,
): { masked: string; spans: string[] } {
  ...
  const supportsNestedBlockComments =
    dialect === 'postgres' || dialect === 'sqlserver' || dialect === 'clickhouse';
  const isMysqlDashComment = (c: string | undefined) =>
    c === undefined || c.charCodeAt(0) <= 32 || c.charCodeAt(0) === 127;
  ...
  while (i < len) {
    ...
    // 行注释 -- ... / # ...（不跨行，但遮罩可防御未来变更）
    const startsDashComment =
      ch === '-' && next === '-' && (dialect !== 'mysql' || isMysqlDashComment(sql[i + 2]));
...
function keepLogicalOperatorsOnSameLine(
  sql: string,
  dialect: SqlFormatDialect = 'generic',
): string {
  ...
  const collapsed = masked
    .replace(/\n[ \t]*(AND|OR|XOR)\b/gi, ' $1')
    .replace(/\b(AND|OR|XOR)[ \t]*\n[ \t]*/gi, '$1 ');
  return restoreSpans(collapsed, spans);
}

function normalizeLikeOperatorCase(
  sql: string,
  settings: SqlFormatterSettings,
  dialect: SqlFormatDialect,
): string {
  if ((dialect !== 'mysql' && dialect !== 'sqlite') || settings.keywordCase === 'preserve')
    return sql;
  ...
  const normalized = masked.replace(
    /(^|[^.\w$:@{#])LIKE\b(?!\s*\()/gi,
    (_match, prefix: string) => `${prefix}${keyword}`,
  );
...
function keepFromClauseAndFirstSourceOnSameLine(sql: string): string {
  ...
  for (let index = 0; index < lines.length - 1; index += 1) {
    ...
    const separator = sourceIndent.startsWith(clauseIndent)
      ? sourceIndent.slice(clauseIndent.length)
      : ' ';
...
function applySqlFormatterLayout(
  sql: string,
  settings: SqlFormatterSettings,
  dialect: SqlFormatDialect,
): string {
  let formatted = normalizeLikeOperatorCase(sql, settings, dialect);
  if (settings.logicalOperatorNewline === 'none')
    formatted = keepLogicalOperatorsOnSameLine(formatted, dialect);
  if (settings.fromClauseLayout === 'sameLine')
    formatted = keepFromClauseAndFirstSourceOnSameLine(formatted);
  return formatted;
}
...
export async function formatSqlForEditing(
  sql: string,
  dialect: SqlFormatDialect = 'generic',
  settings: Partial<SqlFormatterSettings> = DEFAULT_SQL_FORMATTER_SETTINGS,
): Promise<string> {
 ...
 */
export async function formatSqlForDisplay(
  sql: string,
  dialect: SqlFormatDialect = 'generic',
  settings: Partial<SqlFormatterSettings> = DEFAULT_SQL_FORMATTER_SETTINGS,
): Promise<string> {

...
export function isConnectionTimeoutErrorMessage(
  message: string,
  ...
  if (/\b(?:pool\s+checkout|checkout|metadata|loading|health check|cancel request)\b/.test(lower))
    return false;
  if (
    ...
    return true;
  return /\b(?:timed out|timeout expired|timeout exceeded|time-out)\b[\s\S]{0,80}\b(?:connection attempt|connect|handshake)\b/.test(
    lower,
  );
}
...
export function isQueryTimeoutErrorMessage(message: string, backendError?: BackendError): boolean {
  ...
  if (
    ...
    lower.includes('查询超时') ||
    lower.includes('查詢逾時') ||
    lower.includes('请求执行超时') ||
    lower.includes('請求執行逾時')
  )
    return true;
  ...
  if (
    /\b(?:canceling|cancelling|canceled|cancelled)\b[\s\S]{0,80}\bstatement\b[\s\S]{0,80}\btimeout\b/.test(
      lower,
    )
  )
    return true;

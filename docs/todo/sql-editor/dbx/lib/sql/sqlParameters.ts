...
const SQL_SERVER_TEMP_TABLE_CONTEXT_KEYWORDS = new Set([
  'table',
  'from',
  'join',
  'into',
...
export function readSqlBracedParameterAt(
  sql: string,
  start: number,
  options?: SqlParameterOptions,
): SqlBracedParameter | null {
  const open = sql.slice(start, start + 2);
  const syntax: SqlParameterSyntax | null =
    open === '${' ? 'shell' : open === '#{' ? 'mybatis' : null;
  if (!syntax || (options?.enabledSyntaxes && !options.enabledSyntaxes.includes(syntax)))
    return null;
  ...
  return {
    ...
    name,
    syntax,
    token: sql.slice(start, closeBrace + 1),
    start,
    end: closeBrace + 1,
  };
}
...
export function extractSqlParameterDescriptors(
  sql: string,
  options?: SqlParameterOptions,
  ...
  for (const occurrence of findSqlParameterOccurrences(sql, options)) {
    if (occurrence.replacement === 'mybatis-where' && occurrence.where) {
      for (const nested of extractSqlParameterDescriptors(occurrence.where.body, options))
        appendDescriptor(nested);
      continue;
    }
    appendDescriptor({
      key: occurrence.key,
    ...
    if (occurrence.replacement === 'mybatis-foreach' && occurrence.foreach) {
      for (const nested of extractSqlParameterDescriptors(occurrence.foreach.body, options)) {
        if (nested.key !== occurrence.foreach.item && nested.key !== occurrence.foreach.index)
          appendDescriptor(nested);
      }
    }
  }
...
export function substituteSqlParameters(
  sql: string,
  values: Record<string, SqlParameterInput>,
  options?: SqlParameterOptions,
): string {
  ...
  for (const occurrence of occurrences) {
    ...
    result +=
      occurrence.replacement === 'string-fragment'
        ? sqlParameterStringFragment(input)
        : occurrence.replacement === 'quoted-string'
          ? sqlParameterQuotedString(input)
          : sqlParameterLiteral(input);
    cursor = occurrence.end;
  }
  result += sql.slice(cursor);
  return decodeMyBatisEntities
    ? decodeMyBatisXmlComparisonEntities(result, options?.databaseType)
    : result;
}

const MYBATIS_WHERE_LEADING_CONJUNCTION_RE = /^(?:AND|OR)\s+/i;

function renderMyBatisWhere(
  where: MyBatisWhere,
  values: Record<string, SqlParameterInput>,
  options?: SqlParameterOptions,
): string {
...
function renderMyBatisForeach(
  foreach: MyBatisForeach,
  input: SqlParameterInput,
  values: Record<string, SqlParameterInput>,
  options?: SqlParameterOptions,
): string {
  ...
  const renderedItems = items.map((item, index) => {
    const itemInput: SqlParameterInput =
      item === null ? { kind: 'null', value: 'NULL' } : { kind: input.kind, value: String(item) };
...
function findSqlParameterOccurrences(
  sql: string,
  options?: SqlParameterOptions,
): ParameterOccurrence[] {
  ...
  const isSyntaxEnabled = (syntax: SqlParameterSyntax) =>
    !enabledSyntaxes || enabledSyntaxes.has(syntax);
  const complexTypeFieldSeparators =
    supportsNamedParameters && isSyntaxEnabled('named')
      ? collectComplexTypeFieldSeparators(sql, databaseType)
      : new Set<number>();
  const duckDbStructFieldSeparators =
    supportsNamedParameters && isSyntaxEnabled('named') && databaseType === 'duckdb'
      ? collectDuckDbStructFieldSeparators(sql)
      : new Set<number>();
  const triggerPseudoRecordFieldStarts =
    supportsNamedParameters && isSyntaxEnabled('named')
      ? collectTriggerPseudoRecordFieldStarts(sql, databaseType)
      : new Set<number>();
  let i = 0;
  let dollarQuoteEnd = '';
  ...
  while (i < sql.length) {
    ...
    if (ch === "'" || ch === '"') {
      ...
      if (ch === "'" && !hasSqlStringLiteralPrefix(sql, i)) {
        occurrences.push(
          ...collectEmbeddedQuotedBracedPlaceholders(sql, i + 1, quotedEnd, isSyntaxEnabled),
        );
      }
      i = quotedEnd;
      continue;
    ...
    if (ch === '[') {
      if (databaseType === 'postgres') {
        postgresBracketStack.push({
          constructor: isPostgresArrayConstructorBracket(
            sql,
            i,
            postgresBracketStack[postgresBracketStack.length - 1]?.constructor ?? false,
          ),
          parenthesisDepth,
        });
        i += 1;
    ...
    if (ch === '@' && isSyntaxEnabled('sqlserver')) {
      ...
      if (
        ...
        next !== '@' &&
        sql[i - 1] !== '@' &&
        !isOracleDatabaseLinkMarker(sql, i, options?.databaseType) &&
...
function isMysqlRoutineLabelSeparator(
  sql: string,
  colonIndex: number,
  followingName: string,
  databaseType?: DatabaseType,
): boolean {
  if (databaseType !== 'mysql' || !MYSQL_ROUTINE_LABEL_STATEMENTS.has(followingName.toLowerCase()))
    return false;
  ...
  return (
    prefix.length === 0 ||
    prefix.endsWith(';') ||
    MYSQL_ROUTINE_LABEL_CONTEXTS.has(previousKeyword(sql, labelStart))
  );
}

function readMyBatisForeachAt(
  sql: string,
  start: number,
  databaseType?: DatabaseType,
): { collection: string; render: MyBatisForeach; end: number } | null {
  ...
  if (
    !PARAMETER_NAME_RE.test(collection) ||
    !PARAMETER_NAME_RE.test(item) ||
    (index !== undefined && !PARAMETER_NAME_RE.test(index))
  )
    return null;
...
function findMatchingXmlTagClose(
  sql: string,
  start: number,
  tagName: string,
  databaseType?: DatabaseType,
...
function readXmlTagAt(
  sql: string,
  start: number,
  tagName: string,
): { closing: boolean; end: number } | null {
...
function isDuckDbCompactPrefixAliasSeparator(
  sql: string,
  index: number,
  databaseType?: DatabaseType,
): boolean {
...
function isOracleDatabaseLinkMarker(
  sql: string,
  index: number,
  ...
  const previous = sql[index - 1];
  return (
    PARAMETER_NAME_CHAR_RE.test(previous) ||
    previous === '$' ||
    previous === '#' ||
    previous === '"'
  );
...
function isPostgresQuestionMarkOperator(
  sql: string,
  index: number,
  databaseType: DatabaseType | undefined,
): boolean {
...
function collectTriggerPseudoRecordFieldStarts(
  sql: string,
  databaseType?: DatabaseType,
): Set<number> {
  ...
  while (i < sql.length) {
    ...
    if (aliases && ch === ':') {
      ...
      if (
        ...
        isTriggerPseudoRecordFieldReference(sql, i + 1 + name.length)
      ) {
        starts.add(i);
        i += 1 + name.length;
        continue;
...
function triggerPseudoRecordDefaults(
  databaseType?: DatabaseType,
): readonly TriggerPseudoRecordName[] | null {
...
function collectTriggerReferencingAliases(
  sql: string,
  start: number,
  aliases: Set<string>,
  defaults: readonly TriggerPseudoRecordName[],
): number {
...
function isStandaloneSlashDelimiter(sql: string, start: number): boolean {
  let before = start - 1;
  while (before >= 0 && (sql[before] === ' ' || sql[before] === '\t' || sql[before] === '\r'))
    before -= 1;
  if (before >= 0 && sql[before] !== '\n') return false;

  let after = start + 1;
  while (after < sql.length && (sql[after] === ' ' || sql[after] === '\t' || sql[after] === '\r'))
    after += 1;
...
function isJdbcxMcpScopedPackage(sql: string, start: number, nameEnd: number): boolean {
  ...
  return (
    /^\s*mcp\s*\(/i.test(extensionPrefix) && /(?:^|[,\s])args\s*=[^,]*$/i.test(extensionPrefix)
  );
}
...
function collectComplexTypeFieldSeparatorsInDeclaration(
  sql: string,
  start: number,
  kind: ComplexTypeDeclarationKind,
  separators: Set<number>,
  databaseType?: DatabaseType,
): number {
  let i = start;
...
function readComplexTypeDeclaration(
  sql: string,
  start: number,
): { kind: ComplexTypeDeclarationKind; openingBracket: number } | null {
  const kind: ComplexTypeDeclarationKind | null = matchesWord(sql, start, 'struct')
    ? 'struct'
    : matchesWord(sql, start, 'variant')
      ? 'variant'
      : null;
  if (!kind) return null;
...
function readComplexTypeFieldNameEnd(
  sql: string,
  start: number,
  kind: ComplexTypeDeclarationKind,
  databaseType?: DatabaseType,
...
function readVariantFieldNameEnd(sql: string, start: number): number {
  let i = start;
  const modifier = matchesWord(sql, i, 'match_name')
    ? 'match_name'
    : matchesWord(sql, i, 'match_name_glob')
      ? 'match_name_glob'
      : '';
...
function collectNativeSqlServerParameters(
  sql: string,
  databaseType?: DatabaseType,
): { declared: Set<string>; ignoredStarts: Set<number> } {
  ...
  while (i < sql.length) {
    ...
    if (
      (matchesWord(sql, i, 'create') || matchesWord(sql, i, 'alter')) &&
      isRoutineDefinitionStart(sql, i)
    ) {
      i = collectRoutineDefinitionVariables(sql, i, declared, databaseType);
      continue;
    }
    if (matchesWord(sql, i, 'exec') || matchesWord(sql, i, 'execute')) {
      i = collectExecNamedArgumentStarts(
        sql,
        i + (matchesWord(sql, i, 'exec') ? 'exec'.length : 'execute'.length),
        ignoredStarts,
        databaseType,
      );
      continue;
...
function collectDeclareStatementVariables(
  sql: string,
  start: number,
  declared: Set<string>,
  databaseType?: DatabaseType,
...
function collectSetStatementVariables(
  sql: string,
  start: number,
  declared: Set<string>,
  databaseType?: DatabaseType,
  ...
  while (i < sql.length) {
    ...
    if (ch === '@') {
      ...
      if (
        ...
        next !== '@' &&
        sql[i - 1] !== '@' &&
        isSetAssignmentTarget(sql, i + 1 + name.length)
      ) {
        declared.add(name.toLowerCase());
        i += 1 + name.length;
        continue;
...
function collectSelectAssignmentVariables(
  sql: string,
  start: number,
  declared: Set<string>,
  databaseType?: DatabaseType,
...
function collectMysqlSelectIntoTargets(
  sql: string,
  start: number,
  declared: Set<string>,
): number | null {
...
function collectRoutineDefinitionVariables(
  sql: string,
  start: number,
  declared: Set<string>,
  databaseType?: DatabaseType,
...
function collectExecNamedArgumentStarts(
  sql: string,
  start: number,
  ignoredStarts: Set<number>,
  databaseType?: DatabaseType,
): number {
  let i = start;
  while (i < sql.length) {
...
function isRoutineDefinitionStart(sql: string, start: number): boolean {
  const keyword = matchesWord(sql, start, 'create')
    ? 'create'
    : matchesWord(sql, start, 'alter')
      ? 'alter'
      : '';
...
function isSqlStatementKeyword(sql: string, start: number): boolean {
  return [
    ...
    'drop',
    'truncate',
  ].some((keyword) => matchesWord(sql, start, keyword));
}
...
function matchesWord(sql: string, start: number, word: string): boolean {
  ...
  return (
    !PARAMETER_NAME_CHAR_RE.test(sql[start - 1] ?? '') &&
    !PARAMETER_NAME_CHAR_RE.test(sql[start + word.length] ?? '')
  );
}
...
function tryReadQuotedBracedPlaceholder(
  sql: string,
  start: number,
  quote: "'" | '"',
  isSyntaxEnabled: (syntax: SqlParameterSyntax) => boolean,
): ParameterOccurrence | null {
...
function collectEmbeddedQuotedBracedPlaceholders(
  sql: string,
  contentStart: number,
  quotedEnd: number,
  isSyntaxEnabled: (syntax: SqlParameterSyntax) => boolean,
...
function hasSqlStringLiteralPrefix(sql: string, quoteStart: number): boolean {
  ...
  if (
    quoteStart >= 2 &&
    sql[quoteStart - 1] === '&' &&
    (sql[quoteStart - 2] === 'U' || sql[quoteStart - 2] === 'u') &&
    !PARAMETER_NAME_CHAR_RE.test(sql[quoteStart - 3] ?? '')
  ) {
    return true;
  }
...
function isPostgresArrayConstructorBracket(
  sql: string,
  start: number,
  parentIsConstructor: boolean,
): boolean {
...
function isPostgresSliceSeparator(
  stack: Array<{ constructor: boolean; parenthesisDepth: number }>,
  parenthesisDepth: number,
): boolean {
...
function sqlParameterQuotedString(input: SqlParameterInput): string {
  if (
    input.kind === 'null' ||
    ((input.kind === 'number' || input.kind === 'raw') && !input.value.trim())
  )
    return 'NULL';

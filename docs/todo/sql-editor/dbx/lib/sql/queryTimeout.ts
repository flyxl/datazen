...
export function queryTimeoutSecsForConnection(
  ...
  if (connection?.query_timeout_inherit === true) {
    ...
    return Number.isFinite(globalValue) && globalValue >= 0
      ? globalValue
 ...
 */
export function queryTimeoutSecsForConcurrentIndex(
  configuredTimeoutSecs: number,
  hasConcurrentIndexBuild: boolean,
): number {
...
export function frontendQueryTimeoutSecsForSql(
  sql: string,
...
export function frontendQueryTimeoutDelayMs(timeoutSecs: number): number | undefined {
  ...
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0 || timeoutMs > MAX_BROWSER_TIMER_DELAY_MS)
    return undefined;
  return timeoutMs;
}
...
function postgresQueryMayReturnRows(sql: string, databaseType: DatabaseType): boolean {
  return splitSqlStatementRanges(sql, databaseType).some(({ sql: statement }) => {
    ...
    const firstKeyword = tokens.find(
      (token) => token.kind === 'word' && token.depth === 0,
    )?.normalized;
    ...
    return tokens.some(
      (token) => token.kind === 'word' && token.depth === 0 && token.normalized === 'returning',
    );
  });
}

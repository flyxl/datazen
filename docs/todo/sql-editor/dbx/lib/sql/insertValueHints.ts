...
function statementTokenGroups(
  sql: string,
  tokens: readonly SqlSemanticToken[],
): Array<{ span: SqlSemanticSpan; tokens: SqlSemanticToken[] }> {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (span.end > span.start)
      groups.push({ span, tokens: significantTokens(tokens.slice(tokenStart, index)) });
    start = item.span.end;
    tokenStart = index + 1;
  }
  const last = trimSpan(sql, start, sql.length);
  if (last.end > last.start)
    groups.push({ span: last, tokens: significantTokens(tokens.slice(tokenStart)) });
  return groups;
}
...
export function readQualifiedName(
  tokens: readonly SqlSemanticToken[],
  startIndex: number,
): { name: string; schema?: string; database?: string; nextIndex: number } | null {
...
function valueStartsInRow(
  tokens: readonly SqlSemanticToken[],
  openIndex: number,
): { starts: number[]; nextIndex: number } | null {
  const open = tokens[openIndex];
  if (!open || open.text !== '(') return null;
...
function parseValuesRows(tokens: readonly SqlSemanticToken[], valuesIndex: number): number[][] {
  ...
  while (index < tokens.length) {
    ...
    if (
      item.kind === 'word' &&
      (item.normalized === 'returning' || item.normalized === 'on' || item.normalized === 'select')
    )
      break;
    if (item.text === '(') {
...
function findInsertSourceSelectIndex(
  tokens: readonly SqlSemanticToken[],
  from: number,
  sourceDepth: number,
): number {
  if (
    tokens[from]?.text === '(' &&
    tokens[from + 1]?.depth === sourceDepth + 1 &&
    tokens[from + 1]?.kind === 'word' &&
    tokens[from + 1]?.normalized === 'select'
  ) {
    return from + 1;
  }
...
const SELECT_CLAUSE_BOUNDARIES = new Set([
  'from',
  'into',
  'where',
  'group',
...
function isWildcardProjection(
  ...
  return (
    topLevel.length % 2 === 1 &&
    topLevel.every((item, index) =>
      index % 2 === 0
        ? index === topLevel.length - 1
          ? item.text === '*'
          : tokenIsIdentifier(item)
        : item.text === '.',
    )
  );
}

function selectProjectionStarts(
  tokens: readonly SqlSemanticToken[],
  selectIndex: number,
): number[] {
  ...
  if (
    ...
    tokens[index]?.normalized === 'all'
  ) {
    index += 1;
  } else if (
    tokens[index]?.depth === depth &&
    tokens[index]?.kind === 'word' &&
    tokens[index]?.normalized === 'distinct'
  ) {
    index += 1;
  ...
  const hasTopModifier =
    maybeTop?.depth === depth &&
    maybeTop.kind === 'word' &&
    maybeTop.normalized === 'top' &&
    topValue?.depth === depth &&
    (topValue.text === '(' ||
      topValue.kind === 'number' ||
      topValue.kind === 'parameter' ||
      (topValue.kind === 'word' && topValue.text.startsWith('@')));
  if (hasTopModifier) {
    index += 1;
    ...
    if (
      tokens[index]?.depth === depth &&
      tokens[index]?.normalized === 'with' &&
      tokens[index + 1]?.depth === depth &&
      tokens[index + 1]?.normalized === 'ties'
    )
      index += 2;
  }
  ...
  for (; index < tokens.length; index += 1) {
    ...
    if (index <= expressionStart || isWildcardProjection(tokens, expressionStart, index, depth))
      return [];
...
function parseInsertClause(
  tokens: readonly SqlSemanticToken[],
  span: SqlSemanticSpan,
): InsertValuesClause | null {
  ...
  if (
    tokenIsIdentifier(tokens[index]) &&
    tokens[index]?.normalized !== 'values' &&
    tokens[index]?.normalized !== 'select' &&
    tokens[index]?.normalized !== 'default'
  ) {
  ...
  const rows =
    selectIndex >= 0 && (valuesIndex < 0 || selectIndex < valuesIndex)
      ? [selectProjectionStarts(tokens, selectIndex)]
      : parseValuesRows(tokens, valuesIndex);
 ...
 */
function scanStatementBoundaries(
  sql: string,
  from: number,
  searchEndFrom: number,
  hardStop: number,
  trackStart: boolean,
  dialectId: string,
  initialState?: LexState,
): { start: number; startFound: boolean; end: number; endFound: boolean; finalState: LexState } {
  const state: LexState = initialState
    ? { ...initialState }
    : { depth: 0, inLineComment: false, inBlockComment: false, quote: null, dollarTag: null };
  let start = from;
...
function computeSqlStatementWindow(
  sql: string,
  from: number,
  to: number,
  dialectId: string,
  ...
  while (!result.endFound && hardStop < sql.length) {
    ...
    result = scanStatementBoundaries(
      sql,
      resumeFrom,
      safeTo,
      hardStop,
...
export function findEnclosingDollarQuoteStart(sql: string, position: number): number | null {
  return findEnclosingDollarQuoteStartInText(
    { length: sql.length, sliceString: (from, to) => sql.slice(from, to) },
    position,
  );
}
...
export function findEnclosingDollarQuoteStartInText(
  doc: SqlTextSlice,
  position: number,
): number | null {
...
export function expandToSqlStatementWindow(
  sql: string,
  from: number,
  to: number,
  dialectId = 'mysql',
  ...
  if (
    statementWindowMemo &&
    statementWindowMemo.sql === sql &&
    statementWindowMemo.from === from &&
    statementWindowMemo.to === to &&
    statementWindowMemo.dialectId === dialectId
  ) {
...
export function parseInsertValuesClausesInRanges(
  sql: string,
  ranges: readonly TextRange[],
): InsertValuesClause[] {
  if (!sql.trim() || ranges.length === 0) return [];
  const windows = mergeTextRanges(
    ranges.map((range) => expandToSqlStatementWindow(sql, range.from, range.to)),
  );
...
export function buildInsertValueHints(
  clauses: readonly InsertValuesClause[],
  options: ParseInsertValueHintsOptions = {},
): InsertValueHint[] {
  const hints: InsertValueHint[] = [];
  for (const clause of clauses) {
    const columns =
      clause.columns ?? options.resolveTableColumns?.(clause.table, clause.schema, clause.database);
...
export function parseInsertValueHints(
  sql: string,
  options: ParseInsertValueHintsOptions = {},
): InsertValueHint[] {
...
export function parseInsertValueHintsInRanges(
  sql: string,
  ranges: readonly TextRange[],
  options: ParseInsertValueHintsOptions = {},
): InsertValueHint[] {
...
export function parseInsertValueHintsAtCursor(
  sql: string,
  cursor: number,
  options: ParseInsertValueHintsOptions = {},
): InsertValueHint[] {

...
interface ResolveNewQueryTargetInput {
  activeTab?: Pick<
    QueryTab,
    'connectionId' | 'database' | 'schema' | 'catalog' | 'objectBrowser' | 'tableMeta'
  >;
...
export function resolveNewQueryTarget(input: ResolveNewQueryTargetInput): NewQueryTarget | null {
  const primaryContext =
    input.preferredSource === 'sidebar' ? input.selectedTreeNode || undefined : input.activeTab;
  const secondaryContext =
    input.preferredSource === 'sidebar' ? input.activeTab : input.selectedTreeNode || undefined;
...
function targetFromContext(
  context:
    | Pick<
        ...
        'connectionId' | 'database' | 'schema' | 'catalog' | 'objectBrowser' | 'tableMeta'
      >
    | Pick<TreeNode, 'connectionId' | 'database' | 'schema' | 'catalog'>
    | undefined,
  connections: Pick<ConnectionConfig, 'id' | 'host' | 'database' | 'default_schema' | 'db_type'>[],
): NewQueryTarget | null {
  ...
  const database =
    connection.db_type === 'sqlite'
      ? normalizeSqliteNamespace(contextDatabase, connection)
      : contextDatabase;
  ...
  return {
    ...
    database,
    schema:
      context.schema ?? objectBrowser?.schema ?? tableMeta?.schema ?? connection.default_schema,
...
export interface ResolveNewQueryTableInput {
  activeTab?: Pick<
    ...
    | 'title'
  > | null;
  selectedTreeNode?: Pick<
    TreeNode,
    'type' | 'connectionId' | 'database' | 'schema' | 'catalog' | 'tableName' | 'label'
  > | null;
  preferredSource?: NewQueryContextSource;
}
...
const NEW_QUERY_PREFILL_DISABLED_TYPES: ReadonlySet<DatabaseType | undefined> = new Set<
...
export function buildSelectAllSql(
  ...
  table: Pick<NewQueryTable, 'schema' | 'catalog' | 'tableName'> &
    Partial<Pick<NewQueryTable, 'database'>>,
  identifierQuote?: string,
  driverProfile?: string,
  includeDatabaseName = false,
): string {
  if (databaseType === 'victoriametrics') return metricRangeQuery(table.tableName);
  const ref = qualifiedTableName({
    databaseType,
    driverProfile,
    identifierQuote,
 ...
 */
export function resolveNewQueryInitialSql(
  input: ResolveNewQueryInitialSqlInput,
): string | undefined {
  ...
  if (
    !table ||
    table.connectionId !== input.targetConnectionId ||
    table.database !== input.targetDatabase
  )
    return undefined;

  return buildSelectAllSql(
    input.databaseType,
    table,
    input.identifierQuote,
    input.driverProfile,
    input.includeDatabaseName,
  );
}

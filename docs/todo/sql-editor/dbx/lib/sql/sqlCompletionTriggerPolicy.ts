...
export const SQL_COMPLETION_TRIGGER_MODES: readonly SqlCompletionTriggerMode[] = [
  'manual',
  'require-prefix',
  'positional',
];

export function originForTypedSqlCompletionStart(
  activeOrigin: SqlCompletionTriggerOrigin | null,
): SqlCompletionTriggerOrigin {
...
export function originForSqlCompletionProvider(
  activeOrigin: SqlCompletionTriggerOrigin | null,
  explicit: boolean,
): SqlCompletionTriggerOrigin {
...
export function normalizeCompletionTriggerMode(value: unknown): SqlCompletionTriggerMode {
  if (
    typeof value === 'string' &&
    (value === 'manual' || value === 'require-prefix' || value === 'positional')
  ) {
    return value;
  }
  return 'positional';
 ...
 */
export function shouldAllowSqlCompletionTrigger(
  mode: SqlCompletionTriggerMode,
  facts: SqlCompletionTriggerFacts,
): boolean {
  ...
  if (mode === 'require-prefix') {
    return (
      ...
      (facts.useDatabasePrefix != null && facts.useDatabasePrefix.length > 0)
    );
  }
  // positional

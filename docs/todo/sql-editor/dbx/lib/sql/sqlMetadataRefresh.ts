...
const EXPLICIT_TEMP_OBJECT_DDL_RE =
  /\b(?:CREATE|ALTER|DROP|RENAME)\s+(?:OR\s+REPLACE\s+)?(?:(?:GLOBAL|LOCAL)\s+)?TEMP(?:ORARY)?\s+(?:TABLE|VIEW|INDEX)\b/i;
const SQLSERVER_TEMP_TABLE_DDL_RE =
  /\b(?:CREATE|ALTER|DROP)\s+TABLE\s+(?:IF\s+(?:NOT\s+)?EXISTS\s+)?(?:\[##?[^\]]+\]|##?[A-Za-z_][\w$#]*)/i;
...
const SQLSERVER_TEMP_TABLE_TOKEN_RE =
...
export function sqlMetadataRefreshTarget(
  sql: string,
  activeSchema?: string,
): SqlMetadataRefreshTarget {

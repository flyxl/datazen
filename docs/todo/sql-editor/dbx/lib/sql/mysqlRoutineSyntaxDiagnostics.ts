...
const NON_ROUTINE_CREATE_TYPES = new Set([
  'DATABASE',
  'INDEX',
  'LOGFILE',
  'ROLE',
...
export function analyzeMysqlRoutineSyntax(sql: string): MysqlRoutineSyntaxAnalysis {
  ...
  for (const statement of executableStatementRanges(sql, 'mysql')) {
    ...
    declarations.forEach((declaration, declarationIndex) => {
      const nextDeclarationTokenIndex =
        declarations[declarationIndex + 1]?.createTokenIndex ?? tokens.length;
      const parameterStart = findTokenKind(
        tokens,
        declaration.tokenIndex + 1,
        nextDeclarationTokenIndex,
        'leftParen',
      );
      if (parameterStart === -1) return;
      const parameterEnd = findMatchingRightParen(
        tokens,
        parameterStart,
        nextDeclarationTokenIndex,
      );
      ...
      if (previousParameterToken?.kind === 'comma') {
        diagnostics.push(
          diagnosticAtOffset(
            sql,
            previousParameterToken.from,
            previousParameterToken.to,
            'Trailing comma is not allowed in a MySQL routine parameter list',
          ),
        );
      }
...
function findRoutineDeclarations(tokens: readonly RoutineToken[]): RoutineDeclaration[] {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    for (
      ...
      lookahead < Math.min(tokens.length, index + 24);
      lookahead += 1
    ) {
...
function findMatchingRightParen(
  tokens: readonly RoutineToken[],
  start: number,
  to: number,
): number {
  ...
  while (index < sql.length) {
    ...
    if (word) {
      tokens.push({
        ...
        from: baseOffset + index,
        to: baseOffset + index + word.length,
      });
      index += word.length;
      continue;
    }

 ...
 */
export function tokenizeSqlSemantic(
  input: string,
  dialectId = 'mysql',
  options?: {
    mysqlDashCommentRequiresWhitespace?: boolean;
    mysqlBackslashEscape?: boolean;
    mysqlDoubleQuoteIsString?: boolean;
  },
): SqlSemanticToken[] {
  ...
  while (index < input.length) {
    ...
    if (
      ch === '-' &&
      next === '-' &&
      (!mysqlDashCommentRequiresWhitespace ||
    ...
    if (ch === "'") {
      ...
      index = quoted.end;
      tokens.push(
        token('string', input.slice(start, index), start, index, depth, "'", quoted.closed),
      );
      continue;
    }
    ...
    if (ch === '$') {
      ...
      if (marker) {
        ...
        tokens.push(
          token('string', input.slice(start, index), start, index, depth, marker, closing >= 0),
        );
        continue;
      }
    }
...
export function isSuppressedSqlSemanticContext(
  tokens: readonly SqlSemanticToken[],
  cursor: number,
): boolean {
  return tokens.some(
    (item) =>
      (item.kind === 'comment' || item.kind === 'string') &&
      item.span.start < cursor &&
      cursor <= item.span.end,
  );
}

export function findActiveSqlStatementSpan(
  sql: string,
  tokens: readonly SqlSemanticToken[],
  cursor: number,
): SqlSemanticSpan {
...
export function tokenIsIdentifier(
  tokenValue: SqlSemanticToken | undefined,
): tokenValue is SqlSemanticToken {

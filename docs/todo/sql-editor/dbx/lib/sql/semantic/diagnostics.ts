...
import {
  ...
  isOraclePlSqlStatement,
  type SqlTextRange,
} from '@/lib/sql/sqlStatementRanges';
...
export function sqlSemanticDiagnosticRangesForViewport(
  sql: string,
  visibleRanges: readonly SqlSemanticDiagnosticVisibleRange[],
  databaseType?: DatabaseType,
): SqlTextRange[] {
  const statements =
    databaseType === 'sqlserver'
      ? sqlServerSemanticDiagnosticRanges(sql)
      : executableStatementRanges(sql, databaseType);
...
function isSqlServerRoutineDefinitionBatch(sql: string): boolean {
  ...
  const routineKeyword = (value: string | undefined) =>
    value === 'PROC' || value === 'PROCEDURE' || value === 'FUNCTION';
  return (
    (keywords[0] === 'ALTER' && routineKeyword(keywords[1])) ||
    (keywords[0] === 'CREATE' && routineKeyword(keywords[1])) ||
    (keywords[0] === 'CREATE' &&
      keywords[1] === 'OR' &&
      keywords[2] === 'ALTER' &&
      routineKeyword(keywords[3]))
  );
}
...
function sqlServerBatchRanges(sql: string): SqlTextRange[] {
  ...
  for (const separator of separators) {
    if (separator.from > from)
      ranges.push({ from, to: separator.from, sql: sql.slice(from, separator.from) });
    from = separator.to;
  }
...
function sqlServerGoSeparatorRanges(sql: string): Array<{ from: number; to: number }> {
  ...
  while (lineStart <= sql.length) {
    ...
    if (
      ...
      state.quote === 'none' &&
      isSqlServerGoSeparatorLine(line)
    ) {
...
function pushSqlServerDiagnosticBatchRanges(
  ranges: SqlTextRange[],
  sql: string,
  batch: readonly SqlTextRange[],
) {
  if (
    batch.length > 1 &&
    batch.some((statement) => sqlServerStatementNeedsBatchContext(statement.sql))
  ) {
...
function sqlServerStatementNeedsBatchContext(sql: string): boolean {
  return (
    /^\s*(?:WHILE|BEGIN|END|IF|ELSE|TRY|CATCH)\b/i.test(sql) ||
    /^\s*DECLARE\s+(?:\[[^\]]+\]|"[^"]+"|[A-Z_@#][\w@$#]*)\s+CURSOR\b/i.test(sql)
  );
}
...
export function buildSqlSemanticDiagnostics(
  ...
  for (const column of analysis.columns) {
    ...
    if (
      schema.databaseType === 'tdengine' &&
      normalizeName(column.name) === DBX_TDENGINE_TBNAME_COLUMN
    ) {
...
function completionTableReferenceKey(
  table: Pick<SqlCompletionTable, 'name' | 'database' | 'schema'>,
): string {
  if (table.schema)
    return normalizeName(
      `${table.database ? `${table.database}.` : ''}${table.schema}.${table.name}`,
    );
  return normalizeName(table.name);
}

function isUnquotedOracleSystemValueReference(
...
export function isSqlVirtualTableReference(
...
function sqlTextSpanToOffsetRange(
  sql: string,
  span: SqlTextSpan,
  ...
  const to = sqlTextPositionToOffset(
    sql,
    Math.max(span.end_line, span.start_line),
    Math.max(span.end_column, span.start_column),
  );
...
function offsetToSqlTextStartPosition(
  sql: string,
  offset: number,
...
function offsetToSqlTextEndPosition(
  sql: string,
  offset: number,
): { line: number; column: number } | null {
...
export function buildSqlParserErrorDiagnostic(
  error: unknown,
  sql: string,
): SqlSemanticDiagnostic | null {
...
export function shouldRunSqlSemanticDiagnostics(
  ...
  if (
    (context.suggestTables || context.exclusiveTableSuggestions) &&
    isCursorAfterTableTrigger(sql, cursor)
  )
    return false;
  return true;
}

export function isSqlSemanticDiagnosticInputContext(
  sql: string,
  cursor: number,
  options: { databaseType?: DatabaseType } = {},
): boolean {
...
function resolveColumnTable(
  ...
  sql?: string,
  scopesById?: Map<number, SqlReferenceScope>,
): SqlTableReference | null {
  const candidateTables = candidateTablesForColumn(tables, column, sql, scopesById);
  if (column.qualifier) {
    return (
      tableLookupFor(candidateTables).get(normalizeName(column.qualifier)) ??
      (sql ? null : (knownTables.get(normalizeName(column.qualifier)) ?? null))
    );
  }
...
function candidateTablesForColumn(
  tables: SqlTableReference[],
  column: SqlColumnReference,
  sql?: string,
  scopesById?: Map<number, SqlReferenceScope>,
...
function tablesInVisibleScopes(
  tables: SqlTableReference[],
  column: SqlColumnReference,
  scopesById?: Map<number, SqlReferenceScope>,
): SqlTableReference[] | null {
...
function tableLookupFor(tables: SqlTableReference[]): Map<string, SqlTableReference> {
  ...
  for (const table of tables) {
    ...
    if (table.database && table.schema)
      lookup.set(normalizeName(`${table.database}.${table.schema}.${table.name}`), table);
  }
  return lookup;
}

function tablesInSameStatement(
  tables: SqlTableReference[],
  column: SqlColumnReference,
  sql: string,
): SqlTableReference[] {
  ...
  return tables.filter((table) => {
    ...
    return (
      tableOffset != null &&
      statementIndexAt(sql, tableOffset) === statementIndexAt(sql, columnOffset)
    );
  });
}

function scopesByIdMap(
  scopes: readonly SqlReferenceScope[] | undefined,
): Map<number, SqlReferenceScope> {
...
function columnsForTable(
  ...
  const keys = table.schema
    ...
    : [table.name, ...keysWithTableName(columnsByTable, table.name)];
...
function rangesIntersect(
  left: SqlSemanticDiagnosticVisibleRange,
  right: SqlSemanticDiagnosticVisibleRange,
): boolean {
  return left.from < right.to && right.from < left.to;
}

export function tableReferenceKey(
  table: Pick<SqlTableReference, 'name' | 'database' | 'schema'>,
): string {
  return normalizeName(
    table.schema
      ? `${table.database ? `${table.database}.` : ''}${table.schema}.${table.name}`
...
function normalizeName(value: string): string {
  ...
  while (normalized && `"'\`[]`.includes(normalized[normalized.length - 1]))

...
export type SqlSemanticCompletionScopeKind =
  | 'keyword'
  | 'table'
  | 'schema'
  | 'catalog'
...
function selectStarToken(model: SqlSemanticModel): SqlSemanticToken | undefined {
  ...
  return model.tokens.find(
    (token) =>
      token.text === '*' && token.span.start === range.start && token.span.end === range.end,
  );
}
...
export function sqlSemanticSelectStarTableSources(model: SqlSemanticModel): SqlSemanticRowSource[] {
  ...
  if (model.cursorIntent.targetSourceId) {
    const target = model.rowSources.find(
      (source) => source.id === model.cursorIntent.targetSourceId,
    );
...
export function sqlSemanticSelectStarTableSource(
...
export function sqlSemanticSelectStarQualifierSql(model: SqlSemanticModel): string | undefined {
  ...
  while (
    ...
    isSemanticIdentifier(model.tokens[index - 2])
  ) {
    index -= 2;
...
export function sqlSemanticSelectStarIsOnlyProjection(model: SqlSemanticModel): boolean {
  ...
  const fromIndex = model.tokens.findIndex(
    (token, index) =>
      index > selectIndex &&
      token.span.start >= star.span.end &&
      token.depth === star.depth &&
      token.kind === 'word' &&
      token.normalized === 'from',
  );
  const projectionEnd =
    fromIndex >= 0 ? model.tokens[fromIndex]!.span.start : model.statement.span.end;
  const projectionTokens = model.tokens.filter(
    (token) =>
      token.span.start >= model.tokens[selectIndex]!.span.end &&
      token.span.end <= projectionEnd &&
      token.kind !== 'comment',
  );
  if (
    projectionTokens[0]?.kind === 'word' &&
    (projectionTokens[0].normalized === 'all' || projectionTokens[0].normalized === 'distinct')
  )
    projectionTokens.shift();
  if (projectionTokens.pop() !== star) return false;
  if (projectionTokens.length === 0) return true;
  return (
    projectionTokens.length % 2 === 0 &&
    projectionTokens.every((token, index) =>
      index % 2 === 0 ? isSemanticIdentifier(token) : token.text === '.',
    )
  );
}

export function sqlSemanticReferencedTables(
  model: SqlSemanticModel,
): SqlCompletionReferencedTable[] {
  return model.rowSources
    ...
    .map((source) => {
      ...
      return {
        ...
        schemaQuoted:
          source.qualifierParts.length > 0
            ? !!identifierParts[identifierParts.length - 2]?.quote
            : undefined,
        alias: source.alias,
        aliasSql: source.aliasSpan
          ? model.sql.slice(source.aliasSpan.start, source.aliasSpan.end)
          : source.alias,
...
export function sqlSemanticLocalColumnsByTable(
  model: SqlSemanticModel,
...
function activeProjectionAliasClause(
  model: SqlSemanticModel,
): 'where' | 'groupBy' | 'having' | 'orderBy' | null {
  const words = model.tokens
    .filter((token) => token.span.end <= model.cursor && token.kind === 'word')
...
export function sqlSemanticCompletionScope(model: SqlSemanticModel): SqlSemanticCompletionScope {
  ...
  const targetSource = intent.targetSourceId
    ? model.rowSources.find((source) => source.id === intent.targetSourceId)
    : undefined;
  switch (intent.kind) {
    case 'table':
    case 'delete_target':
    ...
    case 'star':
      return {
        ...
        targetSource,
        useRemoteMetadata:
          intent.confidence !== 'low' && (!!targetSource || model.rowSources.length > 0),
        fallbackReason: intent.fallbackReason,
      };
    case 'suppressed':
      return {
...
function semanticMutationTarget(model: SqlSemanticModel): SqlSemanticRowSource | undefined {
  ...
  return targetId
    ? model.rowSources.find((source) => source.id === targetId)
    : model.rowSources.find((source) => source.kind === 'mutation_target');
}

export function sqlCompletionContextFromSemantic(
  model: SqlSemanticModel,
  base: SqlCompletionContext,
): SqlCompletionContext {
  ...
  if (
    (base.suggestTables || base.exclusiveTableSuggestions) &&
    model.cursorIntent.kind !== 'table' &&
    model.cursorIntent.kind !== 'schema' &&
    model.cursorIntent.kind !== 'catalog' &&
    model.cursorIntent.kind !== 'delete_target'
  ) {
  ...
  const basePrefixHasNonAscii = Array.from(base.prefix).some(
    (character) => (character.codePointAt(0) ?? 0) > 0x7f,
  );
  const useBaseTrailingIdentifier =
    basePrefixHasNonAscii &&
    model.cursorIntent.prefix.length === 0 &&
    model.cursorIntent.replacementRange.start === model.cursorIntent.replacementRange.end &&
  ...
  const qualifierParts =
    model.cursorIntent.qualifierParts.length > 0
      ? [...model.cursorIntent.qualifierParts]
      : useBaseTrailingIdentifier
        ? base.qualifierParts
        : undefined;
  ...
  const suggestTables =
    scope.kind === 'table' || scope.kind === 'schema' || scope.kind === 'catalog';
  const suggestColumns = scope.kind === 'columns';
  const suggestRoutines =
    scope.kind === 'routine' ||
    (suggestColumns && base.suggestRoutines && !base.exclusiveColumnSuggestions);
  ...
  return {
    ...
    suggestColumns,
    suggestKeywords:
      scope.kind === 'keyword' || (!suggestTables && !suggestColumns && !suggestRoutines),
    suggestRoutines,
    ...
    exclusiveColumnSuggestions:
      model.cursorIntent.kind === 'alias_column' ||
      model.cursorIntent.kind === 'insert_column' ||
      model.cursorIntent.kind === 'update_column',
    ...
    insertTable:
      model.cursorIntent.kind === 'insert_column' ? mutationTarget?.name : base.insertTable,
    insertDatabase:
      model.cursorIntent.kind === 'insert_column' ? mutationDatabase : base.insertDatabase,
    insertSchema: model.cursorIntent.kind === 'insert_column' ? mutationSchema : base.insertSchema,
    updateTarget:
      model.cursorIntent.kind === 'update_column' && mutationTarget
        ? { table: mutationTarget.name, schema: mutationSchema }
        : base.updateTarget,
    deleteTarget:
      model.cursorIntent.kind === 'delete_target' && mutationTarget
        ? { table: mutationTarget.name, schema: mutationSchema }
        : base.deleteTarget,

...
export function sqlSemanticTableReferences(
  model: SqlSemanticModel,
  scopeId = 0,
): SqlSemanticTableReference[] {
  if (model.cursorIntent.kind === 'suppressed' || model.cursorIntent.confidence === 'low')
    return [];
...
export function mergeSqlSemanticReferenceAnalysis(
  ...
  for (const table of semanticTables) {
    const existing = [...merged.values()].find((candidate) => {
      ...
      if (
        candidate.database &&
        table.database &&
        normalized(candidate.database) !== normalized(table.database)
      )
        return false;
      if (candidate.alias && table.alias && normalized(candidate.alias) !== normalized(table.alias))
        return false;
      if (
        candidate.schema &&
        table.schema &&
        normalized(candidate.schema) !== normalized(table.schema)
      )
        return false;
      return true;
    });
...
export function sqlSemanticCompletionReferenceTables(
  model: SqlSemanticModel,
...
export function resolveSqlSemanticNavigationTarget(
  model: SqlSemanticModel,
  identifierParts: readonly string[],
): SqlSemanticNavigationTarget | null {
  if (
    model.cursorIntent.kind === 'suppressed' ||
    model.cursorIntent.confidence === 'low' ||
  ...
  const qualifier =
    normalizedParts.length > 1 ? normalizedParts[normalizedParts.length - 2] : undefined;
  const database =
    normalizedParts.length > 2 ? normalizedParts[normalizedParts.length - 3] : undefined;
  ...
  const source =
    model.rowSources.find((candidate) => {
      ...
      if (qualifier) {
        ...
        return (
          candidateName === name &&
          (!candidateSchema || candidateSchema === qualifier) &&
          (!database || !candidateDatabase || candidateDatabase === database)
        );
      }

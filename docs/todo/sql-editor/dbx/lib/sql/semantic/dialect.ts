...
export interface SqlSemanticDialectAdapter {
  ...
  qualifierRole(
    parts: string[],
    context: 'table' | 'column' | 'routine',
  ): 'catalog' | 'schema' | 'table' | 'package' | 'unknown';
}
...
function roleForGenericQualifier(
  parts: string[],
  context: 'table' | 'column' | 'routine',
): 'catalog' | 'schema' | 'table' | 'package' | 'unknown' {
...
export function sqlReferenceAnalysisDialectFor(options: {
...
export function sqlSemanticDialectFor(options: {
  databaseType?: DatabaseType;
  dialect?: 'mysql' | 'postgres' | 'sqlserver' | 'clickhouse';
}): SqlSemanticDialectAdapter {
  if (options.databaseType === 'clickhouse') return SQL_SEMANTIC_DIALECTS.clickhouse;
  if (options.dialect && SQL_SEMANTIC_DIALECTS[options.dialect])
    return SQL_SEMANTIC_DIALECTS[options.dialect];
 ...
 */
export function resolveSqlDialectId(options: {
  databaseType?: DatabaseType;
  dialect?: 'mysql' | 'postgres' | 'sqlserver' | 'clickhouse';
}): string {

...
export function isExternalSqlFileTooLargeError(
  error: unknown,
): error is ExternalSqlFileTooLargeError {
...
export function readBrowserSqlFile(file: Blob): Promise<string> {
  if (file.size > MAX_EXTERNAL_SQL_EDITOR_FILE_BYTES) {
    return Promise.reject(
      new ExternalSqlFileTooLargeError(file.size, MAX_EXTERNAL_SQL_EDITOR_FILE_BYTES),
    );
  }
...
export function externalSqlFileOpenErrorMessage(
  error: unknown,
  translate: (key: string, params: { size: string; limit: string }) => string,
): string {

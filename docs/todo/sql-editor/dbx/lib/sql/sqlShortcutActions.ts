...
export function enabledSqlShortcutActions(
  actions: readonly SqlShortcutAction[],
): SqlShortcutAction[] {
...
export function findSqlShortcutConflicts(
  actions: readonly SqlShortcutAction[],
  fixedShortcuts: ShortcutSettings,
  platform = globalThis.navigator?.platform || '',
): string[] {
  const conflicts = new Set<string>();
  const editorFixedShortcuts = SHORTCUT_DEFINITIONS.filter(
    (item) => item.scope === 'editor' || item.scope === 'global',
  );
  ...
  for (const action of actions) {
    ...
    const duplicate = actions.find(
      (other) =>
        ...
        other.enabled !== false &&
        other.shortcut.trim() &&
        shortcutsUseSameKeys(other.shortcut, action.shortcut, platform),
    );
    if (duplicate) {
    ...
    const fixedConflict = editorFixedShortcuts.find(
      (item) =>
        fixedShortcuts[item.id] &&
        shortcutsUseSameKeys(fixedShortcuts[item.id], action.shortcut, platform),
    );
...
export function hasSqlShortcutConflicts(
  actions: readonly SqlShortcutAction[],
  fixedShortcuts: ShortcutSettings,
  platform = globalThis.navigator?.platform || '',
): boolean {

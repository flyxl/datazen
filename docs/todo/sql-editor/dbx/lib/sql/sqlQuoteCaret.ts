...
export function resolveSqlSingleQuoteKeyAction(options: {
  previousChar?: string;
  nextChar?: string;
  autoCloseBrackets?: boolean;
}): SqlSingleQuoteKeyAction {
...
export function caretPositionInsideInsertedSqlSingleQuotes(
  options: SqlSingleQuoteCaretOptions,
): number | null {
  const { previousValue, nextValue, selectionStart } = options;
  ...
  const singleQuoteInsertPrevious =
  ...
  const pairInsertPrevious =
    nextValue.slice(0, selectionStart - 2) + nextValue.slice(selectionStart);

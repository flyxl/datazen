...
export type SqlObjectNavigationType =
  | 'table'
  | 'view'
  | 'materialized_view'
  | 'procedure'
...
export function sqlObjectNavigationTarget(
  table: SqlObjectNavigationTarget,
          ...
          : table.type === 'function'
            ...
            : table.type === 'package'
              ...
              : table.type === 'trigger'
                ...
                : 'table';
  if (table.parentName) {
    const owner = table.parentSchema
      ? `${table.parentSchema}.${table.parentName}`
      : table.parentName;
...
export function sqlObjectNavigationTableType(
  table: SqlObjectNavigationTarget,
 ...
 */
export function sqlObjectNavigationSourceKind(
  table: SqlObjectNavigationTarget,
): ObjectSourceKind | undefined {
...
export function sqlObjectNavigationSourceSchema(
  table: SqlObjectNavigationTarget,
...
export function sqlObjectNavigationTypeFromTableType(
  tableType: string | null | undefined,
): SqlObjectNavigationType {
...
export function sqlObjectNavigationTypeFromCompletionObjectType(
  type: string | null | undefined,
): SqlObjectNavigationType | undefined {
  switch (type) {
    case 'procedure':
      return 'procedure';
    case 'function':
...
export function mergeSqlObjectNavigationType(
  left?: SqlObjectNavigationType,
  right?: SqlObjectNavigationType,
): SqlObjectNavigationType | undefined {
...
function parseQualifiedIdentifier(
  text: string,
  start: number,
): { parts: IdentifierPart[]; start: number; end: number } | null {
...
export function extractQualifiedIdentifierAt(
  doc: string,
  pos: number,
): { parts: ExtractedSqlIdentifierPart[]; start: number; end: number } | null {
...
export function extractIdentifierDetailsAt(
  doc: string,
  pos: number,
): ExtractedSqlIdentifier | null {
 ...
 */
export function resolveSqlObjectNavigationIdentity(
  doc: string,
  pos: number,
): SqlObjectNavigationIdentity | null {
  const located = extractQualifiedIdentifierAt(doc, pos);
  ...
  const relationColumnList =
    followedByParen && isSqlRelationColumnListContext(doc.slice(0, located.start));
...
export function sqlObjectNavigationTargetFromIdentity(
  ...
  const type: SqlObjectNavigationType =
    preferType === 'function' ? 'function' : preferType === 'package' ? 'package' : 'procedure';
  ...
  if (identity.parts.length === 2 && identity.qualifier) {
    const asPackageMember =
      options?.asPackageMember ?? (!options?.asSchemaRoutine && identity.twoPartAmbiguous);
    if (asPackageMember) {
      return sqlObjectNavigationTarget({
        name: identity.name,
        nameQuoted: identity.nameQuoted,
...
export function sqlObjectNavigationTargetFromIdentifier(
  ...
  const identity = resolveSqlObjectNavigationIdentity(
    syntheticDoc,
    Math.max(0, syntheticDoc.length - 1),
  );
...
export function normalizeOracleNavigationTarget(
  ...
  return sqlObjectNavigationTarget({
    ...
    ...(target.schema != null
      ? { schema: normalizeOracleNavigationIdentityName(target.schema, target.schemaQuoted) }
      : {}),
    ...(target.parentName != null
      ? {
          parentName: normalizeOracleNavigationIdentityName(
            target.parentName,
            target.parentNameQuoted,
          ),
        }
      : {}),
    ...(target.parentSchema != null
      ? {
          parentSchema: normalizeOracleNavigationIdentityName(
            target.parentSchema,
            target.parentSchemaQuoted,
          ),
        }
      : {}),
  });
...
export function matchTable<T extends { name: string; database?: string; schema?: string }>(
  identifier: string,
  tables: T[],
): T | null {
  const parts = splitQualifiedIdentifier(identifier);
  const normalizedIdentifier =
    parts.length > 0 ? parts.join('.').toLowerCase() : identifier.toLowerCase();
  ...
  if (parts.length >= 2) {
    ...
    const qualified = tables.find(
      (t) =>
        t.name.toLowerCase() === name &&
        t.schema?.toLowerCase() === qualifier &&
        (!database || !t.database || t.database.toLowerCase() === database),
    );
 ...
 * - `schema.package.proc`
 */
export function matchSqlObject<T extends MatchableSqlObject>(
  identifier: string,
  objects: readonly T[],
): T | null {
  const parts = splitQualifiedIdentifier(identifier);
  ...
  if (parts.length === 1) {
    ...
    return (
      objects.find((object) => object.name.toLowerCase() === name && !object.parentName) ??
      objects.find((object) => object.name.toLowerCase() === name) ??
      null
    );
  }
  ...
  if (parts.length === 2) {
    ...
    const bySchema =
      objects.find(
        (object) =>
          ...
          !object.parentName &&
          object.schema?.toLowerCase() === qualifier,
      ) ??
      objects.find(
        (object) =>
          object.name.toLowerCase() === name &&
    ...
    // package.member
    const byPackage = objects.find(
      (object) =>
        object.name.toLowerCase() === name && object.parentName?.toLowerCase() === qualifier,
    );
  ...
  const packageMember = objects.find(
    (object) =>
      object.name.toLowerCase() === name &&
      object.parentName?.toLowerCase() === middle &&
      same(object.parentSchema || object.schema, owner),
  );
  ...
  return (
    objects.find(
      (object) =>
        object.name.toLowerCase() === name &&
        !object.parentName &&
        object.schema?.toLowerCase() === middle,
    ) ?? null
  );
}

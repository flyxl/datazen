...
export function acceptSelectedOrFirstCompletion(
  view: EditorView,
  acceptCompletion: Command | null,
  selectedCompletionIndex: SelectedCompletionIndex | null,
  selectFirstCompletion: Command | null,
...
export function acceptSelectedCompletionWithRetry(
  view: EditorView,
  options: RetryCompletionAcceptanceOptions,
): CompletionAcceptanceAttempt {
  if (options.completionStatus(view.state) !== 'active') return { handled: false };
  if (
    acceptSelectedOrFirstCompletion(
      view,
      options.acceptCompletion,
      options.selectedCompletionIndex,
      options.selectFirstCompletion,
    )
  )
    return { handled: true };

  const initialDoc = view.state.doc;
  const initialSelectionRanges = view.state.selection.ranges.map((range) => ({
    anchor: range.anchor,
    head: range.head,
  }));
  ...
  const retry = () => {
    ...
    if (
      view.state.doc !== initialDoc ||
      selectionRanges.length !== initialSelectionRanges.length ||
      selectionRanges.some(
        (range, index) =>
          range.anchor !== initialSelectionRanges[index]?.anchor ||
          range.head !== initialSelectionRanges[index]?.head,
      )
    ) {
      settle();
      return;

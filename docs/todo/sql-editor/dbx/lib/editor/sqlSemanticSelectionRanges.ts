...
import {
  createSemanticSelectionRangeIndex,
  type SemanticSelectionContext,
  type SemanticSelectionRange,
  type SemanticSelectionRangeIndex,
} from '@/lib/editor/semanticSelectionRanges';
...
const QUERY_CLAUSE_WORDS = new Set([
  'where',
  'having',
  'on',
  'using',
...
function normalizeTokens(input: string, tokens: SqlSemanticToken[]): SqlSelectionToken[] {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (
      ...
      const triple =
        nextNext?.kind === 'operator' && next.span.end === nextNext.span.start
          ? `${combined}${nextNext.text}`
          : '';
    ...
    result.push({
      kind: token.kind,
      text: input.slice(token.span.start, token.span.end),
...
function statementWindows(input: string, tokens: SqlSelectionToken[]): StatementWindow[] {
  ...
  const boundaries = [
    -1,
    ...significant
      .map((token, index) => (token.text === ';' && token.depth === 0 ? index : -1))
  ...
  for (let boundaryIndex = 0; boundaryIndex < boundaries.length - 1; boundaryIndex += 1) {
    ...
    statements.push({
      ...
      tokens: significant.slice(left + 1, last?.text === ';' ? right - 1 : right),
    });
  }
  return statements;
}

function parenthesisMaps(
  tokens: SqlSelectionToken[],
): { opening: Map<number, number>; closing: Map<number, number> } | null {
  const stack: number[] = [];
...
function clauseStart(tokens: SqlSelectionToken[], index: number): number {
  ...
  if (
    (token.normalized === 'group' || token.normalized === 'order') &&
...
function queryBlockEnd(
  ...
  for (let index = startIndex + 1; index < tokens.length; index += 1) {
    ...
    if (!token) continue;
    if (token.text === ')' && token.depth < depth) return token.from;
    if (token.depth === depth && token.kind === 'word' && SET_OPERATOR_WORDS.has(token.normalized))
      return token.from;
  }
  return statementEnd;
}

function nextClauseOrSetBoundary(
  tokens: SqlSelectionToken[],
  startIndex: number,
  statementEnd: number,
): number {
  ...
  for (let index = startIndex + 1; index < tokens.length; index += 1) {
    ...
    if (
      isClauseAt(tokens, index) ||
      (token.kind === 'word' && SET_OPERATOR_WORDS.has(token.normalized))
    )
      return token.from;
  }
...
function isExpressionBoundary(token: SqlSelectionToken | undefined): boolean {
  ...
  return (
    token.normalized === 'select' ||
    token.normalized === 'from' ||
    token.normalized === 'join' ||
    token.normalized === 'group' ||
    token.normalized === 'order' ||
    token.normalized === 'as' ||
    SET_OPERATOR_WORDS.has(token.normalized) ||
    isClauseAt([token], 0)
  );
}
...
function rangeForDirectTokens(
  tokens: SqlSelectionToken[],
  direct: number[],
  from: number,
  to: number,
  maps: { opening: Map<number, number>; closing: Map<number, number> },
  ...
  return {
    from: first.from,
    to: closingIndex == null ? last.to : (tokens[closingIndex]?.to ?? last.to),
  };
}
...
function operandEndForOperator(
  tokens: SqlSelectionToken[],
  direct: number[],
  operatorPosition: number,
): number {
  ...
  return previous?.normalized === 'not' &&
    (operator?.normalized === 'in' ||
      operator?.normalized === 'like' ||
      operator?.normalized === 'between')
    ? operatorPosition - 1
    : operatorPosition;
}

function collectExpressionSection(
  tokens: SqlSelectionToken[],
  direct: number[],
  maps: { opening: Map<number, number>; closing: Map<number, number> },
  ranges: SemanticSelectionRange[],
  ...
  for (let position = 0; position < direct.length; position += 1) {
    ...
    if (
      !token ||
      SQL_BINARY_PRECEDENCE[token.normalized] == null ||
      isUnarySign(tokens, direct, position)
    )
      continue;
    if (token.normalized !== 'and' || !isBetweenAnd(tokens, direct, position))
      operatorPositions.push(position);
  }
  ...
  for (const operatorPosition of operatorPositions) {
    const operand = rangeForDirectTokens(
      tokens,
      direct,
      start,
      operandEndForOperator(tokens, direct, operatorPosition),
      maps,
    );
    if (!operand) return;
  ...
  for (let index = 0; index < operands.length; index += 1) {
    ...
    const previousOperator =
      previousOperatorPosition == null ? undefined : tokens[direct[previousOperatorPosition] ?? -1];
  ...
  for (let index = 0; index < operatorPositions.length; index += 1) {
    ...
    while (
      (SQL_BINARY_PRECEDENCE[operatorStack[operatorStack.length - 1]?.normalized ?? ''] ?? -1) >=
      precedence
    ) {
      if (!reduce()) return;
    }
...
function collectSqlRanges(
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (isClauseAt(tokens, index))
      addTrimmedRange(
        input,
        ranges,
        clauseStart(tokens, index),
        nextClauseOrSetBoundary(tokens, index, statement.end),
      );
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (token?.kind === 'word' && SET_OPERATOR_WORDS.has(token.normalized)) {
      const left = [...queryBlocks, ...setExpressions]
        .filter((range) => range.depth === token.depth && range.to <= token.from)
        .sort((a, b) => b.to - a.to || a.from - b.from)[0];
      const right = queryBlocks
        .filter((range) => range.depth === token.depth && range.from >= token.to)
...
export function analyzeSqlSemanticSelectionRanges(
  doc: string,
  options: SqlSemanticSelectionOptions = {},
): SqlSemanticSelectionAnalysis {
  const dialect = sqlSemanticDialectFor(options).id;
  const rawTokens = tokenizeSqlSemantic(doc, dialect);
  const validationTokens = rawTokens.map((token) => ({
    kind: token.kind,
    text: token.text,
    normalized: token.normalized,
    from: token.span.start,
  ...
  const statements: PreparedStatementWindow[] = statementWindows(doc, tokens).filter(
    (statement) => {
      const statementValidationTokens = validationTokens.filter(
        (token) => token.from >= statement.start && token.to <= statement.end,
      );
      return validateTokens(doc.slice(statement.start, statement.end), statementValidationTokens);
    },
  );
  const stringContents = statements.flatMap((statement) =>
    statement.tokens
      .map(stringContentRange)
      .filter((range): range is SemanticSelectionRange => range != null),
  );
  return { doc, statements, stringContents };
}

export function sqlStringContentRangeAt(
  doc: string,
  position: number,
  assoc: -1 | 1,
  options: SqlSemanticSelectionOptions = {},
  analysis = analyzeSqlSemanticSelectionRanges(doc, options),
): SemanticSelectionRange | null {
  ...
  while (low < high) {
    ...
    if ((analysis.stringContents[middle]?.to ?? Number.POSITIVE_INFINITY) <= character)
      low = middle + 1;
    else high = middle;
  }
...
export function trimSqlStringWildcardBoundaries(
  doc: string,
  range: SemanticSelectionRange,
): SemanticSelectionRange {
  let from = range.from;
  let to = range.to;
...
function findStatement(
  context: SemanticSelectionContext,
  analysis: SqlSemanticSelectionAnalysis,
): PreparedStatementWindow | undefined {
  let low = 0;
  let high = analysis.statements.length;
  while (low < high) {
    const middle = low + Math.floor((high - low) / 2);
    if ((analysis.statements[middle]?.start ?? Number.POSITIVE_INFINITY) <= context.cursor)
      low = middle + 1;
    else high = middle;
  }
  const statement = analysis.statements[low - 1];
  if (!statement || context.cursor < statement.start || context.cursor > statement.end)
    return undefined;
  if (context.current.from < statement.start || context.current.to > statement.end)
    return undefined;
  return statement;
}

function statementRangeIndex(
  doc: string,
  statement: PreparedStatementWindow,
): SemanticSelectionRangeIndex | null {
...
export function sqlSemanticSelectionRanges(
  context: SemanticSelectionContext,
  options: SqlSemanticSelectionOptions = {},
  analysis = analyzeSqlSemanticSelectionRanges(context.doc, options),
): SemanticSelectionRange[] {
  if (analysis.doc !== context.doc)
    analysis = analyzeSqlSemanticSelectionRanges(context.doc, options);
  const statement = findStatement(context, analysis);
  return statement
    ? (statementRangeIndex(context.doc, statement)?.containing(context.current) ?? [])
    : [];
}

export function nextSqlSemanticSelectionRange(
  context: SemanticSelectionContext,
  options: SqlSemanticSelectionOptions = {},
  analysis = analyzeSqlSemanticSelectionRanges(context.doc, options),
): SemanticSelectionRange | null {
  ...
  const statement = findStatement(context, analysis);
  return statement
    ? (statementRangeIndex(context.doc, statement)?.findNext(context.current, context.cursor) ??
        null)
    : null;
}

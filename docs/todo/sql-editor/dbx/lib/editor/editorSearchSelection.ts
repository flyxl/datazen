...
export function isSearchAddSelectionModifier(
  event: SearchSelectionModifierEvent,
...
export function findSearchMatch(
  matches: Iterable<EditorSearchMatch>,
  cursor: number,
  direction: EditorSearchSelectionDirection,
  accept: (match: EditorSearchMatch) => boolean = () => true,
...
export function selectionRangesForSearchMatches(
  matches: readonly EditorSearchMatch[],
): SelectionRange[] {
  return matches.map((match) => EditorSelection.range(match.from, match.to));
}

export function appendSearchMatchSelection(
  selection: EditorSelection,
  matches: Iterable<EditorSearchMatch>,
  direction: EditorSearchSelectionDirection,
): EditorSelection | null {
  const target = findSearchMatch(
    matches,
    selection.main.head,
    direction,
    (match) => !isMatchCovered(selection, match),
  );

...
export function shellLineCommentClass(
  state: import('@codemirror/state').EditorState,
  highlightingFor: ShellLineCommentHighlightDeps['highlightingFor'],
): string {
...
export function createShellLineCommentHighlight({
  ViewPlugin,
  Decoration,
  highlightingFor,
  syntaxTree,
}: ShellLineCommentHighlightDeps): Extension {
  function buildDecorations(view: EditorViewType, className: string): DecorationSet {
    ...
    return Decoration.set(
      [...ranges]
        ...
        .map(([from, to]) => decoration.range(from, to)),
    );
  }

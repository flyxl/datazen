...
function sqlServerBatchStartsRoutine(sql: string): boolean {
  ...
  const batch = sql
    .slice(batchStart)
    .replace(/^(?:\s|--[^\r\n]*(?:\r\n|\r|\n|$)|\/\*[\s\S]*?\*\/)+/u, '');
  return /^(?:CREATE(?:\s+OR\s+ALTER)?|ALTER)\s+(?:PROC(?:EDURE)?|FUNCTION|TRIGGER)\b/iu.test(
    batch,
  );
}

export function shouldStartNextSqlStatementAtColumnZero(
  state: EditorState,
  ...
  return (
    previousRange.to === terminator + 1 ||
    trailingStatementDelimiterPosition(withTerminator, previousRange.to) === terminator
  );
}

export function insertQueryEditorNewline(
  view: EditorView,
  fallback: Command | null | undefined,
  databaseType?: DatabaseType,
): boolean {

...
export function sqlHoverScrollbarMetrics(
  clientWidth: number,
  scrollWidth: number,
  scrollLeft: number,
  trackWidth: number,
  ...
  const thumbWidth =
    maxScroll > 0
      ? Math.max(SQL_HOVER_SCROLLBAR_MIN_THUMB_WIDTH, trackWidth * (clientWidth / scrollWidth))
      : trackWidth;
  const maxThumbLeft = Math.max(0, trackWidth - thumbWidth);
  const thumbLeft =
    maxScroll > 0 ? (Math.min(Math.max(scrollLeft, 0), maxScroll) / maxScroll) * maxThumbLeft : 0;
...
function appendSqlHoverHorizontalScrollbar(
  root: HTMLElement,
  sqlContent: HTMLElement,
): SqlHoverLayoutController {
  ...
  const update = () => {
    const metrics = sqlHoverScrollbarMetrics(
      sqlContent.clientWidth,
      sqlContent.scrollWidth,
      sqlContent.scrollLeft,
      track.clientWidth,
    );
  ...
  const scrollFromPointer = (clientX: number) => {
    ...
    const thumbLeft = Math.min(
      Math.max(clientX - trackRect.left - dragOffset, 0),
  ...
  const handleWheel = (event: WheelEvent) => {
    ...
    const nextScrollLeft = Math.min(
      Math.max(sqlContent.scrollLeft + horizontalDelta, 0),
      maxScroll,
    );
...
export function constrainSqlHoverLayout(
  root: HTMLElement,
  sqlContent: HTMLElement,
): SqlHoverLayoutController {

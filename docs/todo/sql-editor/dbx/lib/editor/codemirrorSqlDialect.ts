...
type CodeMirrorSqlLanguageModule = Pick<
  typeof import('@codemirror/lang-sql'),
  'Cassandra' | 'MSSQL' | 'MySQL' | 'PLSQL' | 'PostgreSQL' | 'SQLite' | 'SQLDialect' | 'StandardSQL'
>;

const MYSQL_CODEMIRROR_DATABASE_TYPES = new Set<DatabaseType>([
  'mysql',
  'doris',
  'starrocks',
  'manticoresearch',
...
const POSTGRES_CODEMIRROR_DATABASE_TYPES = new Set<DatabaseType>([
  'postgres',
  'redshift',
  'gaussdb',
  'kwdb',
...
const ORACLE_CODEMIRROR_DATABASE_TYPES = new Set<DatabaseType>([
  'oracle',
  'dameng',
  'yashandb',
  'oscar',
...
const SQLITE_CODEMIRROR_DATABASE_TYPES = new Set<DatabaseType>([
  'sqlite',
  'rqlite',
  'turso',
  'cloudflare-d1',
...
const CODEMIRROR_SQLITE_EXTENSION_KEYWORDS = new Set(
  'abort analyze attach autoincrement conflict database detach exclusive fail glob ignore index indexed instead isnull notnull offset plan pragma query raise regexp reindex rename replace temp vacuum virtual'.split(
    ' ',
  ),
);
...
const POSTGRES_PLPGSQL_BUILTIN =
  'SQLERRM TG_NAME TG_WHEN TG_LEVEL TG_OP TG_RELID TG_RELNAME TG_TABLE_NAME TG_TABLE_SCHEMA TG_NARGS TG_ARGV';
const POSTGRES_IDENTIFIER_LIKE_KEYWORDS = new Set(
  'COMMENT COUNT DATA DAY HOUR ID KEY LEVEL MINUTE MONTH NAME OWNER PASSWORD POSITION ROLE SECOND TYPE USER VALUE YEAR'.split(
    ' ',
  ),
);
...
function codeMirrorBaseDialect(
  langSql: CodeMirrorSqlLanguageModule,
  dialectName: CodeMirrorSqlDialectName,
...
export function createDbxCodeMirrorSqlDialect(
  langSql: CodeMirrorSqlLanguageModule,
  dialectName: CodeMirrorSqlDialectName = 'mysql',
  databaseType?: DatabaseType,
  ...
  const baseKeywords = isClickHouse
    ? standardSqlKeywordSyntaxTerms(langSql)
    : isPostgres
      ? postgresKeywordSyntaxTerms(baseDialect.spec.keywords || '')
      : baseDialect.spec.keywords || '';
  const baseTypes = isClickHouse ? STANDARD_SQL_TYPES : baseDialect.spec.types || '';
  const commonKeywords = isClickHouse
    ? DBX_COMMON_SQL_KEYWORDS.toLowerCase()
    : DBX_COMMON_SQL_KEYWORDS;
  const baseBuiltin = isSqlServer
    ? sqlServerBuiltinSyntaxTerms(baseDialect.spec.builtin || '')
    : baseDialect.spec.builtin || '';
  ...
  return langSql.SQLDialect.define({
    ...
    keywords: [
      ...
      .join(' '),
    types:
      [baseTypes, isClickHouse ? CLICKHOUSE_TYPES : '', isPostgres ? POSTGRES_PLPGSQL_TYPES : '']
        .filter(Boolean)
        .join(' ') || undefined,
    builtin:
      [
        baseBuiltin,
        isClickHouse ? CLICKHOUSE_BUILTINS : '',
        isPostgres ? `${POSTGRES_BUILTINS} ${POSTGRES_PLPGSQL_BUILTIN}` : '',
        isMysql ? MYSQL_BUILTINS : '',
        driverProfileSqlBuiltinTerms(driverProfile),
      ]
        .filter(Boolean)
        .join(' ') || undefined,
    ...(isClickHouse

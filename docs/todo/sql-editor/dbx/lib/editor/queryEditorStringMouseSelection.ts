...
import {
  ...
  sqlStringContentRangeAt,
  trimSqlStringWildcardBoundaries,
  type SqlSemanticSelectionAnalysis,
...
export function createQueryEditorStringMouseSelection(
  view: EditorView,
  event: MouseEvent,
  options: QueryEditorStringMouseSelectionOptions = {},
    ...
    get(currentEvent, extend, multiple) {
      ...
      let range =
        start.pos === current.pos
          ? startRange
          : semanticOrDefaultRange(view.state, current, options, analysis);
      if (start.pos !== current.pos && !extend) {
        const from = Math.min(startRange.from, range.from);
        const to = Math.max(startRange.to, range.to);
        range =
          from < range.from
            ? EditorSelection.range(from, to, undefined, undefined, range.assoc)
            : EditorSelection.range(to, from, undefined, undefined, range.assoc);
      }
      if (extend)

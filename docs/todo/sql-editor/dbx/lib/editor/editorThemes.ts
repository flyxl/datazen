...
export function sqlSemanticHighlightTheme(
...
export function shellLineCommentTheme(
  EditorView: typeof import('@codemirror/view').EditorView,
): Extension {
  return EditorView.theme({
...
const SUPPORTS_OKLCH =
  typeof CSS !== 'undefined' &&
  typeof CSS.supports === 'function' &&
  CSS.supports('color', 'oklch(0.62 0.19 255)');
...
export function resolveCustomThemeBackgrounds(
  colors?: Pick<CustomThemeColors, 'background'>,
  isDark: boolean = true,
): { background: string; gutterBackground: string } {
  return {
...
function createCustomTheme(
  ...
  const defaultColors = {
    ...
    foreground: isDark ? '#cdd6f4' : '#242424',
  };
  ...
  const c = {
    ...
    ...customThemeColors,
    ...colors,
    gutterBackground: backgrounds.gutterBackground,
  };
...
function createIdeEditorTheme(
  EditorView: typeof import('@codemirror/view').EditorView,
  c: IdeEditorThemeColors,
): Extension {
  const theme = EditorView.theme(
    {
      '&': {
  ...
  const highlightStyle = HighlightStyle.define([
    {
      ...
      color: c.keyword,
      ...(c.keywordBold ? { fontWeight: 'bold' } : {}),
    },
    {
      tag: [tags.string, tags.special(tags.string), tags.regexp, tags.escape, tags.inserted],
      color: c.string,
      ...(c.stringBold ? { fontWeight: 'bold' } : {}),
    },
    {
      tag: [tags.number, tags.integer, tags.float],
      color: c.number,
      ...(c.numberBold ? { fontWeight: 'bold' } : {}),
    },
    {
      tag: [tags.comment, tags.lineComment, tags.blockComment, tags.quote],
      color: c.comment,
      fontStyle: 'italic',
    },
...
export function resolveEditorTheme(
  ...
  appPalette: AppThemePalette = 'pearl',
): Exclude<EditorTheme, 'app'> {
  if (theme === 'app') {
    switch (appPalette) {
      case 'vscode':
 ...
 */
export function editorThemeAppearanceFor(
  appAppearance: AppThemeAppearance,
  appPalette: AppThemePalette,
  customUiColors?: AppCustomUiColors,
): AppThemeAppearance {
 ...
 */
export function editorDiagnosticColors(appearance: AppThemeAppearance): {
  error: string;
  warning: string;
} {
  return appearance === 'dark'
    ? { error: '#f87171', warning: '#fbbf24' }
    : { error: '#dc2626', warning: '#b45309' };
}
...
export function sqlCompletionTheme(
  EditorView: typeof import('@codemirror/view').EditorView,
): Extension {

...
function isSqlServerTransactionBegin(
  state: EditorState,
  tokens: SyntaxNode[],
  index: number,
): boolean {
  const next = tokens[index + 1];
  if (!next || !GAP_IS_WHITESPACE_OR_COMMENT.test(state.sliceDoc(tokens[index].to, next.from)))
    return false;
  ...
  return Boolean(
    transaction &&
      GAP_IS_WHITESPACE_OR_COMMENT.test(state.sliceDoc(next.to, transaction.from)) &&
      state.sliceDoc(transaction.from, transaction.to).toUpperCase() === 'TRANSACTION',
  );
}
...
function addMultilineFoldRange(
  state: EditorState,
  ranges: Map<number, FoldRange>,
  openerPosition: number,
  endPosition: number,
  overwrite = false,
) {
  ...
  if (
    openerLine.number === endLine.number ||
    endPosition <= openerLine.to ||
    (!overwrite && ranges.has(openerLine.number))
  )
    return;
...
function isQueryParens(state: EditorState, node: SyntaxNode): boolean {
  for (let child = node.firstChild; child; child = child.nextSibling) {
    if (child.name === '(' || child.name === 'LineComment' || child.name === 'BlockComment')
      continue;
...
export function collectUnionBranchFoldRanges(
  tokens: ReadonlyArray<{ from: number; keyword: 'SELECT' | 'UNION' }>,
  scopeEnd: number,
): FoldRange[] {
...
function addQueryStructureFoldRanges(
  state: EditorState,
  tree: Tree,
  ranges: Map<number, FoldRange>,
) {
...
function addRestRequestFoldRanges(
  state: EditorState,
  ranges: Map<number, FoldRange>,
  databaseType?: DatabaseType,
): boolean {
  const requests = elasticsearchRestRequestRanges(
    state.doc.toString(),
...
function computeBlockFoldRanges(
  state: EditorState,
  databaseType?: DatabaseType,
  ...
  if (isRestDocument) {
    const nextCacheByDatabaseType =
      cacheByDatabaseType ?? new Map<string, Map<number, FoldRange>>();
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (text === 'END') {
      ...
      const continuation =
        next && GAP_IS_WHITESPACE_OR_COMMENT.test(state.sliceDoc(token.to, next.from))
          ? state.sliceDoc(next.from, next.to).toUpperCase()
          : null;
      ...
      BLOCK_OPENERS.has(text) &&
      (text !== 'BEGIN' || !isSqlServerTransactionBegin(state, tokens, i))
    ) {
      stack.push(token);
    }
  }
...
export function createSqlBlockFoldService(databaseType?: DatabaseType) {
  return foldService.of((state, lineStart) => {
    const range = computeBlockFoldRanges(state, databaseType).get(

...
export function scopeHoverTables(
  tables: SqlCompletionTable[],
  scope: HoverTableScope,
): SqlCompletionTable[] {
...
export function hoverTableMatchesScope(table: SqlCompletionTable, scope: HoverTableScope): boolean {
  ...
  if (
    normalizedScopePart(scope.catalog) &&
    normalizedScopePart(table.catalog) !== normalizedScopePart(scope.catalog)
  )
    return false;
  if (
    normalizedScopePart(scope.schema) &&
    normalizedScopePart(table.schema) !== normalizedScopePart(scope.schema)
  )
    return false;
  return true;
}
...
function formatDefaultValue(value: string): string {
  ...
  if (
    /^(current_timestamp|current_date|current_time|localtimestamp|localtime|now|getdate|random|gen_random_uuid|uuid)\s*(?:\(.*\))?$/i.test(
      value,
    )
  )
    return value;
 ...
 */
export function buildHoverTableSql(
  ...
  const rows: ColumnFieldParts[] = columns.map((c) => {
    ...
    const defaultClause =
      c.column_default != null ? `default ${formatDefaultValue(c.column_default)}` : '';
    const extra = c.extra ?? '';
  ...
  const closeSuffix = tableComment?.trim()
  ...
  for (const idx of nonPrimaryIndexes) {
    ...
    indexLines.push(
      `  on ${qualifiedName} (${idx.columns.map((c) => quoteIdentifier(c)).join(', ')});`,
    );
  }
...
const COLUMN_CLAUSE_BOUNDARIES = new Set([
  'not',
  'null',
  'default',
  'auto_increment',
...
const CONSTRAINT_LINE_KEYWORDS = new Set([
  'primary',
  'unique',
  'key',
  'index',
...
function parseConstraintLine(line: string, parsed: ParsedCreateTable): void {
  let rest = line.trim();
  const named = /^constraint\s+(?:`(?:``|[^`])*`|"(?:""|[^"])*"|\[(?:]]|[^\]])*]|\S+)\s+/i.exec(
    rest,
  );
  ...
  if (
    tokenAt(0) === 'unique' &&
    (tokenAt(1) === 'key' || tokenAt(1) === 'index') &&
    groupIdx >= 2 &&
    groupColumns.length > 0
  ) {
    parsed.indexes.push({
      name: unquoteIdentifier(tokens[groupIdx - 1].text),
...
function parseCreateTableStatement(statement: string): ParsedCreateTable | null {
  const header =
    /^\s*create\s+(?:or\s+replace\s+)?(?:global\s+|local\s+|temporary\s+|temp\s+|unlogged\s+|external\s+)*table\s+(?:if\s+not\s+exists\s+)?/i.exec(
      statement,
    );
...
function parseCreateIndexStatement(statement: string): ParsedIndex | null {
  const header =
    /^\s*create\s+(unique\s+)?index\s+(?:concurrently\s+)?(?:if\s+not\s+exists\s+)?/i.exec(
      statement,
    );
  if (!header) return null;
  const rest = statement.slice(header[0].length);
...
function renderParsedCreateTable(
  parsed: ParsedCreateTable,
  qualifiedNameOverride: string | undefined,
  passthroughStatements: string[],
): string {
  ...
  const closeSuffix = parsed.tableComment?.trim()
    ? ` comment '${parsed.tableComment.replace(/'/g, "''")}';`
    : ';';
...
export function reformatHoverDdl(rawDdl: string, qualifiedName?: string): string {
  ...
  const createIdx = statements.findIndex((s) =>
    /^\s*create\s+(?:or\s+replace\s+)?(?:global\s+|local\s+|temporary\s+|temp\s+|unlogged\s+|external\s+)*table\b/i.test(
      s,
    ),
  );
  ...
  const suffixWithoutComment = suffix
    .replace(/^comment\s*=?\s*('(?:\\'|''|[^'])*')\s*$/i, '')
    .trim();
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    const tableCommentMatch = /^comment\s+on\s+table\s+.+?\s+is\s+('(?:\\'|''|[^'])*')\s*$/is.exec(
      statement,
    );
    ...
    const columnCommentMatch =
      /^comment\s+on\s+column\s+(\S+)\s+is\s+('(?:\\'|''|[^'])*')\s*$/is.exec(statement);

...
function isDesktopSafari(currentNavigator: BrowserNavigator | undefined): boolean {
  ...
  return (
    /Apple Computer/.test(currentNavigator.vendor) &&
    !/Mobile\/\w+/.test(currentNavigator.userAgent) &&
    currentNavigator.maxTouchPoints <= 2
  );
}

export function createQueryEditorPostCompositionKeyGuard(
  options: { navigator?: BrowserNavigator; now?: () => number } = {},
) {
  ...
  function handleKeydown(event: KeyboardEvent) {
    ...
    compositionPendingKey = false;
    if (now() - compositionEndedAt < SAFARI_POST_COMPOSITION_KEY_WINDOW_MS)
 ...
 */
export function createQueryEditorExecutionShortcutBindings(
  shortcut: string,
  run: Command,
  isComposing: (view: EditorView) => boolean,
): KeyBinding[] {
  if (!shortcut) return [];
  return [
    {

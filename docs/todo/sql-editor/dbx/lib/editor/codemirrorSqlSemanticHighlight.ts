...
function maskSuppressedSyntax(
  sql: string,
  window: SqlSemanticHighlightWindow,
  tree: SqlSyntaxTree,
): string {
...
export function sqlSemanticTableNameSpansForSyntaxTree(
  sql: string,
  window: SqlSemanticHighlightWindow,
  tree: SqlSyntaxTree,
  ...
  return sqlSemanticTableNameSpans(maskedSql, options).map((span) => ({
    start: span.start + window.from,
    end: span.end + window.from,
  }));
}

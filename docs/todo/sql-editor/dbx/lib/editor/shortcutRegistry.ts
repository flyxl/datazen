import {
  ...
  parseShortcutStrokes,
  shortcutDisplayParts,
} from '@/lib/editor/shortcutDisplay';
...
// macOS 的 ⌥⌘W 无上述问题
export function closeOtherTabsDefaultShortcut(
...
export function selectionOccurrenceDefaultShortcut(
  actionId: 'addNextSelectionOccurrence' | 'selectAllSelectionOccurrences',
  platform = globalThis.navigator?.platform || '',
): string {
...
export function tabNavigationHistoryDefaultShortcut(
...
const TAB_NAVIGATION_HISTORY_ACTIONS: ShortcutActionId[] = [
  'navigateTabHistoryBack',
  'navigateTabHistoryForward',
];
...
function hasExplicitShortcut(
  settings: Partial<ShortcutSettings> | undefined,
  actionId: ShortcutActionId,
): boolean {
  return (
    !!settings &&
    Object.prototype.hasOwnProperty.call(settings, actionId) &&
    typeof settings[actionId] === 'string'
  );
}

function shortcutsUseSameKeys(
  first: string,
  second: string,
  platform = globalThis.navigator?.platform || '',
): boolean {
  return (
    !!first &&
    !!second &&
    formatShortcut(first, platform).toLowerCase() === formatShortcut(second, platform).toLowerCase()
  );
}

function shortcutDefaultForPlatform(definition: ShortcutDefinition, platform: string): string {
  if (definition.id === 'addNextSelectionOccurrence')
    return selectionOccurrenceDefaultShortcut('addNextSelectionOccurrence', platform);
  if (definition.id === 'selectAllSelectionOccurrences')
    return selectionOccurrenceDefaultShortcut('selectAllSelectionOccurrences', platform);
  if (definition.id === 'closeOtherTabs') return closeOtherTabsDefaultShortcut(platform);
  if (definition.id === 'navigateTabHistoryBack')
    return tabNavigationHistoryDefaultShortcut('back', platform);
  if (definition.id === 'navigateTabHistoryForward')
    return tabNavigationHistoryDefaultShortcut('forward', platform);
  return definition.defaultShortcut;
}

export function needsTabNavigationHistoryShortcutMigration(
  settings?: Partial<ShortcutSettings>,
): boolean {
  return (
    !!settings &&
    TAB_NAVIGATION_HISTORY_ACTIONS.some((actionId) => !hasExplicitShortcut(settings, actionId))
  );
}
...
export function normalizeShortcutSettings(
  ...
  const normalized = Object.fromEntries(
    SHORTCUT_DEFINITIONS.map((definition) => {
      ...
      const normalized =
        definition.inputKind === 'modifier-only'
          ? normalizeModifierOnlyShortcut(configured, definition.defaultShortcut)
          : configured;
  ...
  if (
    settings?.copyCurrentRow === '' &&
    settings?.editTableStructure === LEGACY_COPY_CURRENT_ROW_DEFAULT
  ) {
  ...
  for (const actionId of TAB_NAVIGATION_HISTORY_ACTIONS) {
    ...
    const occupiedByExistingAction = SHORTCUT_DEFINITIONS.some(
      (item) =>
        item.id !== actionId &&
        item.scope === definition.scope &&
        hasExplicitShortcut(settings, item.id) &&
        shortcutsUseSameKeys(normalized[item.id], defaultShortcut, platform),
    );
...
export function findShortcutConflict(
  ...
  const conflict = SHORTCUT_DEFINITIONS.find(
    (item) =>
      item.id !== actionId &&
      item.scope === definition.scope &&
      shortcutsUseSameKeys(shortcuts[item.id], shortcut, platform),
  );

...
export function batchColumnSelectionColumnList(
  candidates: string[],
  mode: BatchColumnSelectionMode,
  qualifier?: string,
): string {
  return candidates
    .map((candidate, index) =>
      mode === 'select' && qualifier && index > 0 ? `${qualifier}.${candidate}` : candidate,
    )
    .join(', ');
}

export function shouldResolveSqlColumnCompletion(options: {
  suggestColumns: boolean;
  hasReferencedTables: boolean;
  prefix: string;
  typedActivation: boolean;
  selectListColumnContext: boolean;
}): boolean {
  return (
    options.suggestColumns &&
    options.hasReferencedTables &&
    (options.prefix.length > 0 || options.typedActivation || options.selectListColumnContext)
  );
}
 ...
 */
export function batchColumnSelectionReplaceTo(options: {
  to: number;
  mode: BatchColumnSelectionMode;
  ...
  return replaceClosingQuote === nextCharacter || (mode === 'insert' && nextCharacter === ')')
    ? to + 1
    : to;
}

export function batchColumnSelectionInsertReplacement(options: {
  document: string;
  to: number;
  columns: string;
  valuesKeyword: 'values' | 'VALUES';
  valueCount: number;
}): { replaceTo: number; insert: string } {

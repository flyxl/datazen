...
export function createQueryEditorSearchKeymap(
  options: QueryEditorSearchKeymapOptions,
): KeyBinding[] {
  return [
    {
      key: 'Mod-f',
...
export function createQueryEditorReplaceShortcutBindings(
  shortcut: string,
  openReplace: () => boolean,
): KeyBinding[] {
  const normalizedShortcut = shortcut.trim();
  return normalizedShortcut && /\s/.test(normalizedShortcut)
    ? [{ key: shortcutToCodeMirrorKey(normalizedShortcut), preventDefault: true, run: openReplace }]
    : [];
}
 ...
 */
export function createQueryEditorReplaceShortcutHandler(options: {
  shortcut: string;
  openReplace: () => boolean;
  isReadOnly: () => boolean;

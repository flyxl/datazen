...
function hasContainingQuote(analysis: PlainTextSelectionAnalysis, cursor: number): boolean {
  return analysis.quotedRanges
    .containing({ from: cursor, to: cursor })
    .some((range) => range.from < cursor && cursor < range.to);
}

function selectionRangeIndex(
  context: SemanticSelectionContext,
  analysis: PlainTextSelectionAnalysis,
): SemanticSelectionRangeIndex {
  return context.preferDelimitedContent && hasContainingQuote(analysis, context.cursor)
    ? analysis.delimitedRanges
    : analysis.allRanges;
}

export function plainTextSelectionRanges(
  context: SemanticSelectionContext,
  analysis = analyzePlainTextSelectionRanges(context.doc),
...
export function nextPlainTextSelectionRange(
  context: SemanticSelectionContext,
  analysis = analyzePlainTextSelectionRanges(context.doc),

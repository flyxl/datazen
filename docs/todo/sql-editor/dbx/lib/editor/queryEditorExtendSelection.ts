...
import {
  analyzePlainTextSelectionRanges,
  nextPlainTextSelectionRange,
} from '@/lib/editor/plainTextSelectionRanges';
import {
  chooseNextSemanticSelectionRange,
  type SemanticSelectionRange,
} from '@/lib/editor/semanticSelectionRanges';
import {
  analyzeSqlSemanticSelectionRanges,
  nextSqlSemanticSelectionRange,
  type SqlSemanticSelectionOptions,
} from '@/lib/editor/sqlSemanticSelectionRanges';

export interface QueryEditorExtendSelectionOptions extends SqlSemanticSelectionOptions {
  language?: 'sql' | 'text';
}

export function extendQueryEditorSelection(
  view: EditorView,
  options: QueryEditorExtendSelectionOptions = {},
): boolean {
  const doc = view.state.doc.toString();
  const plainTextAnalysis = analyzePlainTextSelectionRanges(doc);
  const sqlAnalysis =
    options.language === 'text' ? null : analyzeSqlSemanticSelectionRanges(doc, options);
  let changed = false;
  const ranges = view.state.selection.ranges.map((range) => {
    ...
    const sqlCandidate = sqlAnalysis
      ? nextSqlSemanticSelectionRange(context, options, sqlAnalysis)
      : null;
    ...
    return range.anchor <= range.head
      ...
      : EditorSelection.range(next.to, next.from);
  });
...
export function runQueryEditorAltExtendSelection(
  event: PreventableShortcutEvent,
  shortcut: string,
  view: EditorView,

...
export function trimmedSelectionHorizontalBounds(options: {
  ...
  const lineBreakOnlyAtContentEnd =
    options.from >= options.to &&
    options.includesLineBreak &&
    options.from >= options.lineTo &&
    options.lineTo > options.lineFrom;
  const left =
    options.from < options.to
      ? Math.min(options.startLeft, options.endLeft)
      : lineBreakOnlyAtContentEnd
        ? options.lineEndRight
        : options.lineStartLeft;
  const emptySelectionWidth = Math.max(
    1,
    options.emptySelectionWidth ?? FALLBACK_EMPTY_SELECTION_WIDTH,
  );
  let right =
    options.from < options.to
      ? Math.max(options.startRight, options.endRight)
      : left + emptySelectionWidth;
...
function markerClass(rects: TrimmedSelectionRect[], index: number): string {
  ...
  const touchesPrev =
    !!prev && Math.abs(prev.top + prev.height - current.top) <= CONTIGUOUS_LINE_GAP;
  const touchesNext =
    !!next && Math.abs(current.top + current.height - next.top) <= CONTIGUOUS_LINE_GAP;

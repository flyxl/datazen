...
import {
  applyHoverSearchHighlights,
  clearHoverSearchHighlights,
  createHoverSearch,
  findHoverSearchMatches,
} from '@/lib/editor/sqlHoverSearch';

const DDL = [
  'create table `orders` (',
  '    `id`                    bigint      not null,',
  '    `customer_order_status` varchar(32) null,',
  '    `USER_ID`               bigint      null,',
...
describe('createHoverSearch', () => {
  function setup() {
    ...
    const input = controller.element.querySelector<HTMLInputElement>(
      '[data-sql-hover-search-input="true"]',
    )!;

...
import {
  buildQueryEditorLineNumbersExtension,
  isWrappedLineNumberGutter,
  setQueryEditorLineNumberAlignment,
} from '@/lib/editor/queryEditorLineNumbers';
...
function createEditor(showLineNumbers: boolean) {
  ...
  const selectLine = vi.fn(
    (_view: EditorView, _line: { from: number; to: number }, _event: Event) => true,
  );
  ...
  it('reconfigures line numbers without removing the statement run gutter', () => {
    ...
    view.dispatch({
      effects: lineNumbersCompartment.reconfigure(
        buildQueryEditorLineNumbersExtension(lineNumbers, false, { domEventHandlers: {} }),
      ),
    });
    ...
    view.dispatch({
      effects: lineNumbersCompartment.reconfigure(
        buildQueryEditorLineNumbersExtension(lineNumbers, true, { domEventHandlers: {} }),
      ),
    });

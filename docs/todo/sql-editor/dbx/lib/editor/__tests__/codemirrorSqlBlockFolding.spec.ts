...
import {
  collectUnionBranchFoldRanges,
  createSqlBlockFoldService,
  sqlBlockFoldService,
} from '@/lib/editor/codemirrorSqlBlockFolding';

function stateFor(
  doc: string,
  dialectName: 'mysql' | 'postgres' | 'sqlserver' = 'mysql',
  folding = sqlBlockFoldService,
): EditorState {
  const state = EditorState.create({
    doc,
    extensions: [
      langSql.sql({ dialect: createDbxCodeMirrorSqlDialect(langSql, dialectName) }),
      folding,
    ],
  });
...
describe('sqlBlockFoldService', () => {
  it.each(['elasticsearch', 'easysearch', 'meilisearch'] as const)(
    'folds one %s REST request without a semicolon',
    (databaseType) => {
...
END`;
  ...
  it("regression: END IF inside a BEGIN block must not be mistaken for BEGIN's own closer", () => {
    ...
    expect(
      state.sliceDoc(0, foldable(state, state.doc.line(1).from, state.doc.line(1).to)!.to),
    ).toBe('BEGIN\n  IF 1 = 1 THEN\n    SELECT 1;\n  END IF;\n');
  });
  ...
  it.each(['BEGIN TRAN', 'begin transaction', 'BeGiN DiStRiBuTeD TrAnSaCtIoN'])(
    'does not treat SQL Server %s as a block opener inside BEGIN...END',
    (transactionBegin) => {
      const sql = `BEGIN
  ${transactionBegin};
  SELECT CASE
...
FROM manual_fees;`;
  ...
  it('precomputes UNION branch boundaries with one token-position read per token', () => {
    ...
    let positionReads = 0;
    const tokens: Array<{ readonly from: number; readonly keyword: 'SELECT' | 'UNION' }> =
      Array.from({ length: branchCount * 2 - 1 }, (_, index) => ({
...
SELECT * FROM (SELECT 1 AS value) one_line;`;
  ...
  it('still folds correctly after a dialect reconfigure with the document unchanged', () => {
    ...
    let state = EditorState.create({
      doc: sql,
      extensions: [
        languageComp.of(langSql.sql({ dialect: createDbxCodeMirrorSqlDialect(langSql, 'mysql') })),
        sqlBlockFoldService,
      ],
    });
    ...
    state = state.update({
      effects: languageComp.reconfigure(
        langSql.sql({ dialect: createDbxCodeMirrorSqlDialect(langSql, 'sqlserver') }),
      ),
    }).state;

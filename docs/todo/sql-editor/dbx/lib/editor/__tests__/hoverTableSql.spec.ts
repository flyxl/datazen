...
import {
  buildHoverTableSql,
  ddlForHoverPreview,
  hoverTableMatchesScope,
  normalizeAlignedSqlWhitespace,
  reformatHoverDdl,
  sanitizeHoverDdl,
  scopeHoverTables,
} from '@/lib/editor/hoverTableSql';
...
describe('buildHoverTableSql', () => {
  it('single column, no PK, no indexes', () => {
    const sql = buildHoverTableSql(
      '"mydb"."public"."users"',
      [col({ name: 'email', data_type: 'text', is_nullable: false })],
      [],
    );
  ...
  it('single column with PK', () => {
    const sql = buildHoverTableSql(
      '"mydb"."public"."users"',
      [col({ name: 'id', data_type: 'integer', is_nullable: false, is_primary_key: true })],
      [idx({ name: 'pk_users', is_primary: true, columns: ['id'] })],
    );
    expect(sql).toMatchInlineSnapshot(`
      "create table "mydb"."public"."users" (
          "id" integer not null,
  ...
  it('composite primary key', () => {
    const sql = buildHoverTableSql(
      '"public"."order_items"',
      [
        col({ name: 'order_id', data_type: 'bigint', is_nullable: false }),
        col({ name: 'line_item', data_type: 'integer', is_nullable: false }),
        col({ name: 'sku', data_type: 'varchar(50)' }),
      ],
  ...
  it('no primary key (multi-column)', () => {
    const sql = buildHoverTableSql(
      '"public"."audit_log"',
      [
        col({ name: 'event', data_type: 'text', is_nullable: false }),
        col({ name: 'created_at', data_type: 'timestamptz', column_default: 'now()' }),
      ],
      [],
    );
    expect(sql).toMatchInlineSnapshot(`
  ...
  it('SQL Server varchar(max) and datetime2(7) — trust backend data_type', () => {
    const sql = buildHoverTableSql(
      '"dbo"."orders"',
      [
        col({ name: 'notes', data_type: 'varchar(max)', is_nullable: true }),
        col({ name: 'created_at', data_type: 'datetime2(7)', column_default: 'getdate()' }),
        col({ name: 'status', data_type: 'varchar(20)', is_nullable: false }),
      ],
  ...
  it('trusts backend parameterized data_type over decomposed fields', () => {
    ...
    const sql = buildHoverTableSql(
      '"public"."t"',
      [col({ name: 'name', data_type: 'character varying(255)', character_maximum_length: 255 })],
      [],
    );
  ...
  it('SQL Server (max) from character_maximum_length=-1', () => {
    const sql = buildHoverTableSql(
      '"dbo"."t"',
      [col({ name: 'payload', data_type: 'nvarchar', character_maximum_length: -1 })],
      [],
    );
  ...
  it('includes table comment', () => {
    const sql = buildHoverTableSql(
      '"public"."users"',
      [col({ name: 'id', data_type: 'serial', is_nullable: false })],
      [idx({ name: 'pk_users', is_primary: true, columns: ['id'] })],
      'User accounts',
    );
    expect(sql).toMatchInlineSnapshot(`
  ...
  it('extra column attributes (auto_increment)', () => {
    const sql = buildHoverTableSql(
      '"public"."t"',
      [col({ name: 'id', data_type: 'int', is_nullable: false, extra: 'auto_increment' })],
      [idx({ name: 'pk_t', is_primary: true, columns: ['id'] })],
    );
  ...
  it('non-primary indexes are emitted after DDL', () => {
    const sql = buildHoverTableSql(
      '"public"."t"',
      [
        col({ name: 'a', data_type: 'int' }),
        col({ name: 'b', data_type: 'int' }),
        col({ name: 'c', data_type: 'int' }),
      ],
      [
        idx({ name: 'ix_t_b', columns: ['b'] }),
        idx({ name: 'ix_t_c', is_unique: true, columns: ['c'] }),
      ],
    );
    expect(sql).toMatchInlineSnapshot(`
  ...
  it('column-level PK fallback when no primary index is present', () => {
    const sql = buildHoverTableSql(
      '"public"."t"',
      [
        col({ name: 'id', data_type: 'int', is_nullable: false, is_primary_key: true }),
        col({ name: 'label', data_type: 'text' }),
      ],
      [],
    );
    expect(sql).toMatchInlineSnapshot(`
...
describe('hover table scope', () => {
  it('annotates loaded tables with their catalog, database, and schema', () => {
    expect(
      scopeHoverTables([{ name: 'orders' }], {
        catalog: 'hive',
        database: 'sales',
        schema: 'public',
      }),
    ).toEqual([{ name: 'orders', catalog: 'hive', database: 'sales', schema: 'public' }]);
  });
  ...
  it('rejects a bare-name cache hit from another database or schema', () => {
    ...
    expect(
      hoverTableMatchesScope(
        { name: 'orders', catalog: 'hive', database: 'archive', schema: 'public' },
        target,
      ),
    ).toBe(false);
    expect(
      hoverTableMatchesScope(
        { name: 'orders', catalog: 'hive', database: 'sales', schema: 'audit' },
        target,
      ),
    ).toBe(false);
    expect(
      hoverTableMatchesScope(
        { name: 'orders', catalog: 'iceberg', database: 'sales', schema: 'public' },
        target,
      ),
    ).toBe(false);
    expect(
      hoverTableMatchesScope(
        { name: 'orders', catalog: 'hive', database: 'sales', schema: 'public' },
        target,
      ),
    ).toBe(true);
  });
  ...
  it('does not trust an unscoped cached table', () => {
    expect(
      hoverTableMatchesScope({ name: 'orders' }, { database: 'sales', schema: 'public' }),
    ).toBe(false);
  });
});
...
describe('normalizeAlignedSqlWhitespace', () => {
  it('collapses column-alignment padding to single spaces', () => {
    const aligned =
      '    "id"         integer default nextval(\'x\'::regclass) not null,\n    "username"   character varying(255) not null,';
    expect(normalizeAlignedSqlWhitespace(aligned)).toBe(
      '    "id" integer default nextval(\'x\'::regclass) not null,\n    "username" character varying(255) not null,',
    );
  });

...
const globalShortcutDefinitions = SHORTCUT_DEFINITIONS.filter(
  (definition) => definition.scope === 'global',
);

export function defaultKeymapForGlobalShortcuts(
  ...
  const occupiedGlobalKeys = new Set(
    globalShortcutDefinitions
      .map((definition) => shortcutToCodeMirrorKey(normalizedShortcuts[definition.id]))
      .filter(Boolean),
  );

  return defaultKeymap.filter(
    (binding) =>
      !binding.key || !indentBracketKeys.has(binding.key) || !occupiedGlobalKeys.has(binding.key),
  );
}

 ...
 */
export function applyHoverSearchHighlights(
  container: HTMLElement,
  matches: HoverSearchMatch[],
): HTMLElement[] {
  ...
  for (const { node, start, end } of textNodes) {
    ...
    const segments = matches
      ...
      .map((match) => ({
        from: Math.max(match.start, start) - start,
        to: Math.min(match.end, end) - start,
      }));
...
export function createHoverSearch(options: HoverSearchOptions): HoverSearchController {
  ...
  input.className =
    'w-full rounded border border-border/60 bg-background px-2 py-1 text-[11px] leading-none text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring';

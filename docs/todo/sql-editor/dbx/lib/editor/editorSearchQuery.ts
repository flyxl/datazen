...
export function countEditorSearchMatches(
  query: SearchQuery,
  state: EditorState,
  from: number,
  to: number,
  current?: EditorSearchMatch,
  ...
  for (let result = cursor.next(); !result.done; result = cursor.next()) {
    ...
    count++;
    if (current && result.value.from === current.from && result.value.to === current.to)
      currentIndex = count;
  }
...
export function collectEditorSearchMatches(
  query: SearchQuery,
  state: EditorState,
  from: number,
  to: number,
...
export function replaceEditorSearchMatches(
  ...
  replacement: (match: EditorSearchMatch) => string,
): boolean {

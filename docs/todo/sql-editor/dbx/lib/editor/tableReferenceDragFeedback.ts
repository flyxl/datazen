 ...
 */
export function isPointOverElementRoot(
  clientX: number,
  clientY: number,
...
export function isOverSqlEditorTarget(
  clientX: number,
  clientY: number,
  doc: Document = document,
): boolean {
...
export function beginTableReferenceDragFeedback(
  label: string,
  doc: Document = document,
): TableReferenceDragFeedback {
  const chip = doc.createElement('div');
  chip.dataset.tableReferenceDragChip = '';
  chip.setAttribute('aria-hidden', 'true');
  chip.className =
    'pointer-events-none fixed z-[9999] max-w-72 truncate rounded-md border border-border bg-popover px-2 py-1 font-mono text-xs leading-4 text-popover-foreground shadow-lg';
  ...
  const update = (clientX: number, clientY: number) => {
    ...
    const x = Math.min(
      clientX + CHIP_OFFSET_X,
      viewport.innerWidth - rect.width - CHIP_VIEWPORT_MARGIN,
    );
    const y = Math.min(
      clientY + CHIP_OFFSET_Y,
      viewport.innerHeight - rect.height - CHIP_VIEWPORT_MARGIN,
    );

...
type Viewish = Pick<
  EditorView,
  'coordsAtPos' | 'scaleX' | 'defaultCharacterWidth' | 'defaultLineHeight'
> & {
  state: Pick<EditorView['state'], 'doc'>;
  lineBlockAt: (
    pos: number,
  ) => Pick<ReturnType<EditorView['lineBlockAt']>, 'top' | 'bottom' | 'height'>;
 ...
 */
export function currentStatementFrameRect(
  view: Viewish,
  from: number,
  to: number,
): FrameRect | null {
  ...
  const top = startCoords
    ? startCoords.top - base.top + view.scrollDOM.scrollTop
    : view.lineBlockAt(from).top;

  const endCoords = view.coordsAtPos(to, -1);
  const bottom = endCoords
    ? endCoords.bottom - base.top + view.scrollDOM.scrollTop
    : view.lineBlockAt(to).bottom;
  ...
  for (let lineNumber = startLine.number; lineNumber <= endLine.number; lineNumber += 1) {
    ...
    const wrappedFromCoordinates =
      !!lineLeft && !!lineRight && spansVisualRows(lineLeft, lineRight);
    const wrappedFromLineBlock =
      (!lineLeft || !lineRight) &&
      fullLogicalLine &&
      view.lineBlockAt(anchorFrom).height > view.defaultLineHeight * 1.5;
 ...
 */
export function currentStatementFrameLayer(
  viewModule: CurrentStatementFrameModule,
  resolve: StatementFrameResolver,
): Extension {
  return viewModule.layer({
    ...
    markers(view) {
      ...
      if (!rect) return [];
      return [
        new viewModule.RectangleMarker(

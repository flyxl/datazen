...
export function completionLabelPresentation(
  label: string,
  filterText?: string,
): CompletionLabelPresentation {
  const normalizedFilterText = filterText?.trim();
  if (!normalizedFilterText || label.toLowerCase().startsWith(normalizedFilterText.toLowerCase()))
    return { label };
  return {
...
export function compareSqlCompletions(
  a: CompletionSortItem,
  b: CompletionSortItem,

...
export function buildQueryEditorLineNumbersExtension(
  factory: LineNumbersFactory | null,
  enabled: boolean,
...
export function createQueryEditorLineNumberAlignmentExtension(
      ...
      private scheduleMeasure(view: import('@codemirror/view').EditorView) {
        ...
        view.requestMeasure({
          read: (measuredView) => {
            ...
            return [
              ...measuredView.dom.querySelectorAll<HTMLElement>(
                '.cm-lineNumbers .cm-gutterElement',
              ),
            ].map((element) => ({
              element,
              wrapped: isWrappedLineNumberGutter(
          ...
          write: (measurements) => {
            ...
            for (const { element, wrapped } of measurements)
              setQueryEditorLineNumberAlignment(element, wrapped);
          },
        });
      }
    },

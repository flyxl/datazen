...
export function createSqlSignatureTooltipDom(
  signature: SqlFunctionSignatureHelp | null,
): HTMLElement {
  ...
  signature.overloads.forEach((overload, overloadIndex) => {
    ...
    row.className =
      overloadIndex > 0
        ? 'mt-1 flex items-center gap-2 font-mono'
    ...
    overload.parameterGroups.forEach((group, groupIndex) => {
      ...
      group.forEach((parameter, parameterIndex) => {
        ...
        const active =
          groupIndex === overload.activeGroup && parameterIndex === overload.activeParameter;

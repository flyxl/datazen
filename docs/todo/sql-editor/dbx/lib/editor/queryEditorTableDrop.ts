...
import {
  COLUMN_NAME_COPY_SEPARATOR_VALUES,
  loadColumnNameCopySeparator,
  type ColumnNameCopySeparator,
  isColumnNameCopySeparator,
} from '@/lib/dataGrid/dataGridColumnNameCopy';
...
} from '@/lib/sql/sqlIdentifier.ts';

/** 智能引号使用反引号的方言族。 */
const SMART_QUOTE_BACKTICK_TYPES = new Set<DatabaseType>([
  'mysql',
  'clickhouse',
  'hive',
  'argo',
...
function normalizeTableReferences(value: unknown): QueryEditorTableReferenceEntry[] {
  ...
  for (const item of value) {
    ...
    const tableName =
      typeof (item as QueryEditorTableReferenceEntry).tableName === 'string'
        ? (item as QueryEditorTableReferenceEntry).tableName.trim()
        : '';
    if (!tableName) continue;
    const schema =
      typeof (item as QueryEditorTableReferenceEntry).schema === 'string'
        ? (item as QueryEditorTableReferenceEntry).schema
        : undefined;
...
function applyOptionalReferenceMeta(
  payload: QueryEditorTableReferencePayload,
  source: Partial<QueryEditorTableReferencePayload>,
) {
  ...
  if (typeof source.driverProfile === 'string' && source.driverProfile)
    payload.driverProfile = source.driverProfile;
  if (isColumnNameCopySeparator(source.columnNameSeparator))
    payload.columnNameSeparator = source.columnNameSeparator;
  if (isColumnNameCopySeparator(source.tableNameSeparator))
    payload.tableNameSeparator = source.tableNameSeparator;
...
export function createColumnReferencePayload(options: {
  ...
  database?: string;
  schema?: string;
  columnNames?: readonly (string | undefined | null)[];
  databaseType?: DatabaseType;
  columnNameSeparator?: ColumnNameCopySeparator;
}): QueryEditorTableReferencePayload | null {
...
export function parseTableReferencePayload(
  ...
  try {
    ...
    if (parsed.referenceType === 'database') {
      ...
      if (typeof parsed.driverProfile === 'string' && parsed.driverProfile)
        payload.driverProfile = parsed.driverProfile;
      return payload;
    }
    ...
    const columnName =
      typeof parsed.columnName === 'string' && parsed.columnName ? parsed.columnName : undefined;
    const referenceType =
      parsed.referenceType === 'column' || columnName || columnNames.length > 0
        ? 'column'
        : 'table';
    const payload: QueryEditorTableReferencePayload = {
      kind: 'dbx-table-reference',
...
export function clearActiveTableReferencePayload(
  payload?: QueryEditorTableReferencePayload | null,
) {
...
export function createTableReferenceDropEvent(detail: QueryEditorTableReferenceDropDetail) {
  return new CustomEvent<QueryEditorTableReferenceDropDetail>(DBX_TABLE_REFERENCE_DROP_EVENT, {
    detail,
  });
}

export function createTableReferenceHoverEvent(detail: QueryEditorTableReferenceHoverDetail) {
  return new CustomEvent<QueryEditorTableReferenceHoverDetail>(DBX_TABLE_REFERENCE_HOVER_EVENT, {
    detail,
  });
}
...
function separatorValue(
  payload: QueryEditorTableReferencePayload,
  options: TableReferenceInsertOptions | undefined,
  kind: 'table' | 'column',
): string {
...
export function tableReferenceInsertText(
  ...
  options?: TableReferenceInsertOptions,
): string {
  ...
  const columnNames = payload.columnNames?.length
    ? payload.columnNames
    : payload.columnName
      ? [payload.columnName]
      : [];
  if (payload.referenceType === 'column' && columnNames.length > 0) {
    return columnNames
      .map((name) => quoteColumnReferenceName(databaseType, name))
      .join(separatorValue(payload, options, 'column'));
  }
  ...
  const tableReferences = payload.tableReferences?.length
    ? payload.tableReferences
    : payload.tableName
      ? [{ schema: payload.schema, tableName: payload.tableName }]
      : [];

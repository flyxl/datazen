...
function compareSemanticSelectionRanges(
  left: SemanticSelectionRange,
  right: SemanticSelectionRange,
  cursor: number,
): number {
...
export function createSemanticSelectionRangeIndex(
  candidates: readonly SemanticSelectionRange[],
): SemanticSelectionRangeIndex {
  ...
  const ranges = [...unique.values()].sort(
  ...
  const buildNode = (from: number, to: number): SemanticSelectionRangeIndexNode | undefined => {
    ...
    return {
      ...
      minRangeLength: Math.min(
        left?.minRangeLength ?? Number.POSITIVE_INFINITY,
        right?.minRangeLength ?? Number.POSITIVE_INFINITY,
      ),
      left,
      right,
    };
  ...
  const visitContaining = (
    current: SemanticSelectionRange,
    visit: (range: SemanticSelectionRange) => void,
  ) => {
  ...
  return {
    ...
    findNext(current, cursor) {
      ...
      const visitNode = (node: SemanticSelectionRangeIndexNode | undefined) => {
        ...
        const first =
          (node.left?.minRangeLength ?? Number.POSITIVE_INFINITY) <=
          (node.right?.minRangeLength ?? Number.POSITIVE_INFINITY)
            ? node.left
            : node.right;
...
export function chooseNextSemanticSelectionRange(
  current: SemanticSelectionRange,
  cursor: number,
  candidates: readonly SemanticSelectionRange[],
  ...
  return (
    [...unique.values()].sort((left, right) =>
      compareSemanticSelectionRanges(left, right, cursor),
    )[0] ?? null
  );
}

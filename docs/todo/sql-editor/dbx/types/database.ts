...
export type CompletionAssistantObjectKind =
  | 'database'
  | 'schema'
  | 'table'
  | 'view'
...
export type CompletionAssistantCandidateKind =
  | 'database'
  | 'schema'
  | 'table'
  | 'view'
...
export type TransportLayerConfig =
  ...
  | ({ type: 'http_tunnel' } & HttpTunnelConfig);
...
export type BatchStatementExecutionStatus =
  | 'pending'
  | 'running'
  | 'success'
  | 'error'
...
export type SidebarOrderEntry =
  | { type: 'group'; id: string; children?: SidebarOrderEntry[]; connectionIds?: string[] }

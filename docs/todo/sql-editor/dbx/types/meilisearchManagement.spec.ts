...
describe('Meilisearch task selector safety', () => {
  ...
  it('freezes the same normalized status selector enforced by the backend', () => {
    expect(
      normalizeTaskMutationSelector(
        { indexUids: [' movies ', 'movies'], statuses: ['FAILED', 'processing'] },
        'cancel',
      ),
    ).toEqual({
      indexUids: ['movies'],
    ...
    expect(normalizeTaskMutationSelector({ uids: [2] }, 'delete')?.statuses).toEqual([
      'succeeded',
      'failed',
      'canceled',
    ]);
  ...
  it('normalizes task selector values deterministically', () => {
    expect(
      normalizeTaskSelector({ uids: [3, 1, 3], indexUids: [' books ', ''], statuses: ['FAILED'] }),
    ).toEqual({
      uids: [1, 3],
      indexUids: ['books'],
      statuses: ['failed'],
  ...
  it('formats task statuses, timestamps, and ISO durations for display', () => {
    ...
    expect(formatMeilisearchTaskDateTime('2026-08-13T16:37:28Z', 'en-US')).not.toContain(
      'T16:37:28Z',
    );
    ...
    expect(
      formatMeilisearchTaskDetails(
        { receivedDocuments: 8, indexedDocuments: 8, futureField: true },
        { receivedDocuments: '接收文档数', indexedDocuments: '已索引文档数' },
      ),
    ).toBe('接收文档数: 8 · 已索引文档数: 8 · futureField: true');

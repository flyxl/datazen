...
export type MultiDbExecutionItemStatus =
  | 'pending'
  | 'running'
  | 'success'
  | 'failed'
...
export type SqlExecutionTargetDatabaseTypeResolver = (
  target: MultiDbExecutionTarget,
...
export function multiDbExecutionTargetKey(target: MultiDbExecutionTarget): string {
  return JSON.stringify([
    target.connectionId,
    normalizeOptionalExecutionTargetPart(target.catalog),
    target.database,
    normalizeOptionalExecutionTargetPart(target.schema),
  ]);
}

export function normalizeMultiDbExecutionTarget(
  target: MultiDbExecutionTarget,
): MultiDbExecutionTarget | undefined {
...
export function dedupeMultiDbExecutionTargets(
  targets: readonly MultiDbExecutionTarget[],
): MultiDbExecutionTarget[] {

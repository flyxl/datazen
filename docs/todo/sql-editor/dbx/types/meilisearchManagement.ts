...
export function meilisearchTaskStatusLabel(status: string): string {
  return (
    MEILISEARCH_TASK_STATUS_OPTIONS.find((option) => option.value === status.toLowerCase())
      ?.label ?? status
  );
}

export function formatMeilisearchTaskDateTime(
...
export function formatMeilisearchTaskDuration(
  value: string | null | undefined,
  locale?: string,
): string {
  if (!value) return '-';
  ...
  if (totalSeconds < 1)
    return formatDurationUnit(Math.round(totalSeconds * 1_000), 'millisecond', locale);
  if (totalSeconds < 60) return formatDurationUnit(totalSeconds, 'second', locale, 2);
  if (totalSeconds < 3_600) {
    const minutes = Math.floor(totalSeconds / 60);
    const seconds = Math.round(totalSeconds % 60);
    return seconds
      ? `${formatDurationUnit(minutes, 'minute', locale)} ${formatDurationUnit(seconds, 'second', locale)}`
  ...
  if (totalSeconds < 86_400) {
    ...
    return minutes
      ? `${formatDurationUnit(hours, 'hour', locale)} ${formatDurationUnit(minutes, 'minute', locale)}`
      : formatDurationUnit(hours, 'hour', locale);
  }
  const days = Math.floor(totalSeconds / 86_400);
  const hours = Math.round((totalSeconds % 86_400) / 3_600);
  return hours
    ? `${formatDurationUnit(days, 'day', locale)} ${formatDurationUnit(hours, 'hour', locale)}`
    : formatDurationUnit(days, 'day', locale);
}

export function formatMeilisearchTaskDetails(
...
export function normalizeTaskMutationSelector(
  selector: TaskSelector,
  ...
  const allowed =
    kind === 'cancel' ? ['enqueued', 'processing'] : ['succeeded', 'failed', 'canceled'];
  const statuses = normalized.statuses?.length
    ? allowed.filter((status) => normalized.statuses?.includes(status))
    : allowed;

...
const formatterSectionStart = dialogSource.indexOf(
  `activeSettingsTab === 'formatter'`,
  editorSectionStart,
);
...
const settingsSearchResultsStart = dialogSource.indexOf(
  'v-if="settingsSearchVisible"',
  editorPreviewStart,
);
...
const shortcutsSectionStart = dialogSource.indexOf(
...
describe('EditorSettingsDialog live preview placement', () => {
  it('keeps the live preview below search and outside the editor settings scroller', () => {
    ...
    expect(
      dialogSource.indexOf("activeSettingsTab === 'editor' && !settingsSearchVisible", searchInput),
    ).toBeLessThan(editorPreviewStart);
    ...
    for (const id of [
      'editor-show-statement-run-buttons',
      'editor-show-line-numbers',
      'editor-show-current-statement-frame',
      'editor-sql-semantic-diagnostics',
      'editor-show-table-ddl-hover-preview',
    ]) {
  ...
  it('places execution behavior before SQL completion with high-priority controls first', () => {
    ...
    const selectFirstCompletion = editorSection.indexOf(
      't("settings.selectFirstCompletionOnOpen")',
    );
    ...
    expect(editorSection).toContain(
      'class="flex items-start justify-between gap-4 rounded-md border bg-muted/20 px-3 py-2" data-editor-completion-trigger-mode',
    );
    expect(editorSection).toContain('<SelectTrigger class="h-8 w-44 shrink-0">');
    expect(editorSection).toContain(
      'class="grid gap-4 md:grid-cols-2" data-editor-execution-settings',
    );
    expect(editorSection).toContain(
      'class="flex items-center justify-between gap-4 rounded-md border bg-muted/20 px-3 py-2 md:col-span-2" data-editor-execute-mode',
    );
  ...
  it('groups related execution and unsaved SQL controls without removing saved-target behavior', () => {
    ...
    const executionGroupEnd = editorSection.indexOf(
      'data-editor-sql-completion-settings',
      executionGroup,
    );
    const unsavedGroup = editorSection.indexOf('data-editor-unsaved-sql-settings');
    const unsavedGroupEnd = editorSection.indexOf(
      'generate-sql-include-database-name',
      unsavedGroup,
    );

    for (const id of [
      'editor-execute-all-on-blank-line',
      'editor-show-execution-target-picker',
      'editor-confirm-dangerous-sql',
      'editor-continue-on-error',
    ]) {
  ...
  it('describes the execute shortcut below the mode selector in every supported locale', () => {
    expect(editorSection.indexOf('executeModeDescription')).toBeGreaterThan(
      editorSection.indexOf('executeModeLabel'),
    );

    for (const locale of ['zh-CN', 'zh-TW', 'en', 'es', 'it', 'ja', 'ko', 'pt-BR', 'tr']) {
      const source = readFileSync(
        new URL(`../../../i18n/locales/${locale}.ts`, import.meta.url),
        'utf8',
      );
  ...
  it('shows a concise description for the selected completion trigger mode', () => {
    ...
    for (const locale of ['zh-CN', 'zh-TW', 'en', 'es', 'it', 'ja', 'ko', 'pt-BR', 'tr']) {
      ...
      for (const key of [
        'completionTriggerModeManualDescription',
        'completionTriggerModeRequirePrefixDescription',
        'completionTriggerModePositionalDescription',
      ]) {
  ...
  it('moves table navigation and query prefill controls to Navigation', () => {
    const navigationSection = dialogSource.slice(
      dialogSource.indexOf(`activeSettingsTab === 'navigation'`),
    );
  ...
  it('groups navigation settings by data tabs and sidebar browsing flow', () => {
    ...
    const browseObjectsOnDatabaseActivation = navigationSection.indexOf(
      'id="sidebar-browse-objects-on-database-activation"',
    );
  ...
  it('puts the data grid filter view first with flat choices and a live preview', () => {
    ...
    expect(dataSection).toContain(
      `:class="['overflow-hidden rounded-md border bg-muted/20', settingsSearchTargetClass('data-grid-filter-view')]"`,
    );
    expect(dataSection).toContain('data-data-grid-filter-view-options');
    expect(dataSection).toContain(
      'v-if="dataGridFilterViewPreviewExpanded" class="pointer-events-none select-none overflow-hidden border-t bg-background" data-data-grid-filter-view-preview',
    );
    ...
    expect(dataSection).toContain(
      'dataGridFilterViewPreviewExpanded = !dataGridFilterViewPreviewExpanded',
    );
    ...
    expect(
      dataSection.match(/text-muted-foreground\/70">\{\{ t\("grid.applyFilter"\) \}\}/g),
    ).toHaveLength(2);
    expect(dataSection).not.toContain(
      'rounded bg-primary px-2 py-1 text-primary-foreground">{{ t("grid.applyFilter") }}',
    );
    expect(
      dataSection.match(/editDataGridFilterEditorView = '(quick|conditions|text)'/g),
    ).toHaveLength(3);
    ...
    for (const locale of ['zh-CN', 'zh-TW', 'en', 'es', 'it', 'ja', 'ko', 'pt-BR', 'tr']) {
      ...
      for (const key of [
        'dataGridFilterViewPreview',
        'dataGridFilterViewPreviewExpand',
        'dataGridFilterViewPreviewCollapse',
      ]) {
        expect(source, `${locale}:${key}`).toContain(`${key}:`);
      }
    }
  ...
  it.each(['en', 'zh-CN', 'zh-TW', 'es', 'pt-BR', 'it', 'ja', 'ko', 'tr'])(
    'locale %s translates settings.previewSyntaxErrorHint',
    (locale) => {

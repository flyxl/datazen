...
describe('EditorSettingsDialog apply persistence error handling', () => {
  it('shares a single apply entrypoint between Apply and Apply & Close', () => {
    ...
    const block = dialogSource.slice(
      dialogSource.indexOf('function applySettingsErrorToast'),
      dialogSource.indexOf('async function restartDbxForDuckDbIsolation()'),
    );
  ...
  it('never closes the dialog before persistence succeeds', () => {
    ...
    const block = dialogSource.slice(
      dialogSource.indexOf('async function applySettingsAndClose()'),
      dialogSource.indexOf('async function restartDbxForDuckDbIsolation()'),
    );
  ...
  it('surfaces persistence failures instead of silently swallowing them', () => {
    const resultBlock = dialogSource.slice(
      dialogSource.indexOf('async function applySettingsForResult()'),
      dialogSource.indexOf('async function applySettings()'),
    );

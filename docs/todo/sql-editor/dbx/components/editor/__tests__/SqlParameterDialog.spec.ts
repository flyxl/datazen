...
const historyMocks = vi.hoisted(() => ({
  ...
  remember:
    vi.fn<(values: Record<string, SqlParameterInput>) => Record<string, SqlParameterInput[]>>(),
}));
...
const SQL =
  'SELECT ${text_value}, ${numeric_value}, ${null_value}, ${raw_value}, ${empty_value}, ${text_value}';
...
const RAW_VALUES = Object.fromEntries(
  Object.entries(MIXED_VALUES).map(([key, input]) => [key, { ...input, kind: 'raw' as const }]),
);
...
beforeEach(() => {
  ...
  historyMocks.remember.mockImplementation((values) =>
    Object.fromEntries(Object.entries(values).map(([key, input]) => [key, [{ ...input }]])),
  );
});
...
describe('SqlParameterDialog raw parameter action', () => {
  it('sets every displayed parameter to raw while preserving each current value', async () => {
    ...
    expect(
      Array.from(document.body.querySelectorAll('[role="combobox"]')).map((trigger) =>
        trigger.textContent?.trim(),
      ),
    ).toEqual(Array(PARAMETERS.length).fill('Raw SQL'));
...
describe('SqlParameterDialog ignore parameter action', () => {
  it('closes the dialog and emits the original SQL without remembering or mutating values', async () => {
    ...
    expect(historyMocks.remember).not.toHaveBeenCalled();
    expect(parameterInputs().map((input) => input.value)).toEqual(
      PARAMETERS.map((parameter) => MIXED_VALUES[parameter.key]?.value ?? ''),
    );
  });
...
describe('SqlParameterDialog clear parameter values action', () => {
  it('clears every displayed value while preserving kinds and updating the preview', async () => {
    ...
    ).toEqual(['String', 'Number', 'NULL', 'Raw SQL', 'Boolean']);
    expect(parameterInputs()[2]?.disabled).toBe(true);
  ...
  it('preserves parameter history after clearing current values', async () => {
    ...
    const historyEntry = Array.from(document.body.querySelectorAll('button')).find((button) =>
      button.textContent?.includes('alpha beta'),
    );

...
const affectedChoiceKeys = [
  'tabLayoutScroll',
  'tabLayoutWrap',
  'dataTabReuseAlwaysNew',
  'dataTabReuseSameTable',
  'dataTabReuseActiveTab',
  'routineSourceOpenModeQueryTab',
  'routineSourceOpenModeDialog',
] as const;
...
function elementTagForKeyInButton(buttonKey: string, key: string, element = 'div'): string {
  ...
  if (keyIndex < 0 || start < 0 || end < 0)
    throw new Error(`Missing <${element}> for settings key ${key} in ${buttonKey}`);
...
function expectClassTokens(className: string, tokens: readonly string[]) {
  ...
  for (const token of tokens)
    expect(classes.has(token), `${className} should include ${token}`).toBe(true);
}
...
describe('EditorSettingsDialog choice card containment', () => {
  ...
  it('lets long two-column descriptions wrap inside a shrinkable text container', () => {
    ...
    for (const [buttonKey, descriptionKey] of [
      ...
      expectClassTokens(classNameFromTag(elementTagForKeyInButton(buttonKey, descriptionKey)), [
        'whitespace-normal',
        'break-words',
      ]);
    }
  ...
  it('keeps three-column titles shrinkable without allowing help icons to collapse', () => {
    for (const key of [
      ...
      expect(block).toMatch(
        /<div class="[^"]*\bflex\b[^"]*\bmin-w-0\b[^"]*\bitems-center\b[^"]*">/,
      );
  ...
  it('restores large icon theme choices below the debug log controls', () => {
    expect(sourceIndexForKey('iconTheme')).toBeGreaterThan(
      sourceIndexForKey('debugLoggingEnabled'),
    );
    expect(templateSource).toContain('data-icon-theme-settings');
    for (const key of ['iconThemeDefault', 'iconThemeBlack'] as const) {
      const block = buttonBlockForKey(key);
      expectClassTokens(classNameFromTag(openingTag(block, 'Button')), [
        'settings-choice-card',
        'h-auto',
        'min-w-0',
        'whitespace-normal',

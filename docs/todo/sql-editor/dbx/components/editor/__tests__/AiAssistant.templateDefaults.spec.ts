...
describe('AI assistant applies per-db_type prompt template defaults', () => {
  it('panel-open resolution uses defaults-else-last-used through the tested helper and enforces the char cap', () => {
    const maybeBody = source.slice(
      source.indexOf('async function maybeApplyAutoTemplates()'),
      source.indexOf('// Reset template selection when the user switches'),
    );
    ...
    const applyBody = source.slice(
      source.indexOf('function applyResolvedTemplateIds('),
      source.indexOf('async function maybeApplyAutoTemplates()'),
    );
    expect(applyBody).toContain(
      'capTemplateIdsToCharLimit(ids, promptTemplateStore.templates, ACTIVE_TEMPLATES_TOTAL_MAX)',
    );
  });
  ...
  it('namespace switch re-resolves defaults only when both stores are loaded, otherwise clears', () => {
    const watcherBody = source.slice(
      source.indexOf('aiTemplateNamespaceKey,'),
      source.indexOf('function toggleTemplateId('),
    );
    expect(watcherBody).toContain(
    ...
    expect(watcherBody).toContain(
      'resolveDefaultTemplateIds(templateDbType.value, settings.aiDefaultTemplatesByDbType)',
    );
  ...
  it('panel-open resolution bails when the namespace switched during the load await', () => {
    const maybeBody = source.slice(
      source.indexOf('async function maybeApplyAutoTemplates'),
      source.indexOf('watch(', source.indexOf('async function maybeApplyAutoTemplates')),
    );
  ...
  it('selector-open load retry also applies pending defaults', () => {
    const watcherBody = source.slice(
      source.indexOf('watch(showTemplateSelector'),
      source.indexOf('// Auto-apply per-db_type'),
    );
    expect(watcherBody).toContain(
      'void promptTemplateStore.ensureLoaded().then(() => void maybeApplyAutoTemplates());',
    );
  });
  ...
  it("send records the sent templates as the db_type's last-used selection", () => {
    ...
    const recordIdx = source.indexOf(

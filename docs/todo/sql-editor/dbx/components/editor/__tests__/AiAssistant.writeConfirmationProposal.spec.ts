...
describe('AI assistant write confirmation replacement', () => {
  ...
  it('does not render an executable write confirmation without exact SQL', () => {
    expect(source).toContain(
      'looksLikeWriteSqlProposal(msg.content) && !isActionableWriteSqlProposal(msg.content)',
    );
    expect(source).toContain(
  ...
  it('routes generic action replies separately from write-SQL replies', () => {
    expect(source).toContain(
      'isWriteConfirmation ? t("ai.writeSqlConfirmationReplyYes") : t("ai.proposalConfirmReplyYes")',
    );
    expect(source).toContain(
      'isWriteConfirmation ? t("ai.writeSqlConfirmationReplyNo") : t("ai.proposalConfirmReplyNo")',
    );
  });

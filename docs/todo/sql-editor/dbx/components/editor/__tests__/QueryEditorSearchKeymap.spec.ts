...
// pi-lens-ignore: typescript:2307
import {
  createQueryEditorReplaceShortcutBindings,
  createQueryEditorReplaceShortcutHandler,
  createQueryEditorSearchKeymap,
} from '@/lib/editor/queryEditorSearchKeymap';
...
describe('QueryEditor search keymap precedence', () => {
  it('runs configured editor actions before the built-in search fallback', () => {
    ...
    expect(
      runScopeHandlers(view, new KeyboardEvent('keydown', { key: 'f', ctrlKey: true }), 'editor'),
    ).toBe(true);
        ...
        extensions: [
          Prec.high(
            EditorView.domEventHandlers({
              keydown: createQueryEditorReplaceShortcutHandler({
                shortcut: 'Mod+R',
                openReplace,
                isReadOnly: () => false,
              }),
          ...
          Prec.high(
            keymap.of([
              ...createQueryEditorReplaceShortcutBindings('Mod+R', openReplace),
  ...
  it('keeps the search fallback above lower-priority CodeMirror bindings', () => {
    ...
    const view = new EditorView({
      ...
      state: EditorState.create({
        extensions: [
          keymap.of([{ key: 'Mod-f', run: lowerPrioritySearch }]),
          Prec.high(
            keymap.of(
              createQueryEditorSearchKeymap({
                openSearch,

...
const editorSearchPanelSource = readFileSync(
  new URL('../EditorSearchPanel.vue', import.meta.url),
  'utf8',
);
const queryEditorSource = readFileSync(new URL('../QueryEditor.vue', import.meta.url), 'utf8');
const contentAreaSource = readFileSync(
  new URL('../../layout/ContentArea.vue', import.meta.url),
  'utf8',
);
...
describe('QueryEditor search shortcuts', () => {
  it('opens search and replace in editable editors', () => {
    ...
    const bindings = createQueryEditorSearchKeymap({
      openSearch,
      openReplace,
      isReadOnly: () => false,
    });
  ...
  it('keeps the existing search navigation and read-only replace guards', () => {
    expect(queryEditorSource).toMatch(
      /\.\.\.binding\(shortcuts\.sendSelectionToAi[\s\S]*\.\.\.createQueryEditorSearchKeymap/,
    );
    expect(queryEditorSource).not.toMatch(
      /Prec\.highest\(\s*keymap\.of\(\[\s*\.\.\.createQueryEditorSearchKeymap/,
    );
    expect(queryEditorSource).toMatch(
      /function openReplace\(\): boolean \{\s*if \(props\.readOnly\) return false;/,
    );
    expect(contentAreaSource).toContain(

...
const source = readFileSync(
  path.resolve(process.cwd(), 'apps/desktop/src/components/editor/QueryEditor.vue'),
  'utf8',
);
...
describe('QueryEditor context menu lifecycle', () => {
  ...
  it('uses the target synchronized in the current context-menu event', async () => {
          ...
          h(
            ...
            { items: currentItems },
            {
              default: (slot: { onContextMenu: (event: MouseEvent) => void }) => {
                ...
                return [
                  contextTarget('table-a', 'table_a'),
                  contextTarget('table-b', 'table_b'),
                  contextTarget('empty', null),
                ];
              },
            },
          );
    ...
    expect(await open('table-a')).toMatchObject({
      disabled: false,
      textContent: 'Inspect table_a',
    });
    expect(await open('table-a')).toMatchObject({
      disabled: false,
      textContent: 'Inspect table_a',
...
describe('QueryEditor batch column selection', () => {
  it('keeps the confirmation action visible during pinyin filtering and supports keyboard toggling', () => {
    ...
    const bypassFilterEnd = source.indexOf(
      '\n}\n\nfunction localCompletionDatabaseNames',
      bypassFilterStart,
    );
  ...
  it('applies checked columns directly through Enter and the completion Tab shortcut', () => {
    ...
    expect(source.slice(handleEnterStart, handleEnterEnd)).toContain(
    ...
    expect(source.slice(tabStart, tabEnd)).toContain(
  ...
  it('updates the insertion count while a mouse selection is still in progress', () => {
    ...
    const dragUpdateEnd = source.indexOf(
      '\n}\n\nconst BATCH_COLUMN_SELECTION_AUTO_SCROLL_EDGE_PX',
      dragUpdateStart,
    );

    expect(source).toContain('function updateBatchColumnSelectionActionLabel');
    expect(source.slice(dragUpdateStart, dragUpdateEnd)).toContain(
  ...
  it('cancels stale scroll restoration before a new drag can begin', () => {
    ...
    const refreshEnd = source.indexOf(
      '\n}\n\nfunction finishBatchColumnSelectionDrag',
      refreshStart,
    );
    ...
    expect(source.slice(refreshStart, refreshEnd)).toContain(
...
describe('QueryEditor pointer selection', () => {
  it('lets CodeMirror handle shift-click instead of starting the selection drag gesture', () => {
    ...
    expect(dragSource.indexOf('if (event.shiftKey) return false;')).toBeLessThan(
      dragSource.indexOf('const selection = selectedRangeAtPointer'),
    );
  });
});

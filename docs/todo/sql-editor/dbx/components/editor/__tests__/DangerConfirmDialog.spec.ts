...
describe('DangerConfirmDialog SQL preview', () => {
  ...
  it('highlights only bounded head and tail fragments for huge SQL', async () => {
    const sql = Array.from(
      { length: 40_000 },
      (_, index) => `INSERT INTO t VALUES (${index});`,
    ).join('\n');

    await mountDialog(sql);

    const highlightedCharacters = highlight.mock.calls.reduce(
    ...
    expect(highlight.mock.calls.flatMap(([fragment]) => fragment.split('\n'))).toHaveLength(
      DANGER_PREVIEW_MAX_LINES,
    );
    expect(
      document.body.querySelector('[data-testid="danger-preview-truncated"]')?.textContent,
  ...
  it('copies the full SQL instead of the bounded preview', async () => {
    ...
    const copyButton = Array.from(document.body.querySelectorAll('button')).find(
      (button) => button.title === 'Copy full text',
    );
    copyButton?.click();
    await nextTick();
...
describe('DangerConfirmDialog running/cancel footer state', () => {
  ...
  it('shows an active Cancel Query button while loading when cancelable is true', async () => {
    ...
    await mountDialog(
      ...
      { loading: true, cancelable: true },
      { onCancelRunning },
    );

    const buttons = footerButtons();
    const cancelRunningButton = buttons.find(
      (button) => button.textContent?.trim() === 'Cancel Query',
    );
  ...
  it('disables the Cancel Query button while the cancel itself is in flight', async () => {
    await mountDialog('DELETE FROM big_orders;', {
      loading: true,
      cancelable: true,
      cancelRunningLoading: true,
    });

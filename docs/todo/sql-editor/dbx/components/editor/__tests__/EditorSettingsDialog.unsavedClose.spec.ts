...
describe('EditorSettingsDialog unsaved-changes close guard', () => {
  it('imports the shared unsaved-changes decision helper', () => {
    expect(dialogSource).toContain(
      'shouldConfirmEditorSettingsDialogClose, type EditorSettingsDraft } from "@/lib/settings/editorSettingsDraft"',
    );
  });
  ...
  it('routes every close path through the same guard instead of emitting update:open directly', () => {
    ...
    const closeSettingsBody = dialogSource.slice(
      dialogSource.indexOf('function closeSettings()'),
      dialogSource.indexOf('function cancelUnsavedSettingsClose()'),
    );

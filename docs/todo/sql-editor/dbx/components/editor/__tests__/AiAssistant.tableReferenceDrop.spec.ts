...
import {
  handleAiTableReferenceDropEvent,
  type AiTableReferenceDropContext,
} from '@/lib/ai/aiTableReferenceDrop';
import {
...
function dispatchTableReferenceDrop(
...
describe('AI assistant table reference drop', () => {
  it('accepts a table dropped from the active connection and database', () => {
    expect(
      dispatchTableReferenceDrop(tablePayload(), { connectionId: 'conn-1', database: 'app-db' }),
    ).toEqual(['@public.users']);
  });

  it('rejects a table dropped from another connection', () => {
    expect(
      dispatchTableReferenceDrop(tablePayload({ connectionId: 'conn-2' }), {
        connectionId: 'conn-1',
        database: 'app-db',
      }),
    ).toEqual([]);
  ...
  it('rejects a table dropped from another database', () => {
    expect(
      dispatchTableReferenceDrop(tablePayload({ database: 'analytics' }), {
        connectionId: 'conn-1',
        database: 'app-db',
      }),
    ).toEqual([]);

...
| **Driver Command** | `driver_command.rs` | `execute_driver_command`, `execute_driver_command_stream`, `get_connection_commands`, `get_driver_commands` — SQL `query`/`query_stream`/`execute`、Workflow、Redis `scan_keys`/`get_key`、Schema 目录 `list_databases`/`list_tables`/`get_table_schema`、Schema 对象 `list_objects`、SQLite 驱动内建 ADB 命令（`adb_list_packages` 等，`requiresConnection = false`，决策 2 自 Host 迁入）等统一入口 |
...
| Schema | `schema.rs` | `get_databases`, `get_tables`, `get_columns`, `get_table_schema`, `get_table_data`（可选 `database` 定位）, `get_er_data`, `get_database_objects`, `get_object_ddl`, `get_privileges`（目录/对象/权限命令内部经 Driver Command 执行）；结构变更走 `structure.rs` 的 `plan_table_structure_changes`（可选 `database` 定位） |
...
| 同步 | `commands/sync/` | `inspect_data_sync`, `compare_data_sync`, `execute_data_sync`, `classify_data_sync_pair`；已移除：`compare_databases`/`sync_table`/`sync_tables`（legacy）、`classify_sync_pair`（旧名） |
...
| 首屏背景 | `theme.rs` | `set_surface_background` |

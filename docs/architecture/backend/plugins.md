...
> [返回架构总览](../README.md) · PRD：[docs/prd/ui-plugins.md](../../prd/ui-plugins.md) · 技术方案：[docs/prd/ui-plugins-implementation.md](../../prd/ui-plugins-implementation.md)
...
│ pluginStore/workspaceTabsStore │ PluginPageShell ── postMessage(ui-plugin)──┐│
└──────────────────────────┬──────────────────────────────┼──────────────────┘│
...
└─────────────────────────────────────────────────────────────────────────┘    │
        ▲ sandbox iframe（opaque origin，@datazen/ui-plugin-sdk 或裸 JS）◄──────┘
```
...
## 桥接协议（src/lib/uiPluginBridge.ts）

信封 `{ch:'ui-plugin', type, reqId, target}`；握手 `plugin.ready → host.ready(apiVersion/locale/dark/tokens)`；响应 `${type}.ok|.err` 回显 reqId。路由表 own-property 查找（原型链键一律 E_NOT_FOUND，BUG-F6-01）。
...
- Host 单测：桥接语义+安全（uiPluginBridge*.test）、主题管线（themePackApply.pluginTheme.test）、壳集成（PluginPageShell.bridge）、store、管理页/工作区组件

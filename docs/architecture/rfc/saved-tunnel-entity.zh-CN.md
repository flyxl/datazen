...
1. **bootstrap.rs** `generate_handler!` 注册：
   ```rust
   ...
   ```
2. **前端**：连接表单中「选择已有 tunnel」下拉（`invoke('get_tunnels')`），保存时只写 `tunnelId`。
3. **ConnectionConfig 字面量**：测试/样例中补 `tunnel_id: None`。
4. 确认 `types.rs` 完整（曾拆 `types_rest*.inc.rs`），本地 `cargo check`。

...
**测试**:
- `src-tauri/tests/skill_workflow_tests.rs` — 13 个后端集成测试
- `e2e/specs/skill-workflow.ts` — 6 个前端 E2E 测试
- `scripts/test-cross-db-skill.yaml` — 测试用跨库 Skill 定义
- `scripts/setup-skill-workflow-testdata.sh` — 测试数据初始化脚本
- `.env` — 测试数据库连接配置
...
- 测试文件: `src-tauri/tests/skill_workflow_tests.rs`
- 测试 YAML: `scripts/test-cross-db-skill.yaml`
- 运行命令: `cargo test -p datazen --test skill_workflow_tests -- --nocapture`

### 前端 E2E 测试 (6 tests, 全部通过)

| TC | 描述 |
|----|------|
| SW-01 | Skill CRUD (创建/读取/列表/删除) via IPC |
| SW-02 | 执行 Skill 返回结构化结果 (steps/status/timing) |
| SW-03 | 执行历史自动记录 + 详情查询 + 清理 |
| SW-04 | 变量替换到 SQL 模板 |
| SW-05 | 条件步骤分支执行 (then_steps) |
| SW-06 | 无效 SQL 错误处理 (不崩溃) |

- 测试文件: `e2e/specs/skill-workflow.ts`
- 运行命令: `pnpm e2e -- --spec e2e/specs/skill-workflow.ts`

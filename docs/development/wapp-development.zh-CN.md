# DataZen 工作区应用开发指南（Workspace App / Wapp）

> 面向**第三方应用与主题开发者**。开发 DataZen 的运行时工作区应用（工作区页面 + 主题）**不需要**获取或编译 DataZen 源码——你只需要按本文构建一个 Wapp 包（一个目录 + `manifest.json`），最终用户即可在 DataZen 里安装它。本文从零演示「开发 → 打包 → 安装 → 调试」的完整闭环。
>
> 相关文档：
...
## 0. 什么是「工作区应用（Wapp）」，什么不是

DataZen 拥有正交的多维扩展体系，明确区分：

| | **工作区应用（Wapp，本文）** | 特权扩展点（EP） | 数据库驱动（Driver） |
|---|---|---|---|
| 面向用户 | 独立工作区页面、主题 | 编辑器高级增强（SQLEditor Pro）等 | 数据库连接与方言能力 |
| 需要源码 | **不需要** | 主仓库接口解耦，扩展独立发布 | 编译/链接时注入 |
...
> 仓库内开发者也可以直接运行 `pnpm wapp:pack`（执行 `node scripts/pack-wapp.mjs`），自动将 `packages/wapps/` 下的所有包打包到 `packages/wapps/dist/*.zip`。

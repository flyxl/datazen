...
- 状态：测试中；发现 1 项待验证验收缺口；共享页面最终接线留给 I 轨，桌面 E2E 留待 R 回归
- 编码 commit：c6da58b6
...
- 独立定向 Vitest：`pnpm exec vitest run src/lib/__tests__/schemaObjectSearch.test.ts src/lib/__tests__/tableSqlActions.test.ts`，2 个文件 / 13 个测试通过。
...
- `git diff --check c6da58b6^ c6da58b6` 与工作树 `git diff --check`：通过。
...
- 验收结论：未通过；`bugs.md` 登记 `v01x-object-actions-BUG-001`（搜索结果不保留命中原因/字段）。
...
- `searchSchemaObjects` 当前只返回是否匹配，不返回 `matchReason`/`matchedFields`；按 database、schema、connectionName 或所属 table 命中时，调用方不能直接显示匹配原因，见 `v01x-object-actions-BUG-001`。

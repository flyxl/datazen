...
### 第四轮独立验收（feature/v01x-query-cancel-plus，2026-08-31）

- 验收范围：先后独立审查 `92f86c58fc6319fe5cf26eb2d9b0518c6eeca6cd` 与 `79838de725a31629fd58b2a6b006d526d554b4e8` 的完整 diff，再按本轮新增要求复核事务、MariaDB、ReuseDriver wrapper 与 Host IPC。
- 静态协议验收通过：PG 事务分支在 `transactions[handle.id]` 持有的目标连接上执行 `pg_backend_pid()`，MySQL/MariaDB 事务分支在同一目标连接上执行 `CONNECTION_ID()`；之后分别绑定 `backend_pid/thread_id`，取消只经独立 control pool 执行 `pg_cancel_backend($1)` / `KILL QUERY <thread_id>`。pending、wrong-session、stale、并发 execution-id 隔离和终态 cleanup 均有 driver/Host 测试覆盖；未发现事务分支回退到 session-wide cancel 的路径。
- capability 矩阵验收通过：native PostgreSQL/MySQL、MariaDB concrete driver，以及 QuestDB、Cloudberry、Doris、StarRocks、Manticore、OceanBase Oracle-mode 六个 ReuseDriver wrapper 的 factory/concrete precise capability 一致为 true；ReuseDriver 计算能力时仍与父 driver 的 `supports_query_execution_cancel()` 求交，legacy `supports_cancel_query` 保持独立。Host `cancel_query_impl` 仅检查 precise capability 并调用 `cancel_query_with_execution`，未调用旧 `cancel_query(handle)`。
- Rust 独立实测：`cargo test -p datazen-driver-api` 为 101 passed / 0 failed / 2 ignored doc-tests；`cargo test -p datazen-driver-mysql` 为 72 unit + 8 integration = 80 passed / 0 failed；`cargo test -p datazen-driver-postgres` 为 86 unit + 12 integration + 2 doc-tests = 100 passed / 0 failed；Host 首次沙箱运行因 WireMock 端口权限为 1147 passed / 46 failed / 2 ignored，提权重跑为 1193 passed / 0 failed / 2 ignored。
- 覆盖率（`cargo llvm-cov --lib --summary-only`）：driver-api 总体 lines 73.34% / functions 62.18% / regions 74.31%，其中 `reuse.rs` lines 54.95%；MySQL 总体 lines 55.91% / functions 42.74% / regions 53.91%，其中 `mysql.rs` lines 32.25%；PostgreSQL 总体 lines 57.48% / functions 49.54% / regions 58.90%，其中 `postgres.rs` lines 40.30%；Host 总体 lines 78.62% / functions 72.78% / regions 78.53%，`commands/query.rs` lines 85.12%、`commands/driver_command.rs` 84.43%。驱动低覆盖主要是无真实数据库时连接/stream/控制 SQL 分支未执行。

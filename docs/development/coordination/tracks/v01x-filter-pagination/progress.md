...
| D-E2E-004 | filter 请求 loading 时，FilterEditor/FilterBar/Pagination 展示 busy 并禁用变更控件 | 留待 R 回归 | FilterBar/Pagination 通过；FilterEditor 完成 chip 删除复现 `v01x-filter-pagination-BUG-001`，修复后留待 R |
...
- `npx vitest run --coverage` 定向 4 个修改生产文件：4 files、56 tests passed、0 failed；行覆盖率：`filterExpression.ts` 95.00%、`FilterBar.tsx` 83.33%、`FilterEditor.tsx` 88.98%、`Pagination.tsx` 96.66%，合计 92.81%。语句/分支/函数合计分别为 89.43%/84.00%/92.68%。
...
- `git diff --check` 及 `git diff d8e9c59b^ d8e9c59b --check`：通过。
...
- Bug：`FilterConditionChip` 在 `loading` 时未禁用编辑/删除入口，详见 `v01x-filter-pagination-BUG-001`；本轮不修复。

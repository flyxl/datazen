...
- 验证记录：修复 `chartAvailable` 和 view resolution，使可推断结果不再依赖已保存的 `chartConfig`；无 config 时交由现有 `ChartView` 生成推荐配置。独立复测（2026-08-31）：定向 ResultWorkspace Vitest 3 个测试文件、15 个测试全通过；缺省 config 入口可用且 config callback 正常，确认已修复。
...
- 验证记录：新增独立 Chart 数据点 handler，严格先调用 `onViewChange('table')` 再调用 `onRowDetail(rowIndex)`；Table 行详情仍只转发 row detail，并覆盖缺失回调不抛异常。独立复测（2026-08-31）：定向 ResultWorkspace Vitest 3 个测试文件、15 个测试全通过，调用顺序断言通过，确认已修复。

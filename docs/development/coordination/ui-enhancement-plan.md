...
### Wave 2（2 轨并行：强调色收敛 + 导航外壳统一）

| Track | 任务摘要 | 主要写路径 |
|-------|----------|------------|
| **ui-accent-tokens** | 统一强调色到 `accent` token：清理 `focus:border-blue-500` 与 `text-blue-500` 等硬编码，Badge/Tag 与危险色统一使用语义 token，补齐样式回归测试 | `src/windows/settings/`、`src/components/` 等相关样式文件 |
| **ui-shell-unification** | 外壳与导航打磨：SettingsPage 接入主 TitleBar 与菜单，ContentStatusBar 快捷键平台感知（Cmd/Ctrl 自适应）与上下文匹配 | `src/windows/connection/ConnectionPage.tsx`、`src/windows/settings/SettingsPage.tsx`、`src/windows/connection/ContentStatusBar.tsx` |

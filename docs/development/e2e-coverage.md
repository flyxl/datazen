...
| 数据同步完整用户旅程（compare→preview→execute→回查） | `journeys/data-sync-journey.ts` (DS-JOURNEY-PG / DS-JOURNEY-MYSQL) | Covered（PG + MySQL 双驱动；校验/Review/Execute 全分支；每步 `--screenshot`） |

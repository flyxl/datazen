...
| 数据同步边界与异常（selectBoth / same endpoint / unsupported pair / delete confirm / post-compare filters） | `data-sync-edge-cases.ts` (DS-EDGE-001~011) | Covered（PG 夹具；每步 `--screenshot`） |
| 数据同步完整用户旅程（compare→preview→execute→回查） | `journeys/data-sync-journey.ts` (DS-JOURNEY) | Covered（PG 夹具；每步 `--screenshot`） |

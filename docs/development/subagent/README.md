...
| **测试子代理** (Tester) | [tester.md](tester.md) | 代码审查 + 独立复验 + **新增集成/E2E 测试编写** + Bug 登记 | **必须全新实例，禁止复用编码代理**；只测不修业务代码（但必须写新测试）；零信任 |
...
[调度时序 — 即时测试]
Wave N 编码并行派发 → Coder-A 完成 → 立即启动 Tester-A（不等 Coder-B/C/D）
                    → Coder-B 完成 → 立即启动 Tester-B
                    → ... → 全部 Tester 完成 → 逐轨合入 → R 阶段回归（per-Wave）
```

...
| 步骤 | 做什么 |
|------|--------|
| **对比** | 选源连接、目标连接与表，查看列/索引差异 |
| **计划** | 生成部署 SQL（默认仅加法） |
| **审阅 / 部署** | 确认事务与破坏性确认词后，在目标库执行 |
...
|----|------|
| 源连接 | 期望的结构（desired） |
| 目标连接 | 要改成期望态的库（apply site） |

界面标签：

- **目标缺失（将 ADD）**：源有、目标没有 → 计划里一般是 `ADD COLUMN`
- **目标多余（将 DROP）**：目标有、源没有 → 需开启「允许破坏性变更」才会进计划
- **变更列**：类型 / 可空 / 主键标记不一致
...
1. 打开 **Schema Diff** 窗口  
2. 选择 **源**、**目标**（不能是同一连接）  
3. 在「表名」框填写一张或多张表（每行一个，或逗号分隔）  
4. 点击 **对比** 查看差异摘要  
5. 点击 **生成部署脚本**  
6. 在计划列表中核对每条 SQL 与风险标记（`additive` / `destructive` / `rewrite`）  
7. 进入 **审阅 / 部署**，按需勾选事务与回滚完整性要求  
...
| **Data Synchronization** | 结构完全一致 + 相同 PK → 行 Diff → 审查 → Execute | [data-sync-guide.md](./data-sync-guide.md) |
| **Data Transfer** | 异构 / 结构不同 / 无 PK → 单向搬运（V1 基础） | [data-transfer-guide.md](./data-transfer-guide.md) |

...
| Redis 浏览器 | ✅（全部类型 + 批量操作 + 命令台） | ✅ **E1–E4 已实现**：五类型 CRUD + 批量 + Size + 门控 Flush；深控制台 + Monitor（Info/Memory/Slowlog/Stream 概览）；Standalone/Cluster/Sentinel + mTLS 向导；Pub/Sub 页、RedisJSON（模块探测）、Stream 消费组、DUMP/RESTORE 导入导出。**已知限制**：Sentinel TLS 仅系统信任库（自定义 CA/客户端证书不作用于 Sentinel 节点连接）；Cluster Pub/Sub 走独立非 cluster 连接；RedisJSON 需服务器加载 ReJSON 模块 |

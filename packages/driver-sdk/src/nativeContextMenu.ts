...
import type { NativeMenuItemDef } from './types/menu';

export type ContextMenuPosition = { x: number; y: number };

/**
 * Host-provided web context-menu mount point. The host binds its
 * `contextMenuStore` bridge at module load (schemaStoreBridge pattern), so
 * drivers can call `showNativeContextMenu` without importing host code.
 */
export type ContextMenuBridge = {
  show: (items: NativeMenuItemDef[], pos: ContextMenuPosition) => void;
  hide: () => void;
};

let boundBridge: ContextMenuBridge | null = null;

export function bindContextMenuBridge(bridge: ContextMenuBridge): void {
  boundBridge = bridge;
}

function getBridge(): ContextMenuBridge {
  if (!boundBridge) {
    throw new Error('ContextMenu has not been bound to driver-sdk yet.');
  }
  return boundBridge;
}

// Keep a request revision here as well so a menu opened just before navigation
// cannot mount after the target page has already changed. The bridge call is
// deferred one microtask to preserve the lazy-mount cancellation semantics of
// the previous dynamic-import implementation.
...
export function showNativeContextMenu(items: NativeMenuItemDef[], pos: ContextMenuPosition): void {
  ...
  void Promise.resolve().then(() => {
    cleanup?.();
    if (request !== latestMenuRequest) return;
    bridge.show(items, pos);
  });
}

/** Hide the menu and invalidate any menu request still waiting to mount. */
export function hideNativeContextMenu(): void {
  latestMenuRequest += 1;
  pendingPointerCleanup?.();
  // No bridge bound ⇒ no menu could ever have been shown; nothing to hide.
  if (!boundBridge) return;
  const bridge = boundBridge;
  void Promise.resolve().then(() => bridge.hide());
}

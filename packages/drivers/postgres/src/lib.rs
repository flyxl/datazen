...
impl DatabaseDriverFactory for PostgresFactory {
    fn create(&self) -> Arc<dyn DatabaseDriver> { Arc::new(PostgresDriver::new()) }
...
impl DatabaseDriverFactory for QuestDbFactory {
    fn create(&self) -> Arc<dyn DatabaseDriver> { Arc::new(ReuseDriver::new_with_precise_cancel(Arc::new(PostgresDriver::new()), "questdb", true)) }
...
impl DatabaseDriverFactory for CloudberryFactory {
    fn create(&self) -> Arc<dyn DatabaseDriver> { Arc::new(ReuseDriver::new_with_precise_cancel(Arc::new(PostgresDriver::new()), "cloudberry", true)) }
...
mod tests {
    ...
    fn postgres_factory_advertises_precise_cancellation_by_backend_compatibility() {
        let factories: [&dyn DatabaseDriverFactory; 3] = [&PostgresFactory, &QuestDbFactory, &CloudberryFactory];

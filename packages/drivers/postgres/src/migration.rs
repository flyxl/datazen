        ...
        match op {
            MigrationOperation::CreateTable { table, columns, primary_keys } => {
                let cols = columns.iter().map(|c| {
                    ...
                    format!("{} {}{}{}", qi(&c.name), c.data_type, default, if c.nullable { "" } else { " NOT NULL" })
                }).collect::<Vec<_>>();
            ...
            MigrationOperation::AddColumn { table, column } => {
                let default = column.default_value.as_deref().map(|v| format!(" DEFAULT {v}")).unwrap_or_default();
                Ok(MigrationStatement { sql: format!("ALTER TABLE {} ADD COLUMN {} {}{}{}", qi(table), qi(&column.name), column.data_type, default, if column.nullable { "" } else { " NOT NULL" }), risk: MigrationRisk::Additive, rollback_sql: Some(format!("ALTER TABLE {} DROP COLUMN {}", qi(table), qi(&column.name))), summary: format!("ADD COLUMN {}.{}", table, column.name) })
            }
            ...
            MigrationOperation::AlterColumnType { table, column, to, .. } => Ok(MigrationStatement { sql: format!("ALTER TABLE {} ALTER COLUMN {} TYPE {}", qi(table), qi(column), to), risk: MigrationRisk::Rewrite, rollback_sql: None, summary: format!("ALTER TYPE {}.{}", table, column) }),
            MigrationOperation::SetNullable { table, column, nullable } => Ok(MigrationStatement { sql: if *nullable { format!("ALTER TABLE {} ALTER COLUMN {} DROP NOT NULL", qi(table), qi(column)) } else { format!("ALTER TABLE {} ALTER COLUMN {} SET NOT NULL", qi(table), qi(column)) }, risk: if *nullable { MigrationRisk::Additive } else { MigrationRisk::Rewrite }, rollback_sql: None, summary: format!("ALTER NULLABILITY {}.{}", table, column) }),
            MigrationOperation::SetComment { table, column, to, .. } => Ok(MigrationStatement { sql: format!("COMMENT ON COLUMN {}.{} IS {}", qi(table), qi(column), to.as_deref().map(|v| format!("'{}'", v.replace('\'', "''"))).unwrap_or_else(|| "NULL".into())), risk: MigrationRisk::Additive, rollback_sql: None, summary: format!("ALTER COMMENT {}.{}", table, column) }),
            MigrationOperation::SetAutoIncrement { .. } | MigrationOperation::DropPrimaryKey { .. } => Err("PostgreSQL migration requires constraint/identity metadata not present in this operation".into()),
        }
    }
}
...
impl MigrationCapabilities for PostgresMigrationCapabilities {
    ...
    fn requires_table_rebuild(&self, operation: &MigrationOperation) -> bool {
        matches!(operation, MigrationOperation::AlterColumnType { .. } | MigrationOperation::SetNullable { nullable: false, .. })
    }
}
...
mod tests {
    ...
    #[test] fn unsupported_constraint_operations_are_not_advertised() {
        let op=MigrationOperation::DropPrimaryKey { table:"users".into(), columns:vec!["id".into()] };
        assert!(!PostgresMigrationCapabilities.supports(&op));

...
impl MigrationRenderer for MysqlMigrationRenderer {
    fn render(&self, op: &MigrationOperation) -> Result<MigrationStatement, String> {
        ...
        match op {
            ...
            MigrationOperation::SetNullable { table, column, nullable } => Ok(MigrationStatement { sql: format!("ALTER TABLE {} MODIFY COLUMN {} {}{}", qi(table), qi(column), "INT", if *nullable { " NULL" } else { " NOT NULL" }), risk: if *nullable { MigrationRisk::Additive } else { MigrationRisk::Rewrite }, rollback_sql: None, summary: format!("ALTER NULLABILITY {}.{}", table, column) }),

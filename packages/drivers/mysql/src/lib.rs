...
impl DatabaseDriverFactory for MysqlFactory {
    ...
    fn supports_cancel_query(&self) -> bool { true }
    fn supports_query_execution_cancel(&self) -> bool { true }
}
datazen_driver_api::register_driver!(&MysqlFactory);

struct MariadbFactory;
impl DatabaseDriverFactory for MariadbFactory {
    fn create(&self) -> Arc<dyn DatabaseDriver> { Arc::new(MysqlDriver::new(true)) }
    fn driver_id(&self) -> &'static str { "mariadb" }
    fn supports_explain(&self) -> bool { true }
    fn supports_cancel_query(&self) -> bool { false }
    fn supports_query_execution_cancel(&self) -> bool { true }
}
...
impl DatabaseDriverFactory for DorisFactory {
    fn create(&self) -> Arc<dyn DatabaseDriver> { Arc::new(ReuseDriver::new_with_precise_cancel(Arc::new(MysqlDriver::new(false)), "doris", true)) }
...
impl DatabaseDriverFactory for StarrocksFactory {
    fn create(&self) -> Arc<dyn DatabaseDriver> { Arc::new(ReuseDriver::new_with_precise_cancel(Arc::new(MysqlDriver::new(false)), "starrocks", true)) }
...
impl DatabaseDriverFactory for ManticoreFactory {
    fn create(&self) -> Arc<dyn DatabaseDriver> { Arc::new(ReuseDriver::new_with_precise_cancel(Arc::new(MysqlDriver::new(false)), "manticore", true)) }
...
impl DatabaseDriverFactory for ObOracleFactory {
    fn create(&self) -> Arc<dyn DatabaseDriver> { Arc::new(ReuseDriver::new_with_precise_cancel(Arc::new(MysqlDriver::new(false)), "ob_oracle", true)) }
...
mod tests {
    ...
    fn mysql_factory_advertises_precise_cancellation_for_mysql_family_servers() {
        let factories: [&dyn DatabaseDriverFactory; 6] = [&MysqlFactory, &MariadbFactory, &DorisFactory, &StarrocksFactory, &ManticoreFactory, &ObOracleFactory];

# DataZen Workspace Apps (wapps)

运行时 Workspace App 应用（UI 页面 + 主题）的**源码包目录**。每个子目录是一个完整、可直接安装的 Workspace App 包，遵循统一插件系统规范（详细设计见 [docs/architecture/backend/wapps.md](../../docs/architecture/backend/wapps.md)）。

> 历史注记：本目录取代 `packages/themes/`（旧 v1 ThemePack 源码树）与 `packages/extensions/`。社区主题 `community.slate-blue` 已按新规范改造为纯主题 wapp；`fixtures/themes/community.fixture-dark` 保留原位，仅服务遗留 theme 模块的单测。

## 包结构

```text
packages/wapps/<publisher>.<name>/
├── manifest.json          # 必需；apiVersion 必须等于宿主 WAPP_API_VERSION (=2)
...
```jsonc
{
  ...
  "apiVersion": 2,
  "icon": "assets/icon.png",       // 可选：应用方形图标（png/webp/svg），显示在应用管理列表
...
> `icon`（可选，应用列表用）：相对路径的方形品牌图标，`png|webp|svg`；与 `entry`/`pages[].icon` 一样走规则 5 安全路径校验，声明了就必须存在。
...
2. 左侧边栏 → **应用**（Wapps） → **安装应用…**
3. 在弹窗中粘贴包的**绝对目录路径**（如 `…/DataZen/packages/wapps/datazen.playground`），或使用预打包的 zip：**`packages/wapps/dist/<id>.zip`**（该目录已 gitignore）。手动重打：

   ```bash
   pnpm wapp:pack datazen.playground
   ```

4. 预览确认（名称/版本/权限清单）→ Install → 卡片出现即成功
5. 测试页面：左侧边栏 → **Workspace** → 点击 *Playground* 打开 Tab
6. 测试主题：设置 → **外观** → 选择 *Playground Night* 或 *Slate Blue*

> 注意：安装是**拷贝语义**——修改源码后需重新安装才能生效（同 id 重装会覆盖并备份旧包为 `{id}.old.bak`）。停用/卸载在应用管理页操作。

...
mod tests {
    ...
    async fn resolve_session_for_connection_reuses_existing_runtime() {
        ...
        let (db_session_id, _driver, returned) = mgr
            .resolve_session_for_connection("cfg-2")
            .await
            .unwrap();
    ...
    async fn resolve_session_for_connection_rejects_db_session_id() {
        ...
        match mgr
            .resolve_session_for_connection(&db_session_id)
            .await
        {
            Err(e) => assert!(matches!(
                e,
                ConnectionError::ConnectionConfigNotFound(_)
            )),
    ...
    async fn resolve_session_for_connection_creates_session_from_connection_id() {
        ...
        let (db_session_id, driver, handle) = mgr
            .resolve_session_for_connection("cfg-fb")
            .await
            .unwrap();

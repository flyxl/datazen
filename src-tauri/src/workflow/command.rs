...
use super::model::ErrorHandlingConfig;
...
pub struct WorkflowCommandStep {
    ...
    #[serde(default)]
    pub on_error: Option<ErrorHandlingConfig>,
}
...
impl WorkflowCommandStep {
    ...
    /// the on-disk YAML. Legacy database selection is preserved as command
    /// input so the migration does not silently lose per-step database state.
...
mod tests {
    ...
    fn legacy_query_normalizes_to_query_command() {
        let step = WorkflowCommandStep::from_legacy_query(
            "users", "SELECT id FROM users", Some("mysql-prod".into()), None, Some(10), None,
        );
    ...
    fn legacy_query_preserves_database_selection() {
        let step = WorkflowCommandStep::from_legacy_query(
            "users", "SELECT id FROM users", Some("mysql-prod".into()), Some("reporting".into()), None, None,
        );
        assert_eq!(step.command, "query");
    ...
    fn explicit_connection_overrides_workflow_connection() {
        let step = WorkflowCommandStep::new(
            "aggregate", "aggregate", Some("mongo-prod".into()), serde_json::json!({"collection": "orders"}),
        );
        assert_eq!(step.effective_connection(Some("mysql-prod")), Some("mongo-prod"));
    }

    #[test]
    fn absent_connection_inherits_workflow_connection() {
        let step = WorkflowCommandStep::new(
            "query", "query", None, serde_json::json!({"sql": "SELECT 1"}),
        );
        assert_eq!(step.effective_connection(Some("mysql-prod")), Some("mysql-prod"));
    }
    ...
    fn command_step_serializes_with_camel_case_fields() {
        let step = WorkflowCommandStep::new(
            "query", "query", Some("mysql-prod".into()), serde_json::json!({"sql": "SELECT 1"}),
        );

    ...
    pub async fn execute_with_options(
        ...
        for var in &workflow.variables {
            ...
            if var.required.unwrap_or(false)
                && context.variables.get(&var.name).map_or(true, |v| v.is_empty())
            {
                return Err(format!("Required variable '{}' is missing", var.name));
            }
        }

        let workflow_connection = workflow
            .connection
            .as_deref()
            .or(connection_id);
            ...
            for step in steps {
                ...
                match step {
                    WorkflowStep::Condition { id, expr, then_steps, else_steps } => {
                        ...
                        let branch = if evaluate_condition(&resolved, context) {
                            then_steps.as_slice()
                        } else {
                        ...
                        step_results.push(StepExecutionResult {
                            ...
                            result: Some(serde_json::json!({"condition": evaluate_condition(&resolved, context)})),
                            execution_time_ms: 0,
                            error: None,
    ...
    async fn handle_step_error(
        ...
        match step.on_error_strategy().unwrap_or_else(|| default_strategy.clone()) {
            ...
            ErrorStrategy::Fallback { steps } => {
                Self::execute_steps(&steps, app_state, connection_id, context, step_results, default_strategy, global_timeout_secs, global_start, options).await
            }
        }
    }
    ...
    async fn execute_single_step(
        ...
        match step {
            ...
            WorkflowStep::Command { id, command, connection, input, timeout_secs, on_error } => {
                ...
                let step = WorkflowCommandStep {
                    id: id.clone(), command: command.clone(), connection: resolved_connection,
                    input: std::mem::take(&mut resolved_input), timeout_secs: *timeout_secs, on_error: on_error.clone(),
                };
                Self::execute_command_step(step, app_state, workflow_connection, context, options, None).await
            }
    ...
    async fn execute_command_step(
        ...
        if step.command == "query" {
            if let Some(sql) = step.input.get("sql").and_then(|v| v.as_str()) {
                ...
                input["sql"] = serde_json::Value::String(clean);
                ...
                return Ok(StepExecutionResult {
                    ...
                    connection_name: Some(conn_name), sql_executed: legacy_sql_template.map(str::to_owned).or_else(|| Some(clean)),
                });
            }
        }
...
fn normalize_query_result(data: &serde_json::Value) -> Result<serde_json::Value, String> {
    ...
    let result = multi.results.into_iter().next()

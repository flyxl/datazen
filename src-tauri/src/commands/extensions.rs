...
static EXTENSION_PICK_SESSIONS: LazyLock<tokio::sync::Mutex<HashMap<String, ExtensionPickSession>>> =
...
async fn take_pick_session(token: &str) -> Result<PathBuf, CommandError> {
    ...
    sessions
        ...
        .ok_or_else(|| {
            CommandError::NotFound("Extension pick session not found or expired".into())
        })
}
    ...
    match kind {
        ExtensionPackageKind::Zip => {
            super::dialog::open_file(
                app,
                vec![("Plugin package".into(), vec!["zip".into()])],
            )
            .await
        }
...
pub async fn inspect_extension_package_with_dialog(
    ...
    let manifest =
        inspect_extension_package_impl(source.to_string_lossy().into_owned()).await?;
...
mod tests {
    ...
    async fn pick_session_is_consumed_once_on_install_path_resolution() {
        let tmp = TempDir::new().unwrap();
        let token = insert_pick_session(tmp.path().to_path_buf())
            .await
            .unwrap();

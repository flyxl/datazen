...
impl MigrationOperation {
    ...
    pub fn to_driver_api(&self) -> datazen_driver_api::MigrationOperation {
        ...
        let col = |c: &ColumnSnapshot| MigrationColumn {
            ...
            default_value: c.default_value.clone(), comment: c.comment.clone(), is_auto_increment: c.is_auto_increment,
        };
        match self {
...
mod tests {
    ...
    fn column(name: &str) -> ColumnSnapshot {
        ColumnSnapshot { name: name.into(), data_type: "int".into(), nullable: true, default_value: None, comment: None, is_primary_key: false, is_auto_increment: false }
    }
    #[test]
    fn destructive_operations_are_marked_destructive() {
        assert_eq!(MigrationOperation::DropColumn { table: "t".into(), column: column("x") }.risk(), StatementRisk::Destructive);

...
mod tests {
    ...
    fn same_bucket_ops_are_sorted_by_table_and_key() {
        ...
        assert!(matches!(&sorted[0], MigrationOperation::SetDefault { table, column, .. } if table == "a" && column == "x"));
        assert!(matches!(&sorted[1], MigrationOperation::SetDefault { table, column, .. } if table == "a" && column == "y"));
        assert!(matches!(&sorted[2], MigrationOperation::SetDefault { table, column, .. } if table == "b" && column == "z"));
    }
    ...
    fn drop_bucket_sorts_deterministically() {
        ...
        let sorted = resolve_dependencies(ops);
        assert!(matches!(&sorted[0], MigrationOperation::DropColumn { column, .. } if column.name == "a"));
        assert!(matches!(&sorted[1], MigrationOperation::DropColumn { column, .. } if column.name == "z"));
    }
}

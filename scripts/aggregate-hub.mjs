...
const waveSection = tracks.length > 0
  ? extractSection('波次记录', '## 波次记录\n\n（待补充）')
  : '## 波次记录\n\n- 所有历史波次均已闭环并合入主集成分支。';
const riskSection = tracks.length > 0
  ? extractSection('跨轨风险', '## 跨轨风险\n\n- 无明显跨轨冲突')
  : '## 跨轨风险\n\n- 无活跃跨轨冲突。';
const rSection = tracks.length > 0
  ? extractSection('R 阶段清单', '## R 阶段清单\n\n- [ ] 全量回归测试')
  : '## R 阶段清单\n\n- [x] 全量回归与构建健全性验证通过';

// 生成功能总览表
let overviewTable = `## 功能总览表\n\n`;
if (tracks.length === 0) {
  overviewTable += `*暂无活跃开发轨道*\n`;
} else {
  overviewTable += `| Track | 任务 | 状态 | 编码 Commit | 测试 Commit | 合并 Commit |\n|-------|------|------|------------|------------|------------|\n`;
...
if (tracks.length === 0) {
  ...
  lockTable += `| Track | 写锁代理 | Worktree | Branch | Phase | 最后心跳 |\n|-------|----------|----------|--------|-------|----------|\n`;
  for (const t of tracks) {

 ...
 *
 * Usage:
 *   node scripts/pack-wapp.mjs            # pack every package
 *   node scripts/pack-wapp.mjs datazen.playground   # a single package by id
 */
...
const WAPPS_DIR = resolve(ROOT, 'packages/wapps');
const OUT_DIR = join(WAPPS_DIR, 'dist');

function listPackageDirs() {
  return readdirSync(WAPPS_DIR, { withFileTypes: true })
...
function readId(pkgDir) {
  const manifestPath = join(WAPPS_DIR, pkgDir, 'manifest.json');
...
function pack(pkgDir, id) {
  const source = join(WAPPS_DIR, pkgDir);
  ...
  console.log(`  ${id}  ->  ${join('packages/wapps/dist', `${id}.zip`)}`);
  return true;
}

const all = listPackageDirs();
if (all.length === 0) {
  console.error(`No package directories under ${WAPPS_DIR}.`);
  process.exit(1);
}
...
console.log(`Packing ${targets.length} wapp${targets.length === 1 ? '' : 's'} to ${OUT_DIR}`);
let ok = 0;

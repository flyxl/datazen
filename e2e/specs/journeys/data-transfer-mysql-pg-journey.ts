/**
 * Data Transfer cross-dialect journey: MySQL → PostgreSQL (IR path, data mode).
 *
 * Requires `e2e/setup-sync-dbs.sh` (datazen_sync_mysql_tgt + datazen_sync_tgt).
 */
...
describe('数据传输 MySQL→PG 跨方言旅程 (DT-MYSQL-PG-JOURNEY)', () => {
  ...
  const SRC_NAME = `DT-MySQL-Src-${STAMP}`;
  const TGT_NAME = `DT-PG-Tgt-${STAMP}`;
  const TABLE = `dt_mysql_pg_${STAMP}`;
  ...
  it('Step 1: 通过 URL 打开数据传输窗口', async () => {
    ...
    await captureJourneyStep('dt-mysql-pg-01-window-open', 0, true);
  });

  it('Step 2: 选择 MySQL 源与 PG 目标并显示 IR 路径', async () => {
    await openDataTransferWindow();
    ...
    await captureJourneyStep('dt-mysql-pg-02-ir-path', 0, true);
  });

  it('Step 3: 进入 setup 并选择 data 模式', async () => {
    await clickNext('dt-mysql-pg-03-setup-step');
    ...
    await captureJourneyStep('dt-mysql-pg-04-data-mode', 0, true);
  });
  ...
  it('Step 5: 推进映射与预览直至执行步骤', async () => {
    for (let i = 0; i < 8; i++) {
      ...
      if (await execute.isExisting().catch(() => false)) {
        await captureJourneyStep('dt-mysql-pg-07-execute-ready', 0, true);
        break;
      }
      const preview = await $('[data-testid="data-transfer-preview"]');
      if (await preview.isDisplayed().catch(() => false)) {
        await captureJourneyStep('dt-mysql-pg-07-preview', 0, true);
      }
      const mapping = await $('[data-testid="data-transfer-mapping-step"]');
      if (await mapping.isDisplayed().catch(() => false)) {
  ...
  it('Step 6: 执行跨方言传输并验证 PG 目标行数', async () => {
    const execute = await $('[data-testid="data-transfer-execute"]');

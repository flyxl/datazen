...
import { createAndConnectPostgreSQL, closeExtraWindows, openQueryTab } from '../helpers.js';
...
describe('PostgreSQL 多库会话 UI (F4-E2E)', () => {
  ...
  before(async () => {
    ...
    if (!(await postgresReachable())) {
      console.warn(`⏩ Skipping PostgreSQL multi-db E2E: ${PG_HOST}:${PG_PORT} unreachable`);
      shouldSkip = true;
      return;
    }

/**
 * MySQL multi-database session UI (F2).
 *
 * Covers:
 ...
 *
 * Credentials: E2E_MYSQL_* (see e2e/.env.example). Skips gracefully when
 * MySQL is unreachable or E2E_SKIP_MYSQL=1.
 */
...
const EXPECTED_DB = process.env.E2E_MYSQL_DB || 'datazen_test';

function skipRequested(): boolean {
  return process.env.E2E_SKIP_MYSQL === '1';
}

async function mysqlReachable(timeoutMs = 2000): Promise<boolean> {
  return new Promise((resolve) => {
    const sock = createConnection({ host: MYSQL_HOST, port: MYSQL_PORT });
...
describe('MySQL 多库会话 UI (F2-E2E)', () => {
  let mainWindow: string;
  let shouldSkip = false;

  before(async () => {
    if (skipRequested()) {
      console.warn('⏩ Skipping MySQL multi-db E2E: E2E_SKIP_MYSQL=1');
      shouldSkip = true;
      return;
    }
    ...
    try {
      ...
      console.warn('⏩ Skipping MySQL multi-db E2E: connect failed', err);
      shouldSkip = true;
    }
  });
  ...
  it('多库时 QueryPanel 应显示数据库选择器 (F2-E2E-003)', async function () {
    if (shouldSkip) return this.skip();

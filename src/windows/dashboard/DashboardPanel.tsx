...
export function DashboardPanel({ initialDashboardId, onDashboardChange }: DashboardPanelProps) {
  ...
  const switchDashboard = useCallback(
    (id: string) => {
      ...
      const board = useDashboardStore.getState().list.find((b) => b.id === id);
      if (board) onDashboardChange?.(id, board.name);
    },
    [onDashboardChange],
  );
  ...
  const handleCreatePanel = useCallback(async () => {
    ...
    await fetchDashboards();
    switchDashboard(board.id);
  }, [fetchDashboards, saveDashboard, switchDashboard, t]);
  ...
  }, [current, deleteDashboard, switchDashboard]);
  ...
  const handleImport = useCallback(async () => {
    ...
    if (imported.id === dashboardId) {
      ...
      switchDashboard(imported.id);
    }
  }, [dashboardId, loadDashboard, switchDashboard]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col" data-testid="dashboard-panel">
      {/* Tab bar */}
      {tabItems.length > 0 && (
            ...
            {tabItems.map((board) => {
              ...
              return (
                <button
                  ...
                  onClick={() => switchDashboard(board.id)}
                >

...
export function DataSyncWindow() {
  ...
  const handleResumeClick = useCallback(
    async (task: SyncTask) => {
      ...
      try {
        ...
        setErrorMsg(
          `${t('sync.checkConflictFailed')} ${e instanceof Error ? e.message : String(e)}`,
        );
        setErrorOpen(true);
      }
    },
  ...
  const selectAll = useCallback(() => {
    setSelectedTables(
      new Set(comparisons.filter((r) => r.status !== 'target_only').map((r) => r.table)),
    );
  }, [comparisons]);
    ...
    return mismatches.map((m) => (
      <div key={m.key} className="border-t border-edge px-3 py-2 text-xs">
        <div className="mb-1 flex items-center gap-2">
          <span className="rounded bg-surface-raised px-1.5 py-0.5 font-mono text-[10px] text-fg-muted">
            {m.key}
          </span>
  ...
  const statusLabel = (status: string) => {
    switch (status) {
      case 'identical':
        return t('sync.identical');
      case 'different':
        return t('sync.different');
      case 'source_only':
        return t('sync.sourceOnly');
      case 'target_only':
        return t('sync.targetOnly');
      default:
        return status;
    }
  };
    ...
    <div className="flex h-screen min-h-0 flex-col bg-surface text-fg">
          ...
          <div className="flex h-full min-h-0">
            <div
              className={cn(
                ...
                detailTable && 'border-r border-edge',
              )}
            >
              <div className="mb-3 flex items-center gap-2">
                ...
                <Button variant="ghost" className="text-xs" onClick={deselectAll}>
                  {t('common.deselectAll')}
                </Button>
                <div className="flex-1" />
                <span className="text-xs text-fg-muted">
                  {comparisons.filter((r) => r.status === 'identical').length} {t('sync.identical')}{' '}
                  / {comparisons.filter((r) => r.status === 'different').length}{' '}
                  {t('sync.different')} /{' '}
                  {comparisons.filter((r) => r.status === 'source_only').length}{' '}
                  {t('sync.sourceOnly')} /{' '}
                  {comparisons.filter((r) => r.status === 'target_only').length}{' '}
                  {t('sync.targetOnly')}
                </span>
              </div>
                    ...
                    <div className="space-y-3 text-xs">
                      {schemaDiff.sourceDdl && (
                        <section>
                          ...
                          <pre className="overflow-x-auto rounded border border-edge bg-surface-alt p-2 font-mono text-[10px] text-fg-secondary">
                            {schemaDiff.sourceDdl}
                          </pre>
                        </section>
                      )}
      ...
      {(syncState === 'compared' || syncState === 'syncing' || syncState === 'done') && (
        <div className="flex shrink-0 items-center gap-3 border-t border-edge px-6 py-3">
          <span className="text-xs text-fg-muted">
            {t('sync.selected', { selected: selectedTables.size, total: comparisons.length })}
          </span>
          <div className="flex-1" />
          <Button
            variant="primary"
            onClick={() => void handleSync()}
            disabled={syncState === 'syncing' || selectedTables.size === 0}
          >
            {syncState === 'syncing' ? (
              <Loader2 className="h-4 w-4 animate-spin" />
            ) : (

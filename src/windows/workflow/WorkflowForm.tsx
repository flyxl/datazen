        ...
        if (type === 'boolean') {
          return (
            <label key={field.name} className="flex items-center gap-2 text-xs text-fg">
              <input type="checkbox" checked={Boolean(current)} onChange={(e) => update(field.name, e.target.checked)} />
          ...
          return (
            <div key={field.name}>
              ...
              <textarea
                ...
                value={typeof current === 'string' ? current : JSON.stringify(current ?? (type === 'array' ? [] : {}), null, 2)}
                onChange={(e) => {
                  const text = e.target.value;
                  setRawJson(text);
                  try { update(field.name, JSON.parse(text)); } catch { /* keep editing */ }
                }}
              />
            </div>
          );
        ...
        return (
          <div key={field.name}>
            ...
            <input
              ...
              onChange={(e) => update(field.name, isNumber && e.target.value !== '' ? Number(e.target.value) : e.target.value)}
              placeholder={typeof field.schema.description === 'string' ? field.schema.description : undefined}
            />
          </div>
        );
      })}
    ...
    <div className="w-full max-w-3xl mx-auto p-6 space-y-4">
      ...
      <div>
        ...
        <Select
          ...
          options={[{ value: '', label: 'No default connection' }, ...connections.map((c) => ({ value: c.id, label: c.name }))]}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => onDraftChange({ ...draft, steps: [...draft.steps, { type: 'command', id: `step${draft.steps.length + 1}`, command: '', input: {} }] })} className="text-accent text-xs hover:underline">+ {t('workflows.addStep')}</button>
        </div>
            ...
            <div key={i} className="mb-3 rounded-lg border border-edge p-3 space-y-2">
              <div className="flex items-center gap-2">
                ...
                {(step.type === 'query' || step.type === 'command') && connections.length > 0 && (
                  <Select value={step.connection ?? ''} options={[{ value: '', label: draft.connection ? 'Inherited' : 'Select connection' }, ...connections.map((c) => ({ value: c.id, label: c.name }))]} onChange={(v) => setStep(i, { connection: v || undefined, command: v ? undefined : step.command })} className="!h-7 !text-xs flex-1" />
                )}
                ...
                <>
                  <div>
                    ...
                    <Select
                      value={step.command ?? ''}
                      options={[
                        { value: '', label: isLoading ? 'Loading commands…' : connId ? 'Select command' : 'Select a connection first' },
                        ...definitions.map((d) => ({ value: d.id, label: d.name })),
                      ]}
                      onChange={(command) => {
                        const next = definitions.find((d) => d.id === command);
                        setStep(i, { command: command || undefined, input: next ? defaultInput(next) : {} });
                      }}
                      className="!h-8 !text-xs w-full"
                    />

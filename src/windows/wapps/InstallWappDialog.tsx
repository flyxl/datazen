...
import { wappCommands, type WappPackageKind } from '../../commands/wapps';
...
export interface InstallWappDialogProps {
  open: boolean;
  onClose: () => void;
 ...
 */
export function InstallWappDialog({ open, onClose, onInstalled }: InstallWappDialogProps) {
  const { t } = useI18n();
  const [step, setStep] = useState<InstallStep>('select');
  const [pickToken, setPickToken] = useState<string | null>(null);
  const [packageLabel, setPackageLabel] = useState('');
  const [manifest, setManifest] = useState<WappManifest | null>(null);
  ...
  const handlePick = async (packageKind: WappPackageKind) => {
    if (inspecting || installing) return;
    setInspecting(true);
    setError(null);
    try {
      const preview = await wappCommands.inspectWappPackageWithDialog(packageKind);
      if (!preview) {
        return;
      }
  ...
  const handleInstall = async () => {
    ...
    setError(null);
    try {
      const installed = await wappCommands.installWapp(pickToken);

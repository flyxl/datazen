...
const pack = {
  ...
  'settings.extensions.empty': '当前构建中没有可用的扩展。',
  'settings.theme': '主题',
  ...
  'settings.editorCompletionQuotePolicyHint':
    '控制 SQL 编辑器补全字段名时是否添加引号。默认不加引号，保留关键字将自动添加引号。',
  'settings.quotePolicy.unquoted': '智能无引号（推荐，关键字自动加引号）',
  'settings.quotePolicy.both': '同时提供（无引号 + 带引号）',
  'settings.quotePolicy.always': '始终添加引号',
  'settings.keymap.preset': '快捷键方案',
  'settings.keymap.preset.default': '默认 (DataZen)',
  'settings.keymap.preset.dbeaver': 'DBeaver',
  'settings.keymap.preset.navicat': 'Navicat',
  'settings.keymap.customize': '自定义快捷键',
  'settings.keymap.resetCustom': '恢复方案默认',
  'keymap.action.execute': '执行当前语句',
  'keymap.action.executeAll': '执行全部语句',
  'keymap.action.newQuery': '新建查询页',
  'keymap.action.closeTab': '关闭当前标签',
  'keymap.action.saveQuery': '保存查询',
  ...
  'settings.ai.modelsTitle': '已配置模型',
  'settings.ai.addModel': '添加模型',
  'settings.ai.editModel': '编辑模型',
  'settings.ai.modelName': '配置名称',
  'settings.ai.modelNamePlaceholder': '例如：本地 Ollama (Qwen 2.5)、云端 Claude 3.7',
  'settings.ai.defaultBadge': '默认',
  'settings.ai.setDefault': '设为默认',
  'settings.ai.duplicate': '复制',
  'settings.ai.noModels': '暂无已配置的模型。点击添加模型开始使用。',
  'settings.ai.tabBasic': '基础配置',
  'settings.ai.tabSafety': '安全门控',
  'settings.ai.safetyPreset': '预设策略',
  'settings.ai.presetCloudStrict': '公网严格',
  'settings.ai.presetEnterpriseBalanced': '企业平衡',
  'settings.ai.presetLocalTrust': '本地私有信任',
  'settings.ai.presetCustom': '自定义',
  'settings.ai.dataEgressLevel': '数据外发策略',
  'settings.ai.egressStrict': '严格模式（清除所有数据行与样本）',
  'settings.ai.egressSampleMasked': '样例脱敏（允许少量样本行并自动掩码）',
  'settings.ai.egressUnrestricted': '完全放行（允许携带完整结果，仅限本地/内网）',
  'settings.ai.maxSampleRows': '最大样本行数',
  'settings.ai.dbToolPolicy': '数据库工具权限',
  'settings.ai.mcpToolPolicy': 'MCP 工具权限',
  'settings.ai.policyDisabled': '禁止调用',
  'settings.ai.policyReadOnly': '仅只读',
  'settings.ai.policyRequireConfirm': '需人工确认',
  'settings.ai.policyUnrestricted': '全自动执行',
  'settings.ai.requireSqlConfirm': 'AI 生成 SQL 执行前需二次确认',
  'settings.ai.unrestrictedWarningTitle': '安全告警：数据完全放行风险',
  'settings.ai.unrestrictedWarningMessage':
    '您正试图将完整真实查询结果集发送至远程公网端点，这可能导致敏感业务数据泄露。确定要继续吗？',
  ...
  'settings.mcpClient.title': '外部 MCP 服务',
} as const;
export default pack;

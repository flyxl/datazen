...
const WAPPS_ROOT = resolve(__dirname, '../../../packages/wapps');
...
function listWappDirs(): string[] {
  return readdirSync(WAPPS_ROOT, { withFileTypes: true })
...
describe('repo wapp packages', () => {
  const dirNames = listWappDirs();

  it('includes the converted community theme and the sample wapp', () => {
  ...
  it.each(dirNames)(
    ...
    (dirName) => {
      ...
      for (const theme of manifest.contributes?.themes ?? []) {
        ...
        if (theme.iconsDir) {
          const iconsPath = join(WAPPS_ROOT, dirName, theme.iconsDir);
          expect(
      ...
      for (const rel of declared.filter(Boolean) as string[]) {
        ...
        expect(existsSync(join(WAPPS_ROOT, dirName, rel)), `${dirName}: ${rel} missing`).toBe(true);
      }
    },
  );
});
...
describe('community.slate-blue theme contract (converted extension)', () => {
  const packDir = join(WAPPS_ROOT, 'community.slate-blue');
...
describe('extensions README contract', () => {
  it('documents layout, install flow, and the legacy-parity theme fields', () => {
    const readme = readFileSync(join(WAPPS_ROOT, 'README.md'), 'utf8');
    expect(readme).toContain('manifest.json');
    expect(readme).toContain('contributes');
    expect(readme).toMatch(/应用管理页|插件管理页/); // install entry point

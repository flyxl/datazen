...
const isLeaf = (node: SqlNamespace): node is readonly string[] => Array.isArray(node);

function isNamespaceEmpty(tree: SqlNamespace): boolean {
  return !isLeaf(tree) && Object.keys(tree).length === 0;
}

function flatFromTables(tables: TableInfo[], views: TableInfo[]): SqlNamespace {
  const result: Record<string, SqlNamespace> = {};
  for (const t of [...tables, ...views]) result[t.name] = [];
  return result;
}

export function hoistNamespaceChild(tree: SqlNamespace, childKey: string | null | undefined): SqlNamespace {
  ...
  if (isLeaf(child)) {
    ...
    for (const name of child) if (!(name in out)) out[name] = [];
    return out;
  }
  const out: Record<string, SqlNamespace> = { ...tree };
  for (const [key, value] of Object.entries(child)) if (!(key in out)) out[key] = value;
  return out;
}

export function hoistNamespacePath(tree: SqlNamespace, path: readonly string[] | undefined): SqlNamespace {
  let out = tree;
  for (const seg of path ?? []) out = hoistNamespaceChild(out, seg);
  return out;
}
...
function mergeUnqualifiedTables(tree: SqlNamespace, tables: TableInfo[], views: TableInfo[]): SqlNamespace {
  if (tables.length === 0 && views.length === 0) return tree;
  const out: Record<string, SqlNamespace> = isLeaf(tree) ? {} : { ...tree };
  for (const item of [...tables, ...views]) if (!(item.name in out)) out[item.name] = [];
  return out;
}

export function buildEditorSchema({ namespaceTree, tables, views, columnMap, currentDatabase, hoistPath }: BuildEditorSchemaInput): SqlNamespace {
